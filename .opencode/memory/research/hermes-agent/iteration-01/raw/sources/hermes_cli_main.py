...
Usage:
    ...
    hermes honcho sessions                 # List directory тЖТ session name mappings
    ...
    hermes honcho migrate                  # Step-by-step migration guide: OpenClaw native тЖТ Hermes + Honcho
...
# IMPORTANT: hermes_bootstrap must be the very first import тАФ it sets up
...
# setup is skipped on Windows тАФ degraded, not broken.  POSIX is unaffected.
try:
...
def _set_process_title() -> None:
    ...
    Purely cosmetic тАФ non-fatal on any platform.

    Strategy (try in order):
      1. ``setproctitle`` (opt-in dep тАФ installed via ``hermes tools`` or
         ``pip install setproctitle``, or bundled in a future release).
      2. ctypes ``prctl(PR_SET_NAME)`` (Linux only, 15-char limit).
      3. ctypes ``pthread_setname_np`` (macOS only, kernel thread name тАФ
         changes lldb/top but not ``ps aux``).
      4. No-op on Windows (the .exe name is already ``hermes.exe``).
    """
    # Strategy 1: setproctitle (best тАФ works on macOS, Linux, BSD)
    try:
    ...
    try:
        ...
        # Windows: the .exe name is already ``hermes.exe`` тАФ nothing to do.
    except Exception:
        pass
...
def _config_default_interface_early() -> str:
    ...
    try:
        ...
        value = "cli"  # best-effort тАФ default to classic REPL on any error
...
# Mouse-tracking residue suppression тАФ runs BEFORE every other import on the
# TUI hot path so the terminal stops emitting SGR/X10 mouse reports while the
# Python launcher is still doing imports (тЙИ100тАУ300ms in cooked + echo mode,
...
# ``^[[<тАжM`` text. The TUI itself runs `resetTerminalModes()` again in
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`hermes --tui тАж >log`, CI capture):
...
# ---------------------------------------------------------------------------
# Profile override тАФ MUST happen before any hermes module import.
#
...
def _apply_profile_override() -> None:
    ...
    # still read active_profile тАФ the user may have switched profiles via
    ...
    # would silently redirect the default gateway into that profile тАФ yielding a
...
# Bridge security.redact_secrets from config.yaml тЖТ HERMES_REDACT_SECRETS env
# var BEFORE hermes_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins тАФ this is config.yaml fallback only.
#
# We also read network.force_ipv4 from the same yaml load to avoid two
# separate config.yaml reads (saves ~17ms on every CLI startup тАФ the second
...
try:
    ...
    pass  # best-effort тАФ redaction stays at default (enabled) on config errors

# Initialize centralized file logging early тАФ all `hermes` subcommands
...
try:
    ...
    pass  # best-effort тАФ don't crash the CLI if logging setup fails

# Apply IPv4 preference early, before any HTTP clients are created.
# We already determined whether to force IPv4 from the raw yaml read above тАФ
...
if _FORCE_IPV4_EARLY:
    try:
        ...
        pass  # best-effort тАФ don't crash if hermes_constants not importable yet
...
def _read_git_revision_fingerprint(repo_root: Path) -> str | None:
    ...
    try:
        ...
        if head.startswith("ref:"):
            ...
            # Ref name is known but unresolved тАФ still stable across launches,
...
def _has_any_provider_configured() -> bool:
    ...
    # OPENAI_BASE_URL alone counts тАФ local models (vLLM, llama.cpp, etc.)
    ...
    # Check config.yaml тАФ if model is a dict with an explicit provider set,
    ...
    # Only count these if Hermes has been explicitly configured тАФ Claude Code
    ...
    try:
        ...
        def _curses_browse(stdscr):
            ...
            while True:
                ...
                # Header line
                if search_text:
                    header = f"  Browse sessions тАФ filter: {search_text}тЦИ"
                    header_attr = curses.A_BOLD
                    if curses.has_colors():
                        header_attr |= curses.color_pair(3)
                else:
                    header = "  Browse sessions тАФ тЖСтЖУ navigate  Enter select  Type to filter  Esc quit"
                    header_attr = curses.A_BOLD
                    if curses.has_colors():
                ...
                if key in {curses.KEY_UP,}:
                    ...
                    # Printable character тЖТ add to search filter
                    search_text += chr(key)
                    filtered = [s for s in sessions if _match(s, search_text)]
                    cursor = 0
                    scroll_offset = 0
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    # users тАФ Podman uses per-user namespaces, Docker needs group access.
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"sudo for {backend} тАФ the -n (non-interactive) flag is required\n"
...
def _print_tui_exit_summary(
    ...
    try:
        ...
        if message_count == 0:
            return  # No real conversation тАФ don't show resume info
...
on dev-dependencies that are *also* declared as peers тАФ the canonical
...
def _workspace_root(dir: Path) -> Path:
    ...
    ``npm install`` cwd stay consistent тАФ a single helper prevents
...
def _tui_need_npm_install(root: Path) -> bool:
    ...
    ``package.json``), skip reinstall entirely тАФ the bundle is self-contained
    ...
    *root* (``ui-tui/dist/entry.js``).
    ...
    For each entry in the root lock's ``packages`` map:
      - missing from hidden lock тЖТ reinstall (unless the entry is marked
      ...
      - present but with differing fields (excluding npm-written runtime
        annotations like ``ideallyInert``) тЖТ reinstall

    Extra entries that exist only in the hidden lock are ignored тАФ stale
...
def _ensure_tui_node() -> None:
    ...
    new binaries in this Python process тАФ regardless of which version manager
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev тЖТ tsx src; else node dist (HERMES_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()
    ...
    def _node_bin(bin: str) -> str:
        ...
        if not path and bin == "node":
            try:
                ...
                pass
        if not path:
            print(f"{bin} not found тАФ install Node.js to use the TUI.")
            sys.exit(1)
        return path
    ...
    if (
        ...
        if not os.environ.get("HERMES_QUIET"):
            print("Installing TUI dependenciesтАж")
...
def _read_cgroup_memory_limit() -> Optional[int]:
    ...
    OOM-killer SIGKILLs Node before V8's own heap monitor ever fires тАФ which
    ...
    for path in candidates:
        ...
        # 0x7FFFFFFFFFFFF000 тЙИ 9.2 EB, sometimes PAGE_COUNTER_MAX). Anything
        # at/above ~1 PB is effectively unconstrained тАФ treat as no limit.
        if limit >= (1 << 50):
            return None
        return limit
    return None
...
def _resolve_tui_heap_mb(default_mb: int = 8192) -> int:
    ...
    clamped to a sane floor (1536MB тАФ below this V8 GC-thrashes and the TUI
...
def _launch_tui(
    ...
    if worktree:
        try:
            ...
            print(f"тЬЧ Failed to create TUI worktree: {exc}", file=sys.stderr)
            wt_info = None
        if not wt_info:
            sys.exit(1)
    ...
    # Guarantee a generous V8 heap for the TUI. Default node cap is ~1.5тАУ4GB
    ...
    # SIGKILLs Node тАФ running no JS handler, writing no breadcrumb, leaving the
...
def _pin_kanban_board_env() -> None:
    ...
    (``hermes kanban тАж``) resolve the board on different paths: the env-pin if
...
def _sync_bundled_skills_quietly() -> None:
    ...
    interaction with Hermes тАФ chat, dashboard (the desktop GUI's backend),
...
def _resolve_use_tui(args) -> bool:
    ...
    Precedence (highest first):
      1. ``--cli`` flag         тЖТ always classic REPL
      2. ``--tui`` flag / ``HERMES_TUI=1`` тЖТ always TUI
      3. ``display.interface`` config value ("cli" | "tui")
      4. default тЖТ classic REPL
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            # -c "session name" тАФ resolve by title or ID
            ...
            # -c with no argument тАФ continue the most recent session
    ...
    if resume_val:
        ...
        if resolved:
            args.resume = resolved
        # If resolution fails, keep the original value тАФ _init_agent will
        # report "Session not found" with the original input

    # xAI retirement warning тАФ one-shot, non-blocking, never fails startup
    try:
        ...
        if _retired_xai_refs:
            sys.stderr.write(
                f"\033[33mтЪа xAI retires {len(_retired_xai_refs)} model(s) "
                f"in your config on {RETIREMENT_DATE}:\033[0m\n"
            )
            for _ref in _retired_xai_refs:
                sys.stderr.write(f"  \033[33mтЪа\033[0m {format_issue(_ref)}\n")
    ...
    # import time). Credentials in .env are still loaded тАФ this flag only
...
def cmd_proxy(args):
    ...
    # Lazy import тАФ pulls in aiohttp, which is gated behind an extras install
...
def cmd_whatsapp(args):
    ...
    print()
    print("тЪХ WhatsApp Setup")
    print("=" * 50)

    # тФАтФА Step 1: Choose mode тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    if not current_mode:
        ...
        print("     People message the bot's number directly тАФ cleanest experience.")
        print(
        ...
        if choice == "1":
            ...
            wa_mode = "bot"
            print("  тЬУ Mode: separate bot number")
            print()
            print("  тФМтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФР")
            print("  тФВ  Getting a second number for the bot:           тФВ")
            print("  тФВ                                                 тФВ")
            print("  тФВ  Easiest: Install WhatsApp Business (free app)  тФВ")
            print("  тФВ  on your phone with a second number:            тФВ")
            print("  тФВ    тАв Dual-SIM: use your 2nd SIM slot            тФВ")
            print("  тФВ    тАв Google Voice: free US number (voice.google) тФВ")
            print("  тФВ    тАв Prepaid SIM: $3-10, verify once            тФВ")
            print("  тФВ                                                 тФВ")
            print("  тФВ  WhatsApp Business runs alongside your personal тФВ")
            print("  тФВ  WhatsApp тАФ no second phone needed.             тФВ")
            print("  тФФтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФШ")
        else:
            save_env_value("WHATSAPP_MODE", "self-chat")
            wa_mode = "self-chat"
            print("  тЬУ Mode: personal number (self-chat)")
    else:
    ...
    # тФАтФА Step 2: Mode is selected, will enable WhatsApp only after pairing тФАтФА
    ...
    # Now: aborted setup leaves WHATSAPP_ENABLED unset тЖТ gateway skips it.
    # Re-runs that already have WHATSAPP_ENABLED=true (from a prior
    # successful pairing) stay enabled тАФ we just don't write it pre-emptively.
    print()
    if (get_env_value("WHATSAPP_ENABLED") or "").lower() == "true":
        print("тЬУ WhatsApp is already enabled")

    # тФАтФА Step 3: Allowed users тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    current_users = get_env_value("WHATSAPP_ALLOWED_USERS") or ""
    if current_users:
        print(f"тЬУ Allowed users: {current_users}")
        try:
        ...
        if phone:
            save_env_value("WHATSAPP_ALLOWED_USERS", phone.replace(" ", ""))
            print(f"  тЬУ Allowed users set: {phone}")
        else:
            print("  тЪа No allowlist тАФ the agent will respond to ALL incoming messages")

    # тФАтФА Step 4: Install bridge dependencies тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    project_root = Path(__file__).resolve().parents[1]
    bridge_dir = project_root / "scripts" / "whatsapp-bridge"
    bridge_script = bridge_dir / "bridge.js"

    if not bridge_script.exists():
        print(f"\nтЬЧ Bridge script not found at {bridge_script}")
        return

    if not (bridge_dir / "node_modules").exists():
        print(
            "\nтЖТ Installing WhatsApp bridge dependencies (this can take a few minutes)..."
        )
        npm = shutil.which("npm")
        if not npm:
            print("  тЬЧ npm not found on PATH тАФ install Node.js first")
            return
        try:
            result = subprocess.run(
            ...
            print("\n  тЬЧ Install cancelled")
            return
        if result.returncode != 0:
            err = (result.stderr or "").strip()
            preview = "\n".join(err.splitlines()[-30:]) if err else "(no output)"
            print("  тЬЧ npm install failed:")
            print(preview)
            return
        print("  тЬУ Dependencies installed")
    else:
        print("тЬУ Bridge dependencies already installed")

    # тФАтФА Step 5: Check for existing session тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    session_dir = get_hermes_home() / "whatsapp" / "session"
    session_dir.mkdir(parents=True, exist_ok=True)

    if (session_dir / "creds.json").exists():
        print("тЬУ Existing WhatsApp session found")
        try:
            response = input(
        ...
        if response.lower() in {"y", "yes"}:
            ...
            session_dir.mkdir(parents=True, exist_ok=True)
            print("  тЬУ Session cleared")
        else:
            # Existing pairing тАФ ensure WHATSAPP_ENABLED reflects that.
            ...
            print("\nтЬУ WhatsApp is configured and paired!")
            print("  Start the gateway with: hermes gateway")
            return

    # тФАтФА Step 6: QR code pairing тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    print()
    print("тФА" * 50)
    if wa_mode == "bot":
        print("ЁЯУ▒ Open WhatsApp (or WhatsApp Business) on the")
        print("   phone with the BOT's number, then scan:")
    else:
        print("ЁЯУ▒ Open WhatsApp on your phone, then scan:")
    print()
    print("   Settings тЖТ Linked Devices тЖТ Link a Device")
    print("тФА" * 50)
    print()
    ...
    # тФАтФА Step 7: Post-pairing тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    print()
    if (session_dir / "creds.json").exists():
        ...
        save_env_value("WHATSAPP_ENABLED", "true")
        print("тЬУ WhatsApp paired successfully!")
        print()
        if wa_mode == "bot":
            print("  Next steps:")
            ...
            print()
            print("  Tip: Agent responses are prefixed with 'тЪХ Hermes Agent'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  hermes gateway")
            print("    2. Open WhatsApp тЖТ Message Yourself")
            print("    3. Type a message тАФ the agent will reply")
            print()
            print("  Tip: Agent responses are prefixed with 'тЪХ Hermes Agent'")
        ...
        print("тЪа Pairing may not have completed. Run 'hermes whatsapp' to try again.")
...
def cmd_whatsapp_cloud(args):
    ...
    Distinct from ``hermes whatsapp`` (the Baileys bridge wizard) тАФ the
...
def cmd_postinstall(args):
    ...
    print("тЪХ Hermes post-install bootstrap")
    print()
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("тЬУ Post-install complete.")


def cmd_model(args):
    """Select default model тАФ starts with provider selection, then model picker."""
    _require_tty("model")
...
def select_provider_and_model(args=None):
    ...
    #
    # Canonical providers are folded into top-level groups (display only тАФ see
    # PROVIDER_GROUPS in hermes_cli/models.py). A multi-member group shows one
    # row ("Kimi / Moonshot тЦ╕"); picking it opens a member sub-picker that
    ...
    #   members == [] тЖТ leaf row, key is a provider slug / action
    #   members != [] тЖТ group row, key is "group:<gid>"
    ...
    for row in grouped_rows:
        if row["kind"] == "group":
            ...
            label = f"{row['label']} тЦ╕ ({group_desc})" if group_desc else f"{row['label']} тЦ╕"
            key = f"group:{gid}"
            ...
            members = []
        if is_active:
            ordered.append((key, f"{label}  тЖР currently active", members))
    ...
    for key, provider_info in _custom_provider_map.items():
        ...
        model_hint = f" тАФ {saved_model}" if saved_model else ""
        label = f"{name} ({short_url}){model_hint}"
        if active and key == active:
            ordered.append((key, f"{label}  тЖР currently active", []))
            default_idx = len(ordered) - 1
        else:
    ...
    # Group row тЖТ drill into a member sub-picker. Default to the active member
    ...
    if selected_members:
        ...
        group_label = ordered[provider_idx][1].split(" тЦ╕", 1)[0]
    ...
    # тФАтФА Post-switch cleanup: clear stale OPENAI_BASE_URL тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
...
# `hermes model` provider picker. It does NOT re-run credential setup тАФ it
...
def _save_aux_choice(
    ...
    *,
    ...
    Only writes the four routing fields тАФ timeout, download_timeout, and any
    ...
    for task, _name, _desc in _all_aux_tasks():
        ...
        for field in ("model", "base_url", "api_key"):
            if entry.get(field):
                entry[field] = ""
                changed = True
        # Preserve timeout/download_timeout тАФ those are user-tuned, not routing
        if changed:
            count += 1
    save_config(cfg)
    return count
...
def _aux_config_menu() -> None:
    """Top-level auxiliary-model picker тАФ choose a task to configure.
    ...
    while True:
        ...
        print()
        print("  Auxiliary models тАФ side-task routing")
        print()
        print("  Side tasks (vision, compression, web extraction, etc.) default")
        print('  to your main chat model.  "auto" means "use my main model" тАФ')
...
def _aux_select_for_task(task: str) -> None:
    ...
    inside the aux picker тАФ users set up new providers through the normal
    ...
    # "auto" always first
    auto_marker = (
        "  тЖР current" if current_provider == "auto" and not current_base_url else ""
    )
    ...
    for p in providers:
        ...
        model_hint = f" тАФ {total} models" if total else ""
        marker = (
            "  тЖР current" if slug == current_provider and not current_base_url else ""
        )
        entries.append((slug, f"{name}{model_hint}{marker}", list(models)))

    # Custom endpoint (raw base_url)
    custom_marker = "  тЖР current" if current_base_url else ""
    entries.append(("__custom__", f"Custom endpoint (direct URL){custom_marker}", []))
    entries.append(("__back__", "Back", []))

    print()
    print(f"  Configure {display_name} тАФ current: {_format_aux_current(task_cfg)}")
    print()
    ...
    # Regular provider тАФ pick a model from its curated list
...
def _prompt_provider_choice(choices, *, default=0, title="Select provider:"):
    ...
    for i, c in enumerate(choices, 1):
        marker = "тЖТ" if i - 1 == default else " "
        print(f"  {marker} {i}. {c}")
    print()
    while True:
        try:
...
def _save_custom_provider(
    ...
    Deduplicates by base_url тАФ if the URL already exists, updates the
    ...
    # Check if this URL is already saved тАФ update model/context_length if so
    ...
    cfg["custom_providers"] = providers
    save_config(cfg)
    print(f'  ЁЯТ╛ Saved to custom providers as "{name}" (edit in config.yaml)')
...
def _remove_custom_provider(config):
    ...
    print(f'тЬЕ Removed "{removed_name}" from custom providers.')
...
# the catalog eagerly costs ~55ms on every `hermes` invocation тАФ including
...
def _prompt_reasoning_effort_selection(efforts, current_effort=""):
    ...
    def _label(effort):
        if effort == current_effort:
            return f"{effort}  тЖР currently in use"
        return effort
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    ...
    ``return`` immediately тАФ the user cancelled entry, declined to replace, or
    ...
    # First-time entry тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    if not existing_key:
    ...
    # Already configured тАФ offer K / R / C тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    from hermes_cli.env_loader import format_secret_source_suffix

    source_suffix = format_secret_source_suffix(key_env) if key_env else ""
    print(f"  {pconfig.name} API key: {existing_key[:8]}... тЬУ{source_suffix}")
    if not key_env:
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            print("  тЬУ Claude Code credentials linked.")
    ...
    try:
        print()
        print("  Running 'claude setup-token' тАФ follow the prompts below.")
        ...
        if token:
            ...
            print("  тЬУ OAuth credentials saved.")
            return True

        # Subprocess completed but no token auto-detected тАФ ask user to paste
        print()
        ...
        if manual_token:
            ...
            print("  тЬУ Setup-token saved.")
            return True

        print("  тЪа Could not detect saved credentials.")
        return False

    except FileNotFoundError:
        # Claude CLI not installed тАФ guide user through manual setup
        print()
        ...
        if token:
            ...
            print("  тЬУ Setup-token saved.")
            return True
        print("  Cancelled тАФ install Claude Code and try again.")
        return False
...
def cmd_slack(args):
    ...
    Dispatches ``hermes slack <subcommand>``. Currently supports:
      manifest тАФ print or write a Slack app manifest with every gateway
                 command registered as a first-class slash.
    """
    sub = getattr(args, "slack_command", None)
    if sub in {None, ""}:
        # No subcommand тАФ print usage hint.
        print(
...
def _print_version_info(*, check_updates: bool = True) -> None:
    ...
    # ``import openai`` тАФ the SDK drags in ~800ms of pydantic-backed type
    ...
    # Show update status (synchronous тАФ acceptable since user asked for version info)
    try:
        ...
        if behind and behind > 0:
            ...
            print(
                f"Update available: {behind} {commits_word} behind тАФ "
...
def cmd_uninstall(args):
    ...
    # Must run before any TTY gate тАФ it's called from a non-interactive child.
...
# of these fail to parse after a pull, the CLI is bricked тАФ the user can't
...
def _validate_critical_files_syntax(root) -> tuple[bool, str | None, str | None]:
    ...
    Python version. The pyc is discarded on function return either way тАФ
    ...
    with tempfile.TemporaryDirectory(prefix="hermes-syntax-check-") as tmpdir:
        for relpath in _UPDATE_CRITICAL_FILES:
            path = root / relpath
            if not path.exists():
                # Missing file is suspicious but not necessarily fatal тАФ a future
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    # Timeout тАФ clean up and use default
    prompt_path.unlink(missing_ok=True)
    response_path.unlink(missing_ok=True)
...
def _run_with_idle_timeout(
    ...
    *,
    ...
    reboot тАФ leaving the editable install in a half-state with the
    ...
    stderr, and an integer returncode. Never raises on idle timeout тАФ
    ...
    def _reader() -> None:
        ...
        for line in proc.stdout:
            try:
                ...
                # Windows cp1252 fallback тАФ same pattern as _say().
    ...
    if idle_killed:
        msg = (
            f"\n  тЪа Build produced no output for {idle_timeout_seconds}s тАФ terminated.\n"
...
def _nixos_build_env() -> dict[str, str] | None:
    ...
    does a bare ``PATH`` lookup тАФ which fails on NixOS.

    Two-tier resolution:
    1. Fast path тАФ the hermes venv's python3 (present in managed installs)
    2. Fallback тАФ resolves the absolute python3 path via ``nix-shell``
    ...
    # python3 already on PATH тАФ nothing to do
    if shutil.which("python3"):
        return None

    # Tier 1: fast path тАФ hermes venv python3, no nix-shell overhead
    for venv_name in ("venv", ".venv"):
        venv_python = PROJECT_ROOT / venv_name / "bin" / "python3"
        if venv_python.exists():
            return {**os.environ, "PYTHON": str(venv_python)}

    # Tier 2: nix-shell fallback тАФ resolves the absolute python3 path once.
    # Slower (~2тАУ5 s for the nix-shell eval) but always works, even without
    ...
    try:
        ...
        pass  # nix-shell not available тАФ caller will get None
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    sync on a WIP checkout).  Without this, ``npm install`` on npm тЙе 10 silently
    rewrites committed lockfiles (stripping ``"peer": true`` etc.), which leaves
    the working tree dirty and causes the next ``hermes update`` to stash the
    lockfile тАФ repeatedly.
    """
    # unicode-animations' postinstall animates to /dev/tty (bypasses
    # --silent/capture_output). It no-ops when CI is set тАФ same as the TUI
    ...
    if lockfile.exists():
        ...
        if ci_result.returncode == 0:
            return ci_result
        # Fall through to `npm install` тАФ lockfile may be out of sync on a
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    if r1.returncode != 0:
        _say(
            f"  {'тЬЧ' if fatal else 'тЪа'} Web UI npm install failed"
        ...
        return False
    # First attempt тАФ stream output via idle-timeout helper (issue #33788).
    ...
    if r2.returncode != 0:
        # Retry once after a short delay тАФ covers boot-time races on Windows
    ...
    if r2.returncode != 0:
        ...
        # (Windows Scheduled Tasks, CI) тАФ issue #23817.
        if dist_index.exists():
            _say("  тЪа Web UI build failed тАФ serving stale dist as fallback")
            if stderr_tail:
                _say(f"  Build error:\n  {stderr_tail}")
            return True

        _say(
            f"  {'тЬЧ' if fatal else 'тЪа'} Web UI build failed"
            + ("" if fatal else " (hermes web will not be available)")
        )
        _relay(r2)
        if fatal:
...
# ---------------------------------------------------------------------------
# Desktop build stamp тАФ content-hash based skip logic
# ---------------------------------------------------------------------------
...
def _compute_desktop_content_hash(project_root: Path) -> str:
    ...
    # Walk apps/desktop/ тАФ prune ignored directories in-place
...
def _electron_download_cache_dirs() -> list[Path]:
    ...
    corrupt zip here тАФ not a bad workspace install тАФ is what poisons the build.
...
def _purge_electron_build_cache(desktop_dir: Path) -> list[Path]:
    ...
    Root cause of the ``ENOENT тАж rename 'тАж/linux-unpacked/electron' ->
    'тАж/linux-unpacked/Hermes'`` desktop build failure: a corrupt zip in the
    ...
    most common corruption here тАФ it reads from the end-of-central-directory
    ...
    them and decide whether a retry is worthwhile (empty list тЗТ nothing to
...
def _redownload_electron_dist(
    ...
    *,
    ...
    release host is unreachable in some regions тАФ #47266), the dist is missing
...
def _stop_desktop_processes_locking_build(desktop_dir: Path) -> list[int]:
    ...
    delete the stale binary and dies with ``remove тАж\\Hermes.exe: Access is
    ...
    this desktop's ``release`` tree are stopped тАФ a packaged install elsewhere or
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"тЬЧ Hermes Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False

    # Reject symlinks тАФ chown/chmod must not follow an attacker-controlled
    ...
    try:
        ...
        print(f"тЬЧ Cannot stat Electron's Linux sandbox helper: {sandbox}")
        return False
    if not stat.S_ISREG(sandbox_lstat.st_mode):
        print(f"тЬЧ Electron's Linux sandbox helper is not a regular file: {sandbox}")
        return False
    ...
    if not sudo:
        print("тЬЧ Hermes Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False

    print("тЖТ Configuring Electron Linux sandbox helper (sudo required)...")
    for command in ([sudo, "chown", "root:root", str(sandbox)], [sudo, "chmod", "4755", str(sandbox)]):
        if subprocess.run(command, check=False).returncode != 0:
            print(f"тЬЧ Failed to configure Electron's Linux sandbox helper: {sandbox}")
            return False
    return True
...
def cmd_gui(args: argparse.Namespace):
    ...
    if skip_build:
        if source_mode:
            if not _desktop_dist_exists(desktop_dir):
                print(f"тЬЧ --skip-build --source was passed but no desktop dist found at: {desktop_dir / 'dist'}")
            ...
            if not (PROJECT_ROOT / "node_modules" / "electron" / "package.json").exists():
                print("тЬЧ --skip-build --source requires existing workspace dependencies.")
                print(f"  Install first:  cd {PROJECT_ROOT} && npm ci")
                print("  Or drop --skip-build to install dependencies and build automatically.")
                sys.exit(1)
            print(f"тЖТ Skipping desktop source build (--skip-build --source); using dist at {desktop_dir / 'dist'}")
        elif packaged_executable is None:
            print(f"тЬЧ --skip-build was passed but no packaged desktop app was found at: {desktop_dir / 'release'}")
            print("  Pre-build first:  cd apps/desktop && npm run pack")
            print("  Or drop --skip-build to package automatically.")
            sys.exit(1)
        else:
            print(f"тЖТ Skipping desktop package build (--skip-build); using {packaged_executable}")
    else:
        ...
        if not build_needed:
            ...
            print(f"тЬУ Desktop {build_label} is up to date (content stamp matches)")
        else:
            print("тЖТ Installing desktop workspace dependencies...")
            nixos_env = _nixos_build_env()
            install_result = _run_npm_install_deterministic(npm, PROJECT_ROOT, capture_output=False, env=nixos_env)
            if install_result.returncode != 0:
                print("тЬЧ Desktop dependency install failed")
                print(f"  Run manually:  cd {PROJECT_ROOT} && npm ci")
                sys.exit(install_result.returncode or 1)

            build_label = "source build" if source_mode else "packaged app"
            print(f"тЖТ Building desktop {build_label}...")
            ...
            if not source_mode:
                ...
                # Stop it first so the rebuild тАФ including the installer's
                # headless --update rebuild тАФ succeeds instead of failing cryptically.
                stopped = _stop_desktop_processes_locking_build(desktop_dir)
                if stopped:
                    print(f"  тЪа Stopped running desktop app to free the build output (pid {', '.join(map(str, stopped))})")
            ...
            if build_result.returncode != 0 and not source_mode:
                ...
                # bad zip. We do NOT try to prove the zip is corrupt ourselves тАФ
                ...
                if purged or restored:
                    print("  тЪа Desktop build failed; refreshed the Electron download and retrying once...")
                    for p in purged:
                        print(f"    - {p}")
            ...
            if build_result.returncode != 0 and not source_mode and not env.get("ELECTRON_MIRROR"):
                ...
                # "retrying" download log). Retry once via npmmirror.com тАФ the
                ...
                print("  тЪа Desktop build still failing; the Electron download from "
                ...
                # downloads Electron тАФ the mirror only helps if it drives
                ...
                if have_dist:
                    ...
                    print("  тЬЧ Could not re-download Electron from the mirror "
                          "(node_modules/electron/dist still missing)")
            if build_result.returncode != 0:
                print("тЬЧ Desktop GUI build failed")
            ...
            # Build succeeded тАФ write the stamp so next run can skip
    ...
    # happen here тАФ it would block the installer and, on Windows, the old exe
    ...
    if getattr(args, "build_only", False):
        if source_mode:
            if not _desktop_dist_exists(desktop_dir):
                print(f"тЬЧ --build-only --source produced no dist at: {desktop_dir / 'dist'}")
                sys.exit(1)
            print(f"тЬУ Desktop source build ready at {desktop_dir / 'dist'} (not launching; --build-only)")
        elif packaged_executable is None:
            print(f"тЬЧ --build-only produced no launchable app at: {desktop_dir / 'release'}")
            print("  Expected an unpacked Electron app for the current OS.")
            sys.exit(1)
        else:
            print(f"тЬУ Desktop packaged app ready: {packaged_executable} (not launching; --build-only)")
        return

    if source_mode:
        print("тЖТ Launching Hermes Desktop from source build...")
        launch_result = subprocess.run([npm, "exec", "--", "electron", "."], cwd=desktop_dir, env=env, check=False)
        sys.exit(launch_result.returncode)

    if packaged_executable is None:
        print(f"тЬЧ Desktop package build completed but no launchable app was found at: {desktop_dir / 'release'}")
        print("  Expected an unpacked Electron app for the current OS.")
        sys.exit(1)
    ...
    print(f"тЖТ Launching packaged Hermes Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    *,
    ...
    auth headers the old backend doesn't recognise тЖТ every API call 401s).

    The dashboard has no service manager (systemd / launchd), no PID file,
    and we can't know the original launch args тАФ so the only sane action
...
def _print_curator_first_run_notice() -> None:
    ...
    if state.get("last_run_at"):
        # Curator has run before (real or already seeded) тАФ no notice needed.
        return
    try:
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``hermes update`` is a high-attention surface тАФ surface the
    ...
    if not last_run_at:
        return  # no curator run yet тАФ first-run notice handles this case
    ...
    # Only print when there's something interesting to show тАФ i.e. the
    ...
    print()
    print(f"тД╣ Skill curator тАФ last run {when}")
...
def _kill_stale_dashboard_processes(
    ...
    recognise тЖТ every API call 401s).
    ...
    print()
    print(f"тЯ▓ Stopping {len(pids)} dashboard process(es) ({reason})")
    ...
    if sys.platform == "win32":
        ...
        # SIGTERM first тАФ give each process a chance to shut down cleanly
        # (uvicorn closes its socket, flushes logs, etc.).
        for pid in pids:
            try:
                os.kill(pid, _signal.SIGTERM)
            except ProcessLookupError:
                # Already gone тАФ count as killed.
                killed.append(pid)
    ...
    for pid in killed:
        print(f"    тЬУ stopped PID {pid}")
    for pid, err_msg in failed:
        print(f"    тЬЧ failed to stop PID {pid}: {err_msg}")
...
def _update_via_zip(args):
    ...
    # if the user asked for something else тАФ exactly the silent-divergence
    # bug --branch was added to prevent. Refuse to proceed in that case
    # rather than lie.
    branch = _resolve_update_branch(args)
    if branch != "main":
        print(
            f"тЬЧ --branch={branch} is not supported on the Windows ZIP-fallback "
            "update path."
        )
        print(
    ...
    print("тЖТ Downloading latest version...")
    tmp_dir = tempfile.mkdtemp(prefix="hermes-update-")
    try:
        zip_path = os.path.join(tmp_dir, f"hermes-agent-{branch}.zip")
        urlretrieve(zip_url, zip_path)

        print("тЖТ Extracting...")
        ...
        with zipfile.ZipFile(zip_path, "r") as zf:
            ...
            # should never contain symlinks тАФ they'd point outside the
        ...
        print(f"тЬУ Updated {update_count} items from ZIP")

    except Exception as e:
        print(f"тЬЧ ZIP update failed: {e}")
        sys.exit(1)
    finally:
    ...
    if removed:
        print(
            f"  тЬУ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        )

    # Reinstall Python dependencies. Prefer .[all], but if one optional extra
    # breaks on this machine, keep base deps and reinstall the remaining extras
    # individually so update does not silently strip working capabilities.
    print("тЖТ Updating Python dependencies...")

    from hermes_cli.managed_uv import ensure_uv, update_managed_uv

    # Keep managed uv current тАФ runs `uv self update` if we already have one.
    update_managed_uv()
    ...
    try:
        ...
        print("тЖТ Syncing bundled skills...")
        result = sync_skills(quiet=True)
        if result["copied"]:
            print(f"  + {len(result['copied'])} new: {', '.join(result['copied'])}")
        if result.get("updated"):
            print(
                f"  тЖС {len(result['updated'])} updated: {', '.join(result['updated'])}"
            )
        if result.get("user_modified"):
            print(f"  ~ {len(result['user_modified'])} user-modified (kept)")
        if result.get("cleaned"):
            print(f"  тИТ {len(result['cleaned'])} removed from manifest")
        if not result["copied"] and not result.get("updated"):
            print("  тЬУ Skills are up to date")
    except Exception:
        pass
    ...
    try:
        ...
        if seed_cache_from_checkout(PROJECT_ROOT):
            print("  тЬУ Model catalog cache refreshed from checkout")
    except Exception as e:
        logger.debug("Model catalog seed during zip update failed: %s", e)

    print()
    print("тЬУ Update complete!")
    try:
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    if unmerged.stdout.strip():
        print("тЖТ Clearing unmerged index entries from a previous conflict...")
    ...
    print("тЖТ Local changes detected тАФ stashing before update...")
    subprocess.run(
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        print()
        print("тЪа Local changes were stashed before updating.")
        print(
    ...
    print("тЖТ Restoring local changes...")
    ...
    # Check for unmerged (conflicted) files тАФ can happen even when returncode is 0
    unmerged = subprocess.run(
    ...
    if restore.returncode != 0 or has_conflicts:
        print("тЬЧ Update pulled new code, but restoring local changes hit conflicts.")
        ...
        print("\nYour stashed changes are preserved тАФ nothing is lost.")
        print(f"  Stash ref: {stash_ref}")

        # Always reset to clean state тАФ leaving conflict markers in source
        ...
        # Don't sys.exit тАФ the code update itself succeeded, only the stash
    ...
    if stash_selector is None:
        print(
            "тЪа Local changes were restored, but Hermes couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "тЪа Local changes were restored, but Hermes couldn't drop the saved stash entry."
            )
    ...
    print("тЪа Local changes were restored on top of the updated codebase.")
...
def _discard_stashed_changes(
    ...
    ``updates.non_interactive_local_changes: discard`` тАФ i.e. they've opted out
    ...
    explicitly captured) тАФ ignored paths like node_modules/venv/build outputs
    ...
    if stash_selector is None:
        print(
            "тЪа Configured to discard local changes on non-interactive update, "
    ...
    if drop.returncode != 0:
        print(
            "тЪа Configured to discard local changes, but Hermes couldn't drop "
    ...
    print("тЖТ Discarded local source changes (updates.non_interactive_local_changes=discard).")
    return True
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("тД╣ Your fork is not tracking the official Hermes repository.")
        ...
        if response in {"", "y", "yes"}:
            print("тЖТ Adding upstream remote...")
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  тЬУ Added upstream: https://github.com/NousResearch/hermes-agent.git"
                )
                has_upstream = True
            else:
                print("  тЬЧ Failed to add upstream remote. Skipping upstream sync.")
                return
        else:
            print(
    ...
    # origin/main, so there's no reason to pull every upstream ref тАФ and a bare
    # fetch drags in thousands of auto-generated branches.
    print()
    print("тЖТ Fetching upstream...")
    try:
        subprocess.run(
        ...
        print("  тЬЧ Failed to fetch upstream. Skipping upstream sync.")
        return
    ...
    if origin_ahead < 0 or upstream_ahead < 0:
        print("  тЬЧ Could not compare branches. Skipping upstream sync.")
        return

    # If origin/main has commits not on upstream, don't trample
    if origin_ahead > 0:
        print()
        print(f"тД╣ Your fork has {origin_ahead} commit(s) not on upstream.")
    ...
    if upstream_ahead == 0:
        print("  тЬУ Fork is up to date with upstream")
        return

    # origin/main is strictly behind upstream/main (can fast-forward)
    print()
    print(f"тЖТ Fork is {upstream_ahead} commit(s) behind upstream")
    print("тЖТ Pulling from upstream...")
    ...
    try:
        ...
        print(
            "  тЬЧ Failed to pull from upstream. You may need to resolve conflicts manually."
        )
        return

    print("  тЬУ Updated from upstream")

    # Try to sync fork back to origin
    print("тЖТ Syncing fork...")
    if _sync_fork_with_upstream(git_cmd, cwd):
        print("  тЬУ Fork synced with upstream")
    else:
        print(
            "  тД╣ Got updates from upstream but couldn't push to fork (no write access?)"
        )
...
def _invalidate_update_cache():
    ...
    The git repo is shared across profiles тАФ when one profile runs
    ``hermes update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware тАФ uses /opt/data in Docker)
    from hermes_constants import get_default_hermes_root
...
def _recover_from_interrupted_install() -> None:
    ...
    Triggered on launch when ``.update-incomplete`` is present тАФ meaning the
    ...
    Output: everything тАФ our status lines AND the streamed pip/uv install
    (which inherits fd 1) тАФ is routed to stderr.  Launches whose stdout is a
    ...
    # process holds it, skip тАФ it is running the same reinstall into the same
    ...
    try:
        ...
        # Couldn't create the lock (read-only fs, perms). Proceed unlocked тАФ
    ...
    try:
        ...
        print(
            "тЪа A previous `hermes update` was interrupted mid-install тАФ "
        ...
        try:
            ...
            _clear_update_incomplete_marker()
            print("тЬУ Dependency installation recovered тАФ your install is healthy again.")
        except Exception as exc:
            ...
            print("тЬЧ Could not auto-recover the interrupted install.")
    ...
    *,
    ...
    def _heartbeat() -> None:
        ...
        while not done.wait(heartbeat_interval_seconds):
            ...
            print(
                f"  тАж still installing dependencies ({elapsed}s elapsed)"
                " тАФ compiling Rust/C extensions can take several minutes",
                flush=True,
            )
...
def _hermes_exe_shims(scripts_dir: Path) -> list[Path]:
    ...
    they're regular Python scripts which can be replaced atomically тАФ no
...
def _detect_concurrent_hermes_instances(
    ...
    Windows blocks DELETE/REPLACE on a running .exe тАФ and even RENAME on the
    ...
    reports itself тАФ this matters on Windows where the setuptools .exe
    ...
    instances exist. Never raises тАФ process enumeration is best-effort.
    """
    if not _is_windows():
        return []
    ...
    # reports its own launcher as a concurrent instance тАФ a false positive
    # (issues #29341, #34795).
    #
    # Two robustness points learned from the field:
    #   1. Use ``proc.parents()`` тАФ it returns the WHOLE ancestor list in one
...
def _format_concurrent_instances_message(
    ...
    lines = ["тЬЧ Another hermes.exe is running:"]
...
def _quarantine_running_hermes_exe(
    ...
    ``FILE_SHARE_DELETE`` тАФ typically AV real-time scanners with transient
    ...
    where we used ``MOVEFILE_DELAY_UNTIL_REBOOT`` are NOT returned тАФ they
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        if scheduled:
            print(
                f"  тЪа {shim.name} is locked by another process; scheduled "
        ...
        # uv try its luck тАФ sometimes uv's own retry handling pulls through.
        print(
            f"  тЪа Could not quarantine {shim.name} ({last_exc.__class__.__name__}: "
...
def _schedule_replace_on_reboot(shim: Path, quarantine_target: Path) -> bool:
    ...
    runs on next boot тАФ at which point no process can hold the .exe.
...
def _run_quarantined_install(
    ...
    *,
    ...
    shims, and on Windows the live ``hermes.exe`` is the running process тАФ
    ...
    install that touches the shims is protected тАФ including the
    ...
    try:
        for stale in scripts_dir.glob("*.exe.old.*"):
            try:
                ...
                pass  # still locked or in use тАФ try again next run
    except OSError:
        pass
...
def _refresh_active_lazy_features() -> None:
    ...
    leaves those packages untouched тАФ so if we bump a pin in
    ...
    user never enabled stay quiet тАФ no churn for cold backends.
    ...
    print()
    print(f"тЖТ Refreshing {len(active)} active lazy backend(s)...")
    ...
    try:
        ...
        print(f"  тЪа Lazy refresh failed unexpectedly: {exc}")
        return
    ...
    if refreshed:
        print(f"  тЖС {len(refreshed)} refreshed: {', '.join(refreshed)}")
    if current:
        print(f"  тЬУ {len(current)} already current")
    if skipped:
    ...
    if failed:
        for feature, status in failed:
            ...
            if len(reason) > 200:
                reason = reason[:200] + "..."
            print(f"  тЪа {feature} failed to refresh: {reason}")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    try:
        ...
        print(
            "  тЪа Optional extras failed, reinstalling base dependencies and retrying extras individually..."
        )
    ...
    if installed_extras:
        print(
            f"  тЬУ Reinstalled optional extras individually: {', '.join(installed_extras)}"
        )
    if failed_extras:
        print(
            f"  тЪа Skipped optional extras that still failed: {', '.join(failed_extras)}"
        )
    ...
    # target venv. uv's incremental resolver has тАФ in the wild тАФ produced
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    update тАФ but the warning makes the partial install visible at the spot
    that caused it, instead of hours later in a downstream subprocess.
    """
    try:
        import tomllib  # Python 3.11+
    except ImportError:  # pragma: no cover тАФ Python < 3.11 unsupported but be safe
        return
    ...
    # Run the check inside the venv Python тАФ sys.executable here may be the
    ...
    print(
        f"  тЪа Verification: {len(missing)} declared dep(s) missing after install: "
        f"{', '.join(missing[:8])}{'...' if len(missing) > 8 else ''}"
    )
    print("  тЖТ Reinstalling base group with --reinstall to repair...")
    ...
    # purpose тАФ the missing dep is in *base* deps; rerunning the full all-
    ...
    try:
        ...
        print("  тЪа Repair install failed; check `hermes update` output above.")
        return

    still_missing = _missing_deps()
    if not still_missing:
        print("  тЬУ All declared core dependencies now installed")
        return
    ...
    print(
        f"  тЖТ Force-installing remaining missing dep(s): {', '.join(specs)}"
    )
    try:
        ...
        print(
            f"  тЪа Could not install: {', '.join(still_missing)}. "
    ...
    if final_missing:
        print(
            f"  тЪа Still missing after repair: {', '.join(final_missing)}. "
            "Run `hermes update --force` after closing other hermes processes."
        )
    else:
        print("  тЬУ All declared core dependencies now installed")
...
def _ensure_uv_for_termux(pip_cmd: list[str]) -> str | None:
    ...
    if not _is_termux_env():
        return None
    try:
        print("  тЖТ Termux detected: trying to install uv for faster dependency updates...")
...
def _update_node_dependencies() -> None:
    ...
    # workspaces тАФ but apps/desktop pulls in Electron as a devDependency,
    ...
    print("тЖТ Updating Node.js dependencies...")
    ...
    if root_result.returncode != 0:
        print("  тЪа npm install failed in repo root")
    ...
    if ws_result.returncode == 0:
        print("  тЬУ repo root + ui-tui, web workspaces (desktop skipped)")
    else:
        print("  тЪа npm workspace install failed")
...
class _UpdateOutputStream:
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
      ...
      process exit тАФ the update keeps going, only the on-screen output
      stops.
    ...
    def write(self, data):
        # Mirror to the log file first тАФ it's the most reliable destination.
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    **intentionally left alone** тАФ those are legitimate cancellation
    ...
    if hasattr(_signal, "SIGHUP"):
        try:
            ...
            # Called from a non-main thread тАФ not fatal.  The update still
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        # path) uses тАФ telling the user to "reinstall via curl" or that
    ...
    if method == "pip":
        ...
        if branch_explicit and branch != "main":
            print(f"тЪа --branch is ignored for PyPI installs (would have checked '{branch}').")
        result = check_via_pypi()
        if result is None:
            print("тЬЧ Could not reach PyPI to check for updates.")
            sys.exit(1)
        elif result == 0:
            print("тЬУ Already up to date.")
        else:
            print("тЪХ Update available on PyPI.")
            print(f"  Run '{recommended_update_command()}' to install.")
        return

    git_dir = PROJECT_ROOT / ".git"
    if not git_dir.exists():
        print("тЬЧ Not a git repository тАФ cannot check for updates.")
        sys.exit(1)
    ...
    if branch == "main":
        print("тЖТ Fetching from upstream...")
        ...
        if fetch_result.returncode != 0:
            ...
            print("тЖТ Fetching from origin...")
            fetch_result = subprocess.run(
    ...
    if fetch_result.returncode != 0:
        ...
        if "Could not resolve host" in stderr or "unable to access" in stderr:
            print("тЬЧ Network error тАФ cannot reach the remote repository.")
        elif "Authentication failed" in stderr or "could not read Username" in stderr:
            print("тЬЧ Authentication failed тАФ check your git credentials or SSH key.")
        else:
            print("тЬЧ Failed to fetch.")
            if stderr:
    ...
    if verify_result.returncode != 0:
        print(f"тЬЧ Branch '{branch}' not found on {compare_branch.split('/', 1)[0]}.")
        sys.exit(1)
    ...
    if behind == 0:
        ...
        commits_word = "commit" if behind == 1 else "commits"
        print(f"тЪХ Update available: {behind} {commits_word} behind {compare_branch}.")
        from hermes_cli.config import recommended_update_command
...
def _ensure_fhs_path_guard() -> None:
    ...
    if sys.platform != "linux":
        return
    try:
        if os.geteuid() != 0:  # windows-footgun: ok тАФ Linux FHS helper, guarded by sys.platform == "linux" above + AttributeError catch
            return
    ...
    try:
        ...
        return  # no bash or probe hung тАФ don't block update on this
    if probe.returncode == 0:
        return  # already on PATH, nothing to do

    path_line = 'export PATH="/usr/local/bin:$PATH"'
    path_comment = (
        "# Hermes Agent тАФ ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
    for candidate in (".bashrc", ".bash_profile"):
        ...
        try:
            ...
            print(f"  тЪа Could not update {cfg}: {e}")
            continue
        print(f"  тЬУ Added /usr/local/bin to PATH in {cfg}")
        wrote_any = True
    if wrote_any:
...
def _run_pre_update_backup(args) -> None:
    ...
    has it enabled.  Never raises тАФ a backup failure should not block the
    update itself.
    """
    # CLI flags win over config.  --no-backup beats --backup if both are set.
    if getattr(args, "no_backup", False):
        print("тЧЖ Pre-update backup: skipped (--no-backup)")
        print()
        return
    ...
    if not enabled and not force_backup:
        # Silent by default тАФ the backup is off, most users don't need to
    ...
    try:
        ...
        print(
            f"тЪа Pre-update backup: could not load backup module ({exc}); continuing update."
        )
        print()
        return

    print("тЧЖ Creating pre-update backup...")
    t0 = _time.monotonic()
    try:
        out_path = create_pre_update_backup(keep=int(keep))
    except Exception as exc:  # defensive тАФ helper already swallows, but just in case
        print(f"  тЪа Backup failed: {exc}")
    ...
    if out_path is None:
        print("  тЪа Backup skipped (no files found or write failed); continuing update.")
        print()
        return
...
def _pause_windows_gateways_for_update() -> dict | None:
    ...
    print("тЖТ Stopping Windows gateway process(es) before updating Hermes...")
    try:
    ...
    if profiles:
        print(f"  тЬУ Paused gateway profile(s): {', '.join(sorted(profiles))}")
    if force_killed:
        print(f"  тЖТ Force-stopped {len(force_killed)} gateway process(es)")

    if unmapped_pids:
        print(
            f"  тЖТ Stopped {len(unmapped_pids)} gateway process(es) without profile mapping"
        )
...
def _resume_windows_gateways_after_update(token: dict | None) -> None:
    ...
    if relaunched:
        print()
        print(f"  тЬУ Restarting Windows gateway profile(s): {', '.join(relaunched)}")
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    try:
        ...
        print(f"тЖТ Discarded npm lockfile churn ({len(dirty)} file(s))")
    except Exception:
...
def cmd_update(args):
    ...
    # Docker users can't ``git pull`` тАФ the image excludes ``.git`` from
...
def _cmd_update_pip(args):
    ...
    print(f"тЖТ Current version: {__version__}")
    print("тЖТ Checking PyPI for updates...")
    ...
    if is_uv_tool_install():
        if not uv:
            print("тЬЧ Detected a uv-tool install but managed uv install failed.")
    ...
    print(f"тЖТ Running: {' '.join(cmd)}")
    run_kwargs = {}
    if export_virtualenv:
        run_kwargs["env"] = {**os.environ, "VIRTUAL_ENV": sys.prefix}
    result = subprocess.run(cmd, **run_kwargs)
    if result.returncode != 0:
        print("тЬЧ Update failed")
        sys.exit(1)

    print("тЬУ Update complete! Restart hermes to use the new version.")


def _cmd_update_impl(args, gateway_mode: bool):
    """Body of ``cmd_update`` тАФ kept separate so the wrapper can always
    ...
    print("тЪХ Updating Hermes Agent...")
    print()
    ...
    # Pre-update backup тАФ runs before any git/file mutation so users can
    ...
    if not git_dir.exists():
        if sys.platform == "win32":
            ...
            if method == "pip":
                ...
                return
            print("тЬЧ Not a git repository. Please reinstall:")
            print(
    ...
    # Build git command once тАФ reused for fork detection and the update itself.
    git_cmd = ["git"]
    if sys.platform == "win32":
        git_cmd = ["git", "-c", "windows.appendAtomically=false"]
    ...
    # tree dirty тАФ forcing an autostash on every update and making branch
    ...
    try:
        ...
        # thousands of auto-generated branches тАФ an unscoped fetch can stall for
        # minutes on a non-single-branch checkout. Fetch only what we update
        # against.
        branch = _resolve_update_branch(args)

        print("тЖТ Fetching updates...")
        fetch_result = subprocess.run(
            git_cmd + ["fetch", "origin", branch],
            cwd=PROJECT_ROOT,
            capture_output=True,
        ...
        if fetch_result.returncode != 0:
            ...
            if "Could not resolve host" in stderr or "unable to access" in stderr:
                ...
                print(
                    "тЬЧ Authentication failed тАФ check your git credentials or SSH key."
                )
            else:
                print(f"тЬЧ Failed to fetch updates from origin.")
                if stderr:
                    print(f"  {stderr.splitlines()[0]}")
            sys.exit(1)
        ...
        # the same thing тАФ get HEAD onto the requested branch first, then
        # fast-forward.
        if current_branch != branch:
            ...
            print(f"  тЪа Currently on {label} тАФ switching to {branch} for update...")
            ...
            if checkout_result.returncode != 0:
                ...
                if track_result.returncode != 0:
                    ...
                    print(f"тЬЧ Branch '{branch}' does not exist locally or on origin.")
        ...
        print(f"тЖТ Found {commit_count} new commit(s)")
        ...
        try:
            ...
            if pre_update_snapshot_id:
                print(f"  тЬУ Pre-update snapshot: {pre_update_snapshot_id}")
        except Exception as exc:
            # Never let a snapshot failure block an update.
            logger.debug("Pre-update snapshot failed: %s", exc)

        print("тЖТ Pulling updates...")
        ...
        try:
            ...
            if pull_result.returncode != 0:
                # ff-only failed тАФ local and remote have diverged (e.g. upstream
                # force-pushed or rebase).  Since local changes are already
                # stashed, reset to match the remote exactly.
                print(
                    "  тЪа Fast-forward not possible (history diverged), resetting to match remote..."
                )
                ...
                if reset_result.returncode != 0:
                    print(f"тЬЧ Failed to reset to origin/{branch}.")
            ...
            if not syntax_ok:
                print()
                print("тЬЧ Pulled code has a syntax error in a critical file:")
                ...
                if pre_pull_sha:
                    print()
                    print(f"тЖТ Rolling back to {pre_pull_sha[:10]}...")
                    ...
                    if rollback_result.returncode == 0:
                        print("  тЬУ Rollback complete тАФ your install is unchanged.")
                        print("  Try ``hermes update`` again later once a fix lands.")
                    else:
                        print("  тЬЧ Rollback failed. Recover manually with:")
                        print(f"    cd {PROJECT_ROOT} && git reset --hard {pre_pull_sha}")
                        if rollback_result.stderr.strip():
                            print(f"    ({rollback_result.stderr.strip().splitlines()[0]})")
                else:
                    print()
                    print("  Could not capture pre-pull SHA тАФ recover manually with:")
            ...
            if auto_stash_ref is not None:
                # Don't attempt stash restore if the code update itself failed тАФ
                # working tree is in an unknown state.
                if not update_succeeded:
                    print(
                        f"  тД╣я╕П  Local changes preserved in stash (ref: {auto_stash_ref})"
                    )
        ...
        # Clear stale .pyc bytecode cache тАФ prevents ImportError on gateway
        ...
        if uv_bin:
            ...
            if _is_termux_env(uv_env):
                ...
                print("  тЖТ Termux detected: using uv + curated termux-all optional profile...")
            if _is_termux_env(uv_env) and _is_android_python():
                print("  тЖТ Termux/Android detected: prebuilding psutil with Linux source path compatibility...")
            ...
            if _is_termux_env():
                install_group = "termux-all"
                print("  тЖТ Termux detected: using curated termux-all optional profile...")
        ...
        # breadcrumb now тАФ the remaining steps (lazy refresh, node deps, web
        ...
        if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
            print("тЖТ Checking if desktop app needs rebuilding...")
            ...
            # again for the venv тАФ this covers a still-settling rebuild window
            # the first wait didn't fully catch.
            build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                build_result = subprocess.run(_desktop_build_cmd, cwd=PROJECT_ROOT, check=False)
            if build_result.returncode != 0:
                print("  тЪа Desktop build failed (non-fatal; run `hermes desktop` to retry)")

        print()
        print("тЬУ Code updated!")
        ...
        # current тАФ so copy it straight over ~/.hermes/cache/model_catalog.json
        ...
        try:
            ...
            pass  # non-fatal тАФ worst case a lazy import fails gracefully
        ...
        try:
            ...
            if all_profiles:
                print()
                print("тЖТ Syncing bundled skills to all profiles...")
                for p in all_profiles:
                    try:
                        ...
                        if r and r.get("skipped_opt_out"):
                            ...
                            if updated:
                                parts.append(f"тЖС{updated} updated")
                            if modified:
        ...
        try:
            ...
            if backfilled:
                print()
                print(
                    f"тЖТ Seeded .env for {len(backfilled)} profile(s) "
        ...
        print()
        print("тЖТ Checking configuration for new options...")
        ...
        if version_bump_only:
            # Nothing for the user to fill in тАФ only the config format version
            ...
            print()
            print(
                f"  тД╣ Updating config format (v{current_ver} тЖТ v{latest_ver})тАж"
            )
            try:
                migrate_config(interactive=False, quiet=True)
                print("  тЬУ Config format updated (no new settings to configure)")
            except Exception as _mig_err:
                print(f"  тЪая╕П  Config format update failed: {_mig_err}")
            ...
            def _print_items(items, label, key, fallback_key=None):
                ...
                for it in shown:
                    if isinstance(it, dict):
                        ...
                        desc = ""
                    if desc:
                        print(f"      тАв {name} тАФ {desc}")
                    else:
                        print(f"      тАв {name}")
                extra = len(items) - len(shown)
                if extra > 0:
                    print(f"      тАж and {extra} more")

            if missing_env:
                print(
                    f"  тЪая╕П  {len(missing_env)} new required setting(s) need configuration"
                )
                _print_items(missing_env, "New settings", "name")
            if missing_config:
                print(f"  тД╣я╕П  {len(missing_config)} new config option(s) available")
                _print_items(missing_config, "New options", "key")

            print()
            if assume_yes:
                print(
                    "  тД╣ --yes: auto-applying config migration (skipping API-key prompts)."
                )
                response = "y"
            elif gateway_mode:
                response = (
                ...
                print("  тД╣ Non-interactive session тАФ applying safe config migrations.")
                response = "auto"
            else:
                try:
                    response = (
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if results["env_added"] or results["config_added"]:
                    print()
                    print("тЬУ Configuration updated!")
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  тД╣ API keys require manual entry: hermes config migrate")
            else:
                print()
                print("Skipped. Run 'hermes config migrate' later to configure.")
        else:
            print("  тЬУ Configuration is up to date")
        ...
        try:
            ...
            if cron_restore:
                print()
                print(
                    "  тЪая╕П  cron/jobs.json was emptied during this update тАФ "
        ...
        # has never run тАФ i.e. the window where the ticker would otherwise
        ...
        # Most-recent curator run notice тАФ show-once per run. Surfaces the
        # rename map (`old-name тЖТ umbrella`) on the high-attention update
        ...
        try:
            if sys.platform == "darwin" and shutil.which("cua-driver"):
                ...
                print()
                print("тЖТ Refreshing cua-driver (Computer Use)...")
        ...
        # the cgroup (KillMode=mixed тЖТ SIGKILL to remaining processes),
        ...
        # timeout message.
        #
        # Writing the marker here тАФ after git pull + pip install succeed but
        # before we attempt the restart тАФ ensures the new gateway sees it
        ...
        try:
            ...
            def _resolve_manage_cmd(scope_: str, scope_cmd_: list, svc_name_: str):
                ...
                10-15s timeout тАФ the user sees the prompt flash and "exit
                ...
                non-interactive sudo (``sudo -n``) тАФ first a blanket probe,
                ...
                privileged restart anyway).  If neither works, return None тАФ
                ...
                if (
                    ...
                    and os.geteuid() != 0  # windows-footgun: ok тАФ systemd path, Linux-only
                ):
                    ...
                    try:
                        ...
                        if not sudo_ok:
                            # Blanket sudo refused тАФ a targeted sudoers entry
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            # None тЗТ no non-interactive privilege path; we
                            ...
                            # flash and "exit directly" тАФ reported June 2026).
                            ...
                            # request_restart(via_service=True) тЖТ drain тЖТ
                            ...
                            if _main_pid > 0:
                                print(
                                    f"  тЖТ {svc_name}: draining (up to {int(_drain_budget)}s)..."
                                )
                            ...
                            if _graceful_ok:
                                ...
                                # passwordless sudo) skip it тАФ systemd's own
                                ...
                                if _manage_cmd is None and _restart_sec > 5.0:
                                    print(
                                        f"  тЖТ {svc_name}: waiting for systemd "
                                ...
                                print(
                                    f"  тЪа {svc_name} drained but didn't relaunch тАФ forcing restart"
                                )
                            ...
                            # тАФ it flashes and dies before the user can
                            # answer.  Skip with clear instructions instead.
                            if _manage_cmd is None:
                                print(
                                    f"  тЪа {svc_name} is a system service and restarting it needs root.\n"
                            ...
                            if restart.returncode == 0:
                                ...
                                if _wait_for_service_active(
                                    ...
                                    # Retry once тАФ transient startup failures
                                    ...
                                    # crash.
                                    print(
                                        f"  тЪа {svc_name} died after restart, retrying..."
                                    )
                                    subprocess.run(
                                        _manage_cmd + ["reset-failed", svc_name],
                                        capture_output=True,
                                    ...
                                    if _wait_for_service_active(
                                        ...
                                        restarted_services.append(svc_name)
                                        print(f"  тЬУ {svc_name} recovered on retry")
                                    else:
                                        _scope_flag = "--user " if scope == "user" else ""
                                        _sudo_hint = "sudo " if scope == "system" else ""
                                        print(
                                            f"  тЬЧ {svc_name} failed to stay running after restart.\n"
                                ...
                                print(
                                    f"  тЪа Failed to restart {svc_name}: {restart.stderr.strip()}"
                                )
                    except FileNotFoundError:
                        pass
                    except subprocess.TimeoutExpired as exc:
                        # Don't swallow this silently тАФ a wedged systemctl
                        # call here used to make the whole restart phase
                        # vanish with no output (June 2026 report).
                        print(
                            f"  тЪа systemctl timed out during the {scope}-scope "
                    ...
                    if plist_path.exists():
                        ...
                        if check.returncode == 0:
                            try:
                                ...
                                print(f"  тЪа Gateway restart failed: {stderr}")
            ...
            for pid, proc in profile_processes.items():
                ...
                # the drain budget, fall back to SIGTERM тАФ the watcher
                ...
                # via back-off retries тАФ but a brief wait here reduces the
            ...
            if restarted_services or killed_pids:
                ...
                for svc in restarted_services:
                    print(f"  тЬУ Restarted {svc}")
                if relaunched_profiles:
                    names = ", ".join(relaunched_profiles)
                    print(f"  тЬУ Restarting manual gateway profile(s): {names}")
                unmapped_count = len(killed_pids) - len(relaunched_profiles)
                if unmapped_count:
                    print(f"  тЖТ Stopped {unmapped_count} manual gateway process(es)")
            ...
            if not restarted_services and not killed_pids:
                # No gateways were running тАФ nothing to do
                pass
            ...
            # watchers wait 120s for the old PID to exit тАФ if it never
            ...
            try:
                ...
                # started AFTER our restart attempt тАФ respecting user
                # intent, we don't kill those.
                _stuck = [pid for pid in _surviving if pid in killed_pids]
                if _stuck:
                    print()
                    print(
                        f"  тЪа {len(_stuck)} gateway process(es) ignored SIGTERM тАФ force-killing"
                    )
                    from gateway.status import terminate_pid as _terminate_pid
                    for pid in _stuck:
                        try:
                            # Routes through taskkill /T /F on Windows,
                            # SIGKILL on POSIX тАФ _signal.SIGKILL doesn't
        ...
        try:
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                print()
                print("тЪа Legacy Hermes gateway unit(s) detected:")
        ...
        # Kill stale dashboard processes тАФ the dashboard has no service
        ...
        if sys.platform == "win32":
            print(f"тЪа Git update failed: {e}")
            print("тЖТ Falling back to ZIP download...")
            print()
            _update_via_zip(args)
        else:
            print(f"тЬЧ Update failed: {e}")
            sys.exit(1)
...
def cmd_profile(args):
    """Profile management тАФ create, delete, list, switch, alias."""
    from hermes_cli.profiles import (
        list_profiles,
        create_profile,
        delete_profile,
    ...
    if action is None:
        # Bare `hermes profile` тАФ show current profile status
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    ...
                    print(f"Alias:          {alias_display} тЖТ hermes -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        for p in profiles:
            ...
            name = p.name
            model = (p.model or "тАФ")[:26]
            gw = "running" if p.gateway_running else "stopped"
            alias = (p.alias_name or p.name) if p.alias_path else "тАФ"
            if p.is_default:
                alias = "тАФ"
        ...
        try:
            ...
            if not (clone_config or clone_all):
                ...
                if result and result.get("skipped_opt_out"):
                    ...
                    print(
                        "тЪа Skills could not be seeded. Run `{} update` to retry.".format(
                            name
                        )
                    )
            ...
            if not no_alias:
                ...
                if collision:
                    print(f"\nтЪа Cannot create alias '{name}' тАФ {collision}")
                    print(
                    ...
                    if wrapper_path:
                        ...
                        if not _is_wrapper_dir_in_path():
                            print(f"\nтЪа {_get_wrapper_dir()} is not in your PATH.")
                            print(
            ...
            if clone or clone_all:
                ...
                print(
                    f"\n  тЪа This profile has no API keys yet. Run '{name} setup' first,"
                )
        ...
        if alias_name:
            ...
            print(f"Alias:   {alias_name} тЖТ hermes -p {name}  ({wrapper})")
        print()
        ...
        if remove:
            if remove_wrapper_script(alias_name):
                print(f"тЬУ Removed alias '{alias_name}'")
            else:
            ...
            if wrapper_path:
                print(f"тЬУ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"тЪа {_get_wrapper_dir()} is not in your PATH.")
        ...
        try:
            ...
            print(f"\nProfile renamed: {args.old_name} тЖТ {args.new_name}")
        ...
        try:
            ...
            print(f"тЬУ Exported '{name}' to {result_path}")
        except (ValueError, FileNotFoundError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        try:
            ...
            print(f"тЬУ Imported profile '{name}' at {profile_dir}")
        ...
        try:
            ...
            print(f"\nтЬУ Installed '{plan.manifest.name}' v{plan.manifest.version}")
        ...
        try:
            ...
            print(f"\nтЬУ Updated '{plan.manifest.name}' тЖТ v{plan.manifest.version}")
            if plan.has_cron:
                print(
...
def _render_distribution_plan(plan) -> None:
    ...
    if plan.existing:
        ...
        # semantics тАФ dist-owned overwritten, config preserved, user data
        ...
        if existing_is_distribution:
            print("  (profile exists тАФ will overwrite distribution-owned files only)")
        else:
            print(
                "  тЪа Profile exists but is NOT a distribution.  Installing here will\n"
            ...
            if not already and plan.target_dir.is_dir():
                ...
                if env_path.is_file():
                    try:
                        ...
                        pass
            status = "тЬУ set" if already else ("needs setting" if er.required else "тАФ")
            line = f"    тАв {er.name} ({tag}, {status})"
            if er.description:
                line += f" тАФ {er.description}"
            print(line)
    if plan.has_cron:
        print(
            "\n  тЪа This distribution ships cron jobs.  They will NOT run "
            "automatically тАФ review and enable manually."
        )
...
def _dashboard_listening(host: str, port: int) -> bool:
    ...
    Any listener counts тАФ even a 401 response proves a dashboard is up.
...
def cmd_dashboard(args):
    ...
    # тФАтФА Unified profile launch routing тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    # launches the dashboard (`worker dashboard` тЖТ HERMES_HOME points into
    # profiles/), default to the machine dashboard:
    #   - already running тЖТ open the browser at ?profile=<name> and exit
    #   - not running     тЖТ re-exec as the machine dashboard (pinned to the
    ...
    if (
        ...
        # HERMES_HOME falls back to $HOME/.hermes = /opt/data/.hermes тАФ an
        ...
        # On Windows, os.execvpe() does not truly replace the process тАФ it
    ...
    if "HERMES_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        if not (_dist_root / "index.html").exists():
            print(f"тЬЧ --skip-build was passed but no web dist found at: {_dist_root}")
            print("  Pre-build first:  npm install --workspace web && npm run build -w web")
            print("  Or drop --skip-build to build automatically.")
            sys.exit(1)
        print(f"тЖТ Skipping web UI build (--skip-build); using dist at {_dist_root}")
    ...
    # providers (image_gen, web, dashboard_auth, тАж).
    try:
        from hermes_cli.plugins import discover_plugins
        discover_plugins()
    except Exception as exc:
        # Discovery failures must not block dashboard startup outright тАФ
        # log and proceed; the gate's fail-closed branch will surface
        # the missing-provider state if it matters.
        print(f"тЪа Plugin discovery failed: {exc}", file=sys.stderr)
    ...
    # tool registry тАФ it never starts MCP discovery (the stdio TUI does that in
    ...
    # available тАФ the desktop app and the dashboard's own Chat tab both rely on
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        # Help-ish invocations тАФ plugin commands not being listed in
...
def _first_positional_argv() -> str | None:
    ...
    Does NOT fully simulate argparse тАФ unknown ``--foo=bar`` / ``--foo
    ...
    while i < len(argv):
        ...
        if tok == "--":
            ...
            return None
        if tok.startswith("-"):
            # ``--flag=value`` carries its value inline тАФ single token.
            if "=" in tok:
                i += 1
                continue
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``hermes`` or only flags тЖТ defaults to ``chat``.
        return False
    if first in _BUILTIN_SUBCOMMANDS:
        return False
    # Unknown token тАФ could be a plugin subcommand, OR a chat prompt
...
def _try_termux_fast_cli_launch() -> bool:
    ...
    # the TUI тАФ explicit --tui/env or display.interface=tui. `--cli` forces this
...
def cmd_memory(args):
    ...
    if sub == "off":
        ...
        save_config(config)
        print("\n  тЬУ Memory provider: built-in only")
        ...
        if not existing:
            print(
                f"\n  Nothing to reset тАФ no memory files found in {display_hermes_home()}/memories/\n"
            )
            return
        ...
        for f, desc in existing:
            ...
            print(f"    тЧЖ {f} ({desc}) тАФ {size:,} bytes")
...
def main():
    ...
    # in ps/top/htop.  Non-fatal тАФ just a nicer UX.
    _set_process_title()
    ...
    # *running* update тАФ that flow writes and clears its own marker, and we
    ...
    # =========================================================================
    # fallback command тАФ manage the fallback provider chain
    # =========================================================================
    ...
    # =========================================================================
    # secrets command тАФ external secret managers (currently: Bitwarden)
    # =========================================================================
    ...
    # Lazy import тАФ only pays for itself when this subcommand is actually used.
    ...
    try:
        ...
        # LSP is optional infrastructure тАФ never let a registration
    ...
    # =========================================================================
    # send command тАФ pipe shell-script output to any configured platform
    # =========================================================================
    ...
    # =========================================================================
    # portal command тАФ Nous Portal status + Tool Gateway routing
    # =========================================================================
    from hermes_cli.portal_cli import add_parser as _add_portal_parser
    _add_portal_parser(subparsers)

    # =========================================================================
    # kanban command тАФ multi-profile collaboration board
    # =========================================================================
    ...
    # =========================================================================
    # hooks command тАФ shell-hook inspection and management
    # =========================================================================
    ...
    # =========================================================================
    # security command тАФ on-demand supply-chain audit
    # =========================================================================
    ...
    checkpoints_parser = subparsers.add_parser(
        ...
        description="Manage the filesystem checkpoint store тАФ the shadow git "
    ...
    # =========================================================================
    # bundles command тАФ skill bundles (alias /<name> for multiple skills)
    # =========================================================================
    ...
    # =========================================================================
    # Plugin CLI commands тАФ dynamically registered by memory/general plugins.
    ...
    # subcommand тАФ ``hermes --help``, ``hermes version``, ``hermes logs``,
    # etc.  This avoids eagerly importing every bundled plugin module
    # (google.cloud.pubsub_v1, aiohttp, grpc, PIL тАж) which costs
    ...
    # =========================================================================
    # curator command тАФ background skill maintenance
    # =========================================================================
    curator_parser = subparsers.add_parser(
        "curator",
        help="Background skill maintenance (curator) тАФ status, run, pause, pin",
        description=(
    ...
    # =========================================================================
    # computer-use command тАФ manage Computer Use (cua-driver) on macOS
    # =========================================================================
    ...
    sessions_browse = sessions_subparsers.add_parser(
        "browse",
        help="Interactive session picker тАФ browse, search, and resume sessions",
    )
    ...
    def cmd_sessions(args):
        ...
        if action == "repair":
            ...
            if reason is None:
                print(f"тЬУ {db_path} opens cleanly тАФ no repair needed.")
                return
            print(f"тЬЧ {db_path} does not open cleanly: {reason}")
            if getattr(args, "check_only", False):
                return
            print("Repairing (a backup copy is made first)тАж")
            ...
            if report.get("repaired"):
                ...
                try:
                    ...
                    print(f"тЬУ Repaired тАФ {n} sessions recovered.")
                except Exception:
                    print("тЬУ Repaired.")
            else:
                print(f"тЬЧ Repair failed: {report.get('error')}")
                if report.get("backup_path"):
        ...
        if action == "list":
            ...
            print("Optimizing session store (FTS merge + VACUUM)тАж")
            try:
    ...
    # e.g. ``hermes -c Pokemon Agent Dev`` тЖТ ``hermes -c 'Pokemon Agent Dev'``
    # тФАтФА Container-aware routing тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    # тФАтФА Defensive subparser routing (bpo-9338 workaround) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    if _has_cmd_token:
        ...
        try:
            ...
            sys.stderr = _saved_stderr
            # Help/version flags (exit code 0) already printed output тАФ
