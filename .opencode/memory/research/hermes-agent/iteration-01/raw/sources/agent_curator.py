"""Curator тАФ background skill maintenance orchestrator.
...
Strict invariants:
  ...
  - Never auto-deletes тАФ only archives. Archive is recoverable.
...
# ---------------------------------------------------------------------------
# .curator_state тАФ persistent scheduler + status
# ---------------------------------------------------------------------------
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``--dry-run``) explicitly тАФ that path bypasses this gate.

    The idle check (min_idle_hours) is applied at the call site where we know
    whether an agent is actively running тАФ here we only enforce the static
    gates.
    """
    if not is_enabled():
        return False
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                "deferred first run тАФ curator seeded, will run after one "
                "interval; use `hermes curator run --dry-run` to preview now"
            )
            save_state(state)
        except Exception as e:  # pragma: no cover тАФ best-effort persistence
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    clock starts NOW rather than at epoch тАФ a long-unused built-in is therefore
    ...
    for row in _u.agent_created_report():
        ...
        if anchor <= archive_cutoff and current != _u.STATE_ARCHIVED:
            ...
            # Skill got used again after being marked stale тАФ reactivate.
...
CURATOR_DRY_RUN_BANNER = (
    ...
    "DRY-RUN тАФ REPORT ONLY. DO NOT MUTATE THE SKILL LIBRARY.\n"
    "тХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХРтХР\n"
    "\n"
    "This is a PREVIEW pass. Follow every instruction below EXCEPT:\n"
    "\n"
    "  тАв DO NOT call skill_manage with action=patch, create, delete, "
    "write_file, or remove_file.\n"
    "  тАв DO NOT call terminal to mv skill directories into .archive/.\n"
    "  тАв DO NOT call terminal to mv, cp, rm, or rewrite any file under "
    "~/.hermes/skills/.\n"
    "  тАв skills_list and skill_view are FINE тАФ read as much as you need.\n"
    "\n"
    "Your output IS the deliverable. Produce the exact same "
    "human-readable summary and structured YAML block you would "
    "produce on a live run тАФ but describe the actions you WOULD take, "
...
CURATOR_REVIEW_PROMPT = (
    ...
    "a FAILURE of the library тАФ not a feature. An agent searching skills "
    ...
    "session-specific detail тАФ not one-session-one-skill micro-entries.\n\n"
    ...
    "tips) and are filtered out of the candidate list below тАФ never resurrect "
    ...
    "overlapping?' тАФ ask 'what is the UMBRELLA CLASS these skills all "
    ...
    "into it.\n"
    "3. Three ways to consolidate тАФ use the right one per cluster:\n"
    "   a. MERGE INTO EXISTING UMBRELLA тАФ one skill in the cluster is "
    ...
    "siblings.\n"
    "   b. CREATE A NEW UMBRELLA SKILL.md тАФ no existing member is broad "
    ...
    "   c. DEMOTE TO REFERENCES/TEMPLATES/SCRIPTS тАФ a sibling has "
    "narrow-but-valuable session-specific content. Move it into the "
    "umbrella's appropriate support directory:\n"
    "      тАв `references/<topic>.md` for session-specific detail OR "
    "condensed knowledge banks (quoted research, API docs excerpts, "
    "domain notes, provider quirks, reproduction recipes)\n"
    "      тАв `templates/<name>.<ext>` for starter files meant to be "
    "copied and modified\n"
    "      тАв `scripts/<name>.<ext>` for statically re-runnable actions "
    "(verification scripts, fixture generators, probes)\n"
    "      Then archive the old sibling. Use `terminal` with `mkdir -p "
    "~/.hermes/skills/<umbrella>/references/ && mv ... <umbrella>/"
    "references/<topic>.md` (or templates/ / scripts/).\n\n"
    "Package integrity тАФ not optional:\n"
    ...
    "   тАв keep it as a standalone skill, OR\n"
    "   тАв fully merge it by re-homing every needed support file into the "
    "umbrella's canonical `references/`, `templates/`, `scripts/`, or "
    "`assets/` directories AND rewrite the destination instructions to "
    "the new paths, OR\n"
    "   тАв archive the entire original skill package unchanged.\n"
    ...
    "merges.\n\n"
    "Your toolset:\n"
    "  - skills_list, skill_view        тАФ read the current landscape\n"
    "  - skill_manage action=patch      тАФ add sections to the umbrella\n"
    "  - skill_manage action=create     тАФ create a new umbrella SKILL.md\n"
    "  - skill_manage action=write_file тАФ add a references/, templates/, "
    "or scripts/ file under an existing skill (the skill must already "
    "exist)\n"
    "  - skill_manage action=delete     тАФ archive a skill. MUST pass "
    "`absorbed_into=<umbrella>` when you've merged its content into another "
    "skill, or `absorbed_into=\"\"` when you're truly pruning with no "
    "forwarding target. This drives cron-job skill-reference migration тАФ "
    "guessing from your YAML summary after the fact is fragile.\n"
    "  - terminal                       тАФ mv a sibling into the archive "
    "OR move its content into a support subfile\n\n"
    "'keep' is a legitimate decision ONLY when the skill is already a "
    "class-level umbrella and none of the proposed merges would improve "
    "discoverability. 'This is narrow but distinct from its siblings' "
    "is NOT a reason to keep тАФ it's a reason to move it under an "
    ...
    "stopped too early тАФ go back and look at the clusters you left "
    "alone.\n\n"
    ...
    "    reason: <one short sentence тАФ why merged, not just 'similar'>\n"
    "prunings:\n"
    "  - name: <skill-name>\n"
    "    reason: <one short sentence тАФ why archived with no merge target>\n"
    "```\n\n"
    ...
    "no absorption тАФ truly stale, irrelevant, or obsolete тАФ X goes under "
...
# ---------------------------------------------------------------------------
# Per-run reports тАФ {YYYYMMDD-HHMMSS}/run.json + REPORT.md under logs/curator/
# ---------------------------------------------------------------------------
...
def _reports_root() -> Path:
    ...
    the v22тЖТv23 migration backfills it for existing profiles, but we
...
def _classify_removed_skills(
    ...
    into another skill (an umbrella) during this run тАФ the content still
    ...
    for tc in tool_calls or []:
        ...
        if isinstance(raw, dict):
            ...
            try:
                ...
                # Truncated or malformed тАФ fall back to substring match on
    ...
    for name in removed:
        ...
        for args in parsed_calls:
            ...
            if target == name:
                continue
            # The target must be a surviving or newly-created skill тАФ
            ...
            #   file_path тАФ needle must be a complete path component
            #     (filename stem or directory name), so "api" does NOT
            #     falsely match "references/api-design.md".
            #   content fields тАФ word-boundary regex so "test" does NOT
...
def _parse_structured_summary(
    ...
    - Missing block тЖТ returns empty lists (we'll fall back to heuristic).
    - Malformed YAML тЖТ returns empty lists and we rely on heuristic.
    - Partial block (e.g. only consolidations) тЖТ returns what we could parse.
    ...
    # Prefer PyYAML when available тАФ every hermes install already has it
...
def _extract_absorbed_into_declarations(
    ...
    signal for classification тАФ the model's own declaration at the moment of
    ...
    ``absorbed_into``, are not in the returned dict тАФ caller falls back to
...
def _reconcile_classification(
    ...
    - **Model-declared `absorbed_into` at delete time is authoritative.** Any
      ...
      ``into != ""`` and target exists тЖТ consolidated. ``into == ""`` тЖТ
      pruned. ``into != ""`` but target doesn't exist тЖТ hallucination; fall
    ...
    - Model-declared pruning is accepted unless the heuristic has
      tool-call evidence that contradicts it (rare тАФ the heuristic would
    ...
    for name in removed:
        ...
        if dec is not None:
            ...
            # in practice тАФ but if it slips through (e.g. the umbrella was
            # deleted LATER in the same run), fall through to the usual
            # signals rather than trusting a broken reference.

        # Model says consolidated тАФ trust it if the destination is real.
        ...
        # Model says consolidated but the umbrella doesn't exist тАФ
...
def _build_rename_summary(
    *,
    ...
    nothing was archived this run тАФ most ticks are no-op and shouldn't add
    extra log noise.

    Format::

        archived 4 skill(s):
          тАв pdf-extraction тЖТ document-tools
          тАв docx-extraction тЖТ document-tools
          тАв flaky-thing тАФ pruned (stale)
          тАв old-utility тЖТ spreadsheet-ops
    ...
    for entry in consolidated:
        ...
        name = entry.get("name", "?")
        into = entry.get("into", "?")
        lines.append(f"  тАв {name} тЖТ {into}")
        shown += 1
    for entry in pruned:
        if shown >= SHOW:
            break
        name = entry.get("name", "?") if isinstance(entry, dict) else str(entry)
        lines.append(f"  тАв {name} тАФ pruned (stale)")
        shown += 1
    if total > SHOW:
        lines.append(f"  тАж and {total - SHOW} more")
    lines.append("full report: hermes curator status")
    # Pin hint тАФ only surface it when there's actually a destination skill
...
def _write_run_report(
    *,
    ...
    couldn't happen (caller logs and continues тАФ reporting is best-effort).
    """
    root = _reports_root()
    try:
        root.mkdir(parents=True, exist_ok=True)
    ...
    # the substring heuristic тАФ the model is telling us directly, at the
    ...
    # it at run time тАФ the scheduler skips it and the job runs without
    ...
    # run.json тАФ machine-readable, full fidelity
    try:
    ...
    # cron_rewrites.json тАФ only when at least one job was touched, to
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    lines.append(f"- state transitions (active тЖФ stale тЖФ archived): "
                 f"**{counts.get('state_transitions', 0)}**")
    lines.append("")

    # Consolidated list тАФ content absorbed into an umbrella. The directory
    ...
    if consolidated:
        ...
        lines.append(
            "_These skills were **absorbed into another skill** during this run тАФ "
        ...
        if len(consolidated) > SHOW:
            lines.append(f"- тАж and {len(consolidated) - SHOW} more (see `run.json`)")
        lines.append("")

    # Pruned list тАФ archived without consolidation. These are the
    # "stale skill pruned" cases the UI should mark clearly.
    pruned = p.get("pruned") or []
    if pruned:
        lines.append(f"### Pruned тАФ archived for staleness ({len(pruned)})\n")
        lines.append(
        ...
        if len(pruned) > SHOW:
            lines.append(f"- тАж and {len(pruned) - SHOW} more (see `run.json`)")
        lines.append("")
    ...
    if trans:
        ...
        for t in trans:
            lines.append(f"- `{t.get('name')}`: {t.get('from')} тЖТ {t.get('to')}")
        lines.append("")

    # Cron job rewrites тАФ show which scheduled jobs had their skill
    ...
    if cron_rewrites_list:
        ...
        for entry in cron_rewrites_list[:SHOW]:
            ...
            lines.append(
                f"- `{job_name}`: `{', '.join(before)}` тЖТ `{', '.join(after) or '(none)'}`"
            )
            for old, new in mapped.items():
                lines.append(f"    - `{old}` тЖТ `{new}` (consolidated)")
            for name in dropped:
                lines.append(f"    - `{name}` dropped (pruned)")
        if len(cron_rewrites_list) > SHOW:
            lines.append(
                f"- тАж and {len(cron_rewrites_list) - SHOW} more "
...
# ---------------------------------------------------------------------------
# Orchestrator тАФ spawn a forked AIAgent for the LLM review pass
# ---------------------------------------------------------------------------
...
def run_curator_review(
    ...
    and the LLM review pass is instructed to produce a report only тАФ no
    ...
    if dry_run:
        ...
        # Pre-mutation snapshot тАФ best-effort, never blocks the run. A
    ...
    # last_run_at or run_count тАФ a preview shouldn't push the next scheduled
    ...
    def _llm_pass():
        ...
        try:
            ...
            if "No agent-created skills" in candidate_list:
                ...
                # bundled" rule for them тАФ but only archiving is permitted, and
        ...
        # Append the rename map (`old-name тЖТ umbrella`) to the user-visible
...
def _resolve_review_runtime(cfg: Dict[str, Any]) -> _ReviewRuntimeBinding:
    ...
    if _legacy_provider and _legacy_model:
        logger.info(
            ...
            "config тАФ please migrate to auxiliary.curator.{provider,model}"
        )
        return _ReviewRuntimeBinding(
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    Curator is a regular auxiliary task slot тАФ ``auxiliary.curator.{provider,model}``
    тАФ so it participates in the canonical aux-model plumbing (``hermes model`` тЖТ
    ...
    means "use the main chat model" тАФ same default as every other aux task.
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `hermes model` тЖТ auxiliary
    ...
    try:
        review_agent = AIAgent(
            ...
            # high iteration ceiling тАФ the pass typically takes 50-100
        ...
        # Disable recursive nudges тАФ the curator must never spawn its own review.
        ...
        result_meta["summary"] = (final[:240] + "тАж") if len(final) > 240 else (final or "no change")
