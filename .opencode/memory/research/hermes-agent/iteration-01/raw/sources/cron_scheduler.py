...
def _resolve_cron_disabled_toolsets(cfg: dict) -> list[str]:
    ...
    Three protected toolsets are always disabled in cron context:
      - ``cronjob`` тАФ would let a cron-spawned agent schedule more cron jobs
      - ``messaging`` тАФ interactive, needs a live gateway session
      - ``clarify`` тАФ interactive, blocks waiting for user input
    ...
    ordinary agent runs (#25752 тАФ LLM-supplied enabled_toolsets was widening
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    1. Per-job ``enabled_toolsets`` (set via ``cronjob`` tool on create/update).
       Keeps the agent's job-scoped toolset override intact тАФ #6130.
    2. Per-platform ``hermes tools`` config for the ``cron`` platform.
       Mirrors gateway behavior (``_get_platform_tools(cfg, platform_key)``)
       so users can gate cron toolsets globally without recreating every job.
    3. ``None`` on any lookup failure тАФ AIAgent loads the full default set
       (legacy behavior before this change, preserved as the safety net).

    _DEFAULT_OFF_TOOLSETS ({moa, homeassistant, rl}) are removed by
    ``_get_platform_tools`` for unconfigured platforms, so fresh installs
    get cron WITHOUT ``moa`` by default (issue reported by Norbert тАФ
    surprise $4.63 run).
    """
...
# Valid delivery platforms тАФ used to validate user-supplied platform names
...
# primary env var тЖТ the previous name.  _get_home_target_chat_id falls
...
# Sequential (env-mutating) cron jobs тАФ workdir jobs that touch
# process-global runtime state тАФ must run one at a time, but must NOT block the
...
def _resolve_origin(job: dict) -> Optional[dict]:
    ...
    ``'str' object has no attribute 'get'`` тАФ ``mark_job_run`` recorded the
...
def _get_home_target_thread_id(platform_name: str) -> Optional[str]:
    ...
    the system-only lobby where the user cannot reply тАФ the gateway returns
...
def cron_delivery_targets() -> list[dict]:
    ...
    room/channel cron posts to) is set тАФ a platform can be configured for
    ...
    prepend the implicit ``local`` option themselves тАФ it needs no config.
    """
    targets: list[dict] = []
    try:
...
def _resolve_single_delivery_target(job: dict, deliver_value: str) -> Optional[dict]:
    ...
    if deliver_value == "origin":
        ...
        # Origin missing (e.g. job created via API/script) тАФ try each
...
def _normalize_deliver_value(deliver) -> str:
    ...
    Historically some callers тАФ MCP clients passing an array, direct edits of
    ``jobs.json``, or stale code paths тАФ have stored a list/tuple like
...
# Routing intent tokens тАФ resolved at fire time, not create time, so a
...
# Media extension sets тАФ audio routing is centralized in gateway.platforms.base
...
def _send_media_via_adapter(
    ...
    send_video, send_document) based on file extension тАФ mirroring the routing logic
...
def _deliver_result(job: dict, content: str, adapters=None, loop=None) -> Optional[str]:
    ...
    use the live adapter first тАФ this supports E2EE rooms (e.g. Matrix) where
    ...
    if not targets:
        if job.get("deliver", "local") != "local":
            ...
            return msg
        return None  # local-only jobs don't deliver тАФ not a failure
    ...
    for target in targets:
        ...
        # Prefer the live adapter when the gateway is running тАФ this supports E2EE
        ...
        if runtime_adapter is not None and loop is not None and getattr(loop, "is_running", lambda: False)():
            ...
            try:
                # Send cleaned text (MEDIA tags stripped) тАФ not the raw content
        ...
        if not delivered:
            ...
            try:
                ...
                # when it raises, the original coro was never started тАФ close it to
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    * ``.sh`` / ``.bash`` тАФ run with ``/bin/bash``
    * anything else тАФ run with the current Python interpreter
    ...
    Returns:
        (success, output) тАФ on failure *output* contains the error message so the
    ...
    # escape тАФ scripts MUST reside within HERMES_HOME/scripts/.
    try:
    ...
    if suffix in {".sh", ".bash"}:
        ...
        # shutil.which returns None тАФ fall back to a clear error rather
...
def _parse_wake_gate(script_output: str) -> bool:
    ...
    JSON like ``{"wakeAgent": false}``, the agent is skipped entirely тАФ no
...
def _build_job_prompt(job: dict, prerun_script: Optional[tuple] = None) -> str:
    ...
    # user-prompt pattern set тАФ see _scan_assembled_cron_prompt.
    ...
    if script_path:
        ...
        if success:
            if script_output:
                ...
                # Script produced no output тАФ nothing to report, skip AI call.
                return None
        else:
            prompt = (
                "## Script Error\n"
    ...
    if context_from:
        ...
        for source_job_id in context_from:
            # Guard against path traversal тАФ valid job IDs are 12-char hex strings
            ...
            try:
                ...
                if not job_output_dir.exists():
                    continue  # silent skip тАФ no output yet
                output_files = sorted(
                ...
                if latest_output:
                    ...
                    continue  # silent skip тАФ empty output
            except (OSError, PermissionError) as e:
                logger.warning("context_from: failed to read output for job %r: %s", source_job_id, e)
                # silent skip тАФ do not pollute the prompt with error messages
    ...
    cron_hint = (
        ...
        "to the user тАФ do NOT use send_message or try to deliver "
        "the output yourself. Just produce your report/output as your "
        "final response and the system handles the rest. "
        "SILENT: If there is genuinely nothing new to report, respond "
        "with exactly \"[SILENT]\" (nothing else) to suppress delivery. "
        "Never combine [SILENT] with content тАФ either report your "
    ...
    for skill_name in skill_names:
        ...
        if not loaded.get("success"):
            ...
            logger.warning("Cron job '%s': skill not found, skipping тАФ %s", job.get("name", job.get("id")), error)
            skipped.append(skill_name)
            continue
    ...
    if skipped:
        notice = (
            ...
            f"'тЪая╕П Skill(s) not found and skipped: {', '.join(skipped)}']"
        )
...
def _scan_assembled_cron_prompt(
    ...
    *,
    ...
    - When the assembled prompt includes runtime-loaded content тАФ skill
      markdown (``has_skills=True``) or DATA injected from a job script's
      stdout / an upstream job's output (``has_injected_data=True``) тАФ the
      ...
      code, the same trust class тАФ and data feeds (e.g. a triage bot
    ...
    if scan_error:
        ...
        logger.warning(
            "Cron job '%s': assembled prompt blocked by injection scanner тАФ %s",
            job_label,
            scan_error,
        )
...
def run_job(job: dict) -> tuple[bool, str, str, Optional[str]]:
    ...
    # ---------------------------------------------------------------
    # no_agent short-circuit тАФ the script IS the job, no LLM involvement.
    # ---------------------------------------------------------------
    ...
    #
    # Semantics:
    #   - script stdout (trimmed) тЖТ delivered verbatim as the final message
    #   - empty stdout            тЖТ silent run (no delivery, success=True)
    #   - non-zero exit / timeout тЖТ delivered as an error alert, success=False
    #   - wakeAgent=false gate    тЖТ treated like empty stdout (silent), since
    ...
    if job.get("no_agent"):
        ...
        # Apply workdir if configured тАФ lets scripts use predictable relative
        ...
        if not ok:
            ...
            # error so the user knows the watchdog itself broke тАФ silent
            # failure for an alerting job is the worst-case outcome.
            alert = (
                f"тЪа Cron watchdog '{job_name}' script failed\n\n"
                f"{output}\n\n"
                f"Time: {now_iso}"
            )
            doc = (
        ...
        # Honour the wakeAgent gate as a silent signal тАФ `wakeAgent: false`
        # means "nothing to report this tick", same as empty stdout.
        if not _parse_wake_gate(output):
            logger.info(
                "Job '%s' (no_agent): wakeAgent=false gate тАФ silent run", job_id
            )
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
        ...
        if not output.strip():
            logger.info("Job '%s' (no_agent): empty stdout тАФ silent run", job_id)
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {now_iso}\n"
    ...
    # ---------------------------------------------------------------
    # Default (LLM) path тАФ import and construct the agent machinery now
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                ...
                "Script gate returned `wakeAgent=false` тАФ agent skipped.\n"
            )
            return True, silent_doc, SILENT_MARKER, None
    ...
    try:
        ...
        logger.warning(
            "Job '%s' (ID: %s): blocked by prompt-injection scanner тАФ %s",
    ...
    # scheduler process тАФ every job this process runs is a cron job.
    ...
    # jobs we leave TERMINAL_CWD untouched тАФ preserves the original behaviour
    ...
    if _job_workdir and not Path(_job_workdir).is_dir():
        ...
        logger.warning(
            "Job '%s': configured workdir %r no longer exists тАФ running without it",
            job_id, _job_workdir,
        )
        _job_workdir = None
    ...
    try:
        ...
        try:
            ...
            # Primary provider auth failed тАФ try fallback chain before giving up.
        ...
        # this, cron jobs never saw any MCP tools тАФ only the gateway / CLI
        ...
        try:
            if _cron_inactivity_limit is None:
                # Unlimited тАФ just wait for the result.
        ...
        if _inactivity_timeout:
            ...
            raise TimeoutError(
                ...
                f"тАФ last activity: {_last_desc}"
            )
...
"""
        ...
        for _var_name in _cron_delivery_vars:
            _VAR_MAP[_var_name].set("")
        if _session_db:
            # Title the cron session from the job (name тЖТ short prompt тЖТ id) so
            # sidebars/history show a meaningful label instead of the injected
            # "[IMPORTANT: тАж]" hint that is the session's first message. Set here
        ...
        # httpx clients cached under that loop are now unusable тАФ reap them
...
def tick(verbose: bool = True, adapters=None, loop=None, sync: bool = True) -> int:
    ...
    Args:
        ...
        adapters: Optional dict mapping Platform тЖТ live adapter (from gateway)
    ...
    try:
        ...
        logger.debug("Tick skipped тАФ another instance holds the lock")
    ...
    try:
        ...
        def _process_job(job: dict) -> bool:
            ...
            try:
                ...
                deliver_content = final_response if success else f"тЪая╕П Cron job '{job.get('name', job['id'])}' failed:\n{error}"
                # Treat whitespace-only final responses the same as empty
                # responses: do not deliver a blank message, and let the
                # empty-response guard below mark the run as a soft failure.
                should_deliver = bool(deliver_content.strip())
                if should_deliver and success and SILENT_MARKER in deliver_content.strip().upper():
                    logger.info("Job '%s': agent returned %s тАФ skipping delivery", job["id"], SILENT_MARKER)
                ...
                # is not "ok" тАФ the agent ran but produced nothing useful.
                # (issue #8585)
        ...
        # os.environ["TERMINAL_CWD"] inside run_job, which is process-global тАФ
        ...
        def _submit_with_guard(job: dict, pool: concurrent.futures.ThreadPoolExecutor):
            ...
            with _running_lock:
                if job_id in _running_job_ids:
                    logger.info("Job '%s' already running тАФ skipping", job.get("name", job_id))
                    return None
        ...
        # WITHOUT blocking the ticker thread тАФ a long workdir job no
        ...
        # Parallel pass тАФ persistent pool, non-blocking dispatch.
        ...
        # (including live user chats) are never touched тАФ only PIDs explicitly
        ...
        if _all_futures:
            ...
            # Nothing dispatched (all skipped / no due jobs) тАФ sweep inline.
            _sweep_mcp_orphans()
