...
Directory layout for user skills:
    ...
    тФФтФАтФА category-name/
        тФФтФАтФА another-skill/
            тФФтФАтФА SKILL.md
"""
...
# Import security scanner тАФ external hub installs always get scanned;
...
def _security_scan_skill(skill_dir: Path) -> Optional[str]:
    ...
    try:
        ...
        if allowed is None:
            # "ask" verdict тАФ for agent-created skills this means dangerous
...
def _containing_skills_root(skill_path: Path) -> Path:
    ...
    match is found (defensive тАФ callers should have located the skill via
    ``_find_skill`` first).
    """
...
def _pinned_guard(name: str) -> Optional[str]:
    ...
    Pin protects a skill from **deletion** тАФ both the curator's auto-archive
...
def _find_skill_in_other_profiles(name: str) -> List[Tuple[str, Path]]:
    ...
    when profile discovery fails тАФ fail-quiet, the caller falls back to
    ...
    # Default profile (~/.hermes/skills) тАФ only consider when active is non-default.
...
def _validate_file_path(file_path: str) -> Optional[str]:
    ...
    # under an allowed subdirectory. Accept its two natural spellings тАФ
    # 'SKILL.md' and '<skill-name>/SKILL.md' тАФ so callers can target the main
...
def _create_skill(name: str, content: str, category: str = None) -> Dict[str, Any]:
    ...
    # Security scan тАФ roll back on block
...
def _patch_skill(
    ...
    # escape sequences, and block-anchor matching тАФ saving the agent
    ...
    result["_change"] = {
        "old": old_string[:200] + ("тАж" if len(old_string) > 200 else ""),
        "new": new_string[:200] + ("тАж" if len(new_string) > 200 else ""),
    }
    return result
...
def _delete_skill(name: str, absorbed_into: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``absorbed_into`` declares intent:
      - ``None`` / missing  тЖТ caller didn't declare (legacy / non-curator path);
      ...
      - ``""`` (empty)      тЖТ explicit "truly pruned, no forwarding target".
      - ``"<skill-name>"``  тЖТ content was absorbed into that umbrella; the
...
def _apply_skill_write_gate(action, name, **payload_kwargs):
    ...
    # stage тАФ record the full skill_manage kwargs so approval can replay it.
...
def skill_manage(
    ...
    if result.get("success"):
        ...
        # review fork creates it тАФ foreground `skill_manage(create)` calls are
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        "memory тАФ reusable approaches for recurring task types. "
        f"New skills go to {display_hermes_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        "Actions: create (full SKILL.md + optional category), "
        "patch (old_string/new_string тАФ preferred for fixes), "
        "edit (full SKILL.md rewrite тАФ major overhauls only), "
        ...
        "exist тАФ create/patch the umbrella first, then delete.\n\n"
        ...
        "Pinned skills are protected from deletion only тАФ skill_manage(action='delete') "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "absorbed_into": {
                "type": "string",
                "description": (
                    "For 'delete' only тАФ declares intent so the curator can "
