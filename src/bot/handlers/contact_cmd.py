...
async def cmd_contact(
    ...
    # 1. Resolve contact via fuzzy matching
    ...
    if contact.username:
        header += f" · @{sanitize_html(contact.username)}"
    if contact.phone:
        header += f" · 📞 {sanitize_html(contact.phone)}"
    if contact.is_bot:
        header += " · 🤖 Бот"
    lines.append(header)

    if contact.folder_names:
        folders = [f.strip() for f in contact.folder_names.split(",") if f.strip()]
        if folders:
            lines.append(f"📂 Папки: {sanitize_html(', '.join(folders))}")

    if contact.archetype:
        lines.append(f"🎭 Архетип: {sanitize_html(contact.archetype)}")
    ...
    if conv_state:
        ...
        if conv_state.last_incoming_at:
            state_parts.append(f"· вх. {_format_time_ago(conv_state.last_incoming_at)}")
