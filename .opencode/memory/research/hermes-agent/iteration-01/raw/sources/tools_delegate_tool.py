...
#   false (default) тЖТ _subagent_auto_deny (safe; matches leaf tool blocklist)
#   true            тЖТ _subagent_auto_approve (opt-in YOLO for cron/batch)
...
# toolset to request explicitly тАФ the correct mechanism for nested
...
_MIN_SPAWN_DEPTH = 1
# No upper ceiling on spawn depth тАФ like max_concurrent_children, depth has a
...
def list_active_subagents() -> List[Dict[str, Any]]:
    ...
    tool_count, status}.  Safe to call from any thread тАФ returns a copy.
    """
    with _active_subagents_lock:
        return [
...
def _extract_output_tail(
    ...
    *,
    ...
    Powers the overlay's "Output" section тАФ the cc-swarm-parity feature.
...
def _get_child_timeout() -> Optional[float]:
    ...
    actually doing тАФ API errors, tool errors, iteration budget тАФ not from a
...
def _get_max_spawn_depth() -> int:
    ...
    Like max_concurrent_children, there is no upper ceiling тАФ but each
...
#   - idle between turns (no current_tool) тАФ probably stuck on a slow API call
#   - inside a tool (current_tool set) тАФ probably running a legitimately long
...
_HEARTBEAT_STALE_CYCLES_IDLE = 15  # 15 * 30s = 450s idle between turns тЖТ stale
_HEARTBEAT_STALE_CYCLES_IN_TOOL = 40  # 40 * 30s = 1200s stuck on same tool тЖТ stale
...
class DelegateEvent(str, enum.Enum):
    ...
    (``tool.started``, ``_thinking``, тАж) to these enum values via
...
# Legacy event strings тЖТ DelegateEvent mapping.
    ...
    *,
    ...
    if role == "orchestrator":
        child_note = (
            ...
            "because they would be at the depth floor тАФ you cannot pass "
        ...
        parts.append(
            ...
            "- Single-step mechanical work тАФ do it directly.\n"
...
def _build_child_progress_callback(
    ...
    *,
    ...
    pause) back by ``subagent_id``.  All are optional for backward compat тАФ
    ...
    if not spinner and not parent_cb:
        return None  # No display тЖТ no callback тЖТ zero behavior change
    ...
    def _identity_kwargs() -> Dict[str, Any]:
        ...
        # The child's own session id тАФ filled into the shared ref once the
    ...
    def _callback(
        ...
        # Lifecycle events emitted by the orchestrator itself тАФ handled
        ...
        if event_type == "subagent.start":
            if spinner and goal_label:
                ...
                try:
                    spinner.print_above(f" {prefix}тФЬтФА ЁЯФА {short}")
                except Exception as e:
        ...
        if event_type == "subagent.text":
            ...
            # No spinner echo тАФ the CLI shows the child via the tree, and the
        ...
        if isinstance(event_type, DelegateEvent):
            ...
            if event is None:
                try:
                    ...
                    return  # Unknown event тАФ ignore
        ...
        if event == DelegateEvent.TASK_THINKING:
            ...
            if spinner:
                ...
                try:
                    spinner.print_above(f' {prefix}тФЬтФА ЁЯТн "{short}"')
                except Exception as e:
                    logger.debug("Spinner print_above failed: %s", e)
        ...
        if event == DelegateEvent.TASK_PROGRESS:
            ...
            if spinner and summary_text:
                try:
                    spinner.print_above(f" {prefix}тФЬтФА ЁЯФА {summary_text}")
                except Exception as e:
                    logger.debug("Spinner print_above failed: %s", e)
            if parent_cb:
                try:
        ...
        # TASK_TOOL_STARTED тАФ display and batch for parent relay
        _tool_count[0] += 1
        if subagent_id is not None:
            with _active_subagents_lock:
        ...
        if spinner:
            ...
            line = f" {prefix}тФЬтФА {emoji} {tool_name}"
            if short:
                line += f'  "{short}"'
            try:
        ...
        if parent_cb:
            ...
            if len(_batch) >= _BATCH_SIZE:
                summary = ", ".join(_batch)
                _relay("subagent.progress", preview=f"ЁЯФА {prefix}{summary}")
                _batch.clear()
...
def _build_child_agent(
    ...
    # ACP transport overrides тАФ lets a non-ACP parent spawn ACP child agents
    ...
    # тФАтФА Role resolution тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    # Honor the caller's role only when BOTH the kill switch and the
    # child's depth allow it.  This is the single point where role
    # degrades to 'leaf' тАФ keeps the rule predictable.  Callers pass
    ...
    # тФАтФА Subagent identity (stable across events, 0-indexed for TUI) тФАтФАтФАтФАтФА
    ...
    if parent_enabled is not None:
        ...
        # enabled_toolsets is None (all tools) тАФ derive from loaded tool names
        import model_tools
    ...
    if toolsets:
        # Intersect with parent тАФ subagent must not gain tools the parent lacks.
    ...
    # orchestrator capability is granted by role, not inherited тАФ see the
    ...
    # different provider than the parent тАФ each provider has its own API surface
    ...
    # the subagent must use direct API calls тАФ not the parent's ACP transport.
    ...
    if override_provider:
        ...
        # provider is overridden тАФ it's a no-op on any other model.
    ...
    # (including the spawn_requested below тАФ first emit happens after this).
    ...
    # тЖТ NULL). Mirrors /branch's ``_branched_from`` pattern тАФ see
    ...
    # Announce the spawn immediately тАФ the child may sit in a queue
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    try:
        ...
        _w(f"# Subagent timeout diagnostic тАФ issue #14726")
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    def _heartbeat_loop():
        while not _heartbeat_stop.wait(_HEARTBEAT_INTERVAL):
            ...
            try:
                ...
                # api_call_count тАФ we don't want that to look stale at the
                # idle threshold.
                ...
                if _stale_count[0] >= stale_limit:
                    logger.warning(
                        ...
                        "heartbeat cycles, tool=%s) тАФ stopping heartbeat",
                        task_index,
                        _stale_count[0],
                        child_tool or "<none>",
                    )
    ...
    try:
        ...
        # Run child with an optional hard timeout (off by default тАФ
        ...
        # Python stack (see #14726 тАФ 0-API-call hangs are opaque without it).
        _worker_thread_holder: Dict[str, Optional[threading.Thread]] = {"t": None}

        def _relay_child_text(delta: str) -> None:
            # Forward the child's streamed reply text up the progress relay so
            # gateway watch windows mirror it live (subagent.text тЖТ message.delta).
        ...
        try:
            ...
            # See #14726 тАФ without this, 0-API-call hangs are black boxes.
            ...
            if is_timeout and child_api_calls == 0:
                ...
                if diagnostic_path:
                    logger.warning(
                        "Subagent %d 0-API-call timeout тАФ diagnostic written to %s",
                        task_index,
                        diagnostic_path,
                    )
            ...
            if is_timeout:
                if child_api_calls == 0:
                    _err = (
                        ...
                        f"making any API call тАФ the child never reached its "
                    ...
                    _err = (
                        ...
                        f"{child_api_calls} API call(s) completed тАФ likely "
            ...
            # Shut down executor without waiting тАФ if the child thread
        ...
        entry: Dict[str, Any] = {
            ...
            # Kilo-Org/kilocode#9448 тАФ previously the footer only reflected the
                ...
                if sibling_writes:
                    ...
                    if mod_paths:
                        reminder = (
                            ...
                            "previously read тАФ re-read before editing: "
                            + ", ".join(mod_paths[:8])
                            + (
        ...
        # optional тАФ missing data degrades gracefully on the client.
...
def delegate_task(
    ...
    # Operator-controlled kill switch тАФ lets the TUI freeze new fan-out
    ...
    # model тАФ reject early with a clear message rather than silently running
    ...
    # Depth limit тАФ configurable via delegation.max_spawn_depth,
    ...
    # Model-supplied max_iterations is ignored тАФ the config value is authoritative
    ...
    if n_tasks == 1:
        ...
        # supported in v1 тАФ the rejection is handled before we get here.
        if background:
            from tools.async_delegation import dispatch_async_delegation
            from tools.approval import get_current_session_key

            # Capture the gateway routing key on THIS (parent) thread тАФ the
            ...
            # (/new) тАФ otherwise the detached subagent dies with whatever
                ...
                return json.dumps(
                    {
                        ...
                        "note": (
                            ...
                            "message when it finishes. Do not wait or poll тАФ "
                            "just continue."
                        ),
                    },
                    ensure_ascii=False,
                )
            # Rejected (at capacity or schedule failure) тАФ surface as a tool
        ...
        with ThreadPoolExecutor(max_workers=max_children) as executor:
            ...
            # until ALL futures finish тАФ if a child agent gets stuck,
            ...
            while pending:
                if getattr(parent_agent, "_interrupt_requested", False) is True:
                    # Parent interrupted тАФ collect whatever finished and
                    ...
                    for f in pending:
                        ...
                        if f.done():
                            ...
                            entry = {
                                ...
                                "status": "interrupted",
                                "summary": None,
                                "error": "Parent agent interrupted тАФ child did not finish in time",
                                "api_calls": 0,
                                "duration_seconds": 0,
                                "_child_role": getattr(
                                    _child_by_index.get(idx), "_delegate_role", None
                ...
                for future in done:
                    ...
                    icon = "тЬУ" if status == "completed" else "тЬЧ"
                    ...
                    if spinner_ref and remaining > 0:
                        try:
                            spinner_ref.update_text(
                                f"ЁЯФА {remaining} task{'s' if remaining != 1 else ''} remaining"
                            )
                        except Exception as e:
    ...
    # additive тАФ each delegate_task call contributes its own children тАФ so
    # nested orchestratorтЖТworker trees roll up naturally: each layer's own
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    provider system тАФ the same path used by CLI/gateway startup. This lets
    ...
    if configured_base_url:
        ...
        api_key = configured_api_key  # None тЖТ inherited from parent in _build_child_agent
        ...
        # /anthropic suffix тАФ Azure AI Foundry, MiniMax, Zhipu GLM, LiteLLM
        # proxies тАФ pick the right transport automatically. Without this,
    ...
    if not configured_provider:
        # No provider override тАФ child inherits everything from parent
        return {
            "model": configured_model,
            "provider": None,
            "base_url": None,
    ...
    # Provider is configured тАФ resolve full credentials
    try:
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "status) and verify it yourself тАФ fetch the URL, stat the file, read "
        "back the content тАФ before telling the user the operation succeeded.\n"
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "tasks": {
                ...
                # No maxItems тАФ the runtime limit is configurable via
            ...
            "background": {
                ...
                "description": (
                    ...
                    "moving on. Single-task only тАФ cannot be combined with the "
                    ...
                    "dispatching тАФ just continue; the result will come to you."
                ),
            },
            "acp_command": {
                "type": "string",
