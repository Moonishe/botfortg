...
# NOTE: `from openai import OpenAI` is deliberately NOT at module top тАФ the
...
# run_agent.py directly as a CLI) тАФ it is NOT needed for library usage.
...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    try:
        ...
        # cwd was unlinked out from under us тАФ nothing meaningful to record.
        return None


# OpenAI lazy proxy + safe stdio + proxy URL helpers тАФ see agent/process_bootstrap.py.
...
# siblings, or the `_ra().<X>` indirection in agent/system_prompt.py тАФ none
...
# =========================================================================
# Large tool result handler тАФ save oversized output to temp file
# =========================================================================


# =========================================================================
# Qwen Portal headers тАФ mimics QwenCode CLI for portal.qwen.ai compatibility.
...
def _pool_may_recover_from_rate_limit(
    ...
    cooldown to expire means retrying against the same exhausted quota тАФ the
    ...
    throttles тАФ even a multi-entry pool shares the same quota window, so
    ...
    if not pool.has_available():
        return False
    # CloudCode / Gemini CLI quotas are account-wide тАФ all pool entries share
...
class AIAgent:
    ...
    def __init__(
        ...
        """Forwarder тАФ see ``agent.agent_init.init_agent``."""
    ...
    def _ensure_db_session(self) -> None:
        ...
        try:
            ...
            # Transient failure (e.g. SQLite lock). Keep _session_db alive тАФ
    ...
    def reset_session_state(
        ...
        # Turn counter (added after reset_session_state was first written тАФ #2635)
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        """Forwarder тАФ see ``agent.agent_runtime_helpers.switch_model``."""
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        """Verbose print тАФ suppressed when actively streaming tokens.
    ...
    def _emit_status(self, message: str) -> None:
        ...
        This helper never raises тАФ exceptions are swallowed so it cannot
    ...
    def _emit_notice(self, notice) -> None:
        ...
        callback errors тАФ a notice must NEVER break the agent loop (D-D fail-open).
        """
    ...
    # тФАтФА Buffered retry/fallback status тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    # (agent.log) are unaffected тАФ every individual emission site still
    ...
    def _clear_status_buffer(self) -> None:
        """Drop buffered retry messages тАФ call on successful recovery."""
        try:
            buf = getattr(self, "_retry_status_buffer", None)
            if buf:
                buf.clear()
    ...
    def _flush_status_buffer(self) -> None:
        """Emit buffered retry messages тАФ call on terminal failure.
    ...
    def _disable_codex_reasoning_replay(
        ...
        counting what was stripped тАФ purely for diagnostic logging.
        """
        stripped_messages = 0
        stripped_items = 0
    ...
    # Stream-diagnostic class header preserved for backward compat тАФ
    ...
    def _stream_diag_init() -> Dict[str, Any]:
        """Forwarder тАФ see ``agent.stream_diag.stream_diag_init``."""
    ...
    def _stream_diag_capture_response(
        ...
        """Forwarder тАФ see ``agent.stream_diag.stream_diag_capture_response``."""
    ...
    def _flatten_exception_chain(error: BaseException) -> str:
        """Forwarder тАФ see ``agent.stream_diag.flatten_exception_chain``."""
    ...
    def _log_stream_retry(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.stream_diag.log_stream_retry``."""
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.stream_diag.emit_stream_drop``."""
    ...
    def _emit_auxiliary_failure(self, task: str, exc: BaseException) -> None:
        ...
        self._emit_warning(f"тЪа Auxiliary {task} failed: {detail}")
    ...
    def _check_compression_model_feasibility(self) -> None:
        """Forwarder тАФ see ``agent.conversation_compression.check_compression_model_feasibility``."""
    ...
    def _replay_compression_warning(self) -> None:
        """Forwarder тАФ see ``agent.conversation_compression.replay_compression_warning``."""
    ...
    def _is_azure_openai_url(self, base_url: str = None) -> bool:
        ...
        does NOT support the Responses API тАФ gpt-5.x models are served
        on the regular ``/chat/completions`` path тАФ so routing decisions
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        changes тАФ the heuristic stays in place as future-proofing even when
        ...
        # Match the gpt-5.5 family тАФ bare ``gpt-5.5``, ``gpt-5.5-codex``,
    ...
    def _anthropic_prompt_cache_policy(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.anthropic_prompt_cache_policy``."""
    ...
    def _model_requires_responses_api(model: str) -> bool:
        ...
        """
        m = model.lower()
        # Strip vendor prefix (e.g. "openai/gpt-5.4" тЖТ "gpt-5.4")
        if "/" in m:
            m = m.rsplit("/", 1)[-1]
    ...
    def _max_tokens_param(self, value: int) -> dict:
        ...
        OpenAI-compatible endpoints fronting those models are recognised тАФ
    ...
    def _strip_think_blocks(self, content: str) -> str:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.strip_think_blocks``."""
    ...
    def _has_natural_response_ending(content: str) -> bool:
        ...
        if stripped.endswith('^'):
            return True
        last = stripped[-1]
        if last in '.!?:)"\']}уАВя╝Бя╝Яя╝Ъя╝ЙуАСуАНуАПуАЛ^':
            return True
    ...
    def _looks_like_codex_intermediate_ack(
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.looks_like_codex_intermediate_ack``."""
    ...
    def _summarize_background_review_actions(
        ...
        """Forwarder тАФ see ``agent.background_review.summarize_background_review_actions``."""
    ...
    def _spawn_background_review(
        ...
        Thin wrapper тАФ the heavy lifting lives in
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.background_review.build_memory_write_metadata``."""
    ...
    def _drop_trailing_empty_response_scaffolding(self, messages: List[Dict]) -> None:
        ...
        ``...tool, user, user`` тАФ a protocol-invalid sequence that most
        ...
        # result. Only runs when scaffolding was actually present тАФ normal
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.repair_message_sequence``."""
    ...
    def _flush_messages_to_session_db(self, messages: List[Dict], conversation_history: List[Dict] = None):
        ...
        multiple exit paths) only write truly new messages тАФ preventing the
        ...
        try:
            ...
            for msg in messages:
                ...
                # Persist multimodal tool results as their text summary only тАФ
    ...
    def _format_tools_for_system_message(self) -> str:
        """Forwarder тАФ see ``agent.system_prompt.format_tools_for_system_message``."""
    ...
    def _convert_to_trajectory_format(self, messages: List[Dict[str, Any]], user_query: str, completed: bool) -> List[Dict[str, Any]]:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.convert_to_trajectory_format``."""
    ...
    def _is_entitlement_failure(
        ...
        the credentials failed validation тАФ that's recoverable by refreshing
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        that **X Premium+ does NOT include API access** тАФ only standalone
        ...
        Matched once per detail string тАФ won't double-decorate if the
        ...
        if not is_entitlement:
            return detail
        hint = (
            " тАФ xAI rejected this OAuth account. NOTE: X Premium+ does NOT "
            "include xAI API access тАФ only standalone SuperGrok subscribers "
    ...
    def _mask_api_key_for_logs(self, key: Any) -> Optional[str]:
        # Azure Foundry Entra ID bearer providers are callables тАФ never
    ...
    def _extract_api_error_context(error: Exception) -> Dict[str, Any]:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.extract_api_error_context``."""
    ...
    def _api_response_payload_for_hook(
        ...
        *,
        ...
        # ``__dict__`` / dataclass introspection тАФ a future refactor of
    ...
    def _dump_api_request_debug(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.dump_api_request_debug``."""
    ...
    def _redact_message_content(content):
        ...
        Respects ``HERMES_REDACT_SECRETS`` via ``redact_sensitive_text`` тАФ
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        try:
            ...
            for msg in messages:
                ...
                # redact_sensitive_text тАФ no-op when disabled. (#19798, #19845)
                if "content" in msg:
                    msg = dict(msg)
            ...
            if log_file.exists():
                try:
                    ...
                    pass  # corrupted existing file тАФ allow the overwrite
    ...
    def interrupt(self, message: str = None) -> None:
        ...
        if not self.quiet_mode:
            print("\nтЪб Interrupt requested" + (f": '{message[:40]}...'" if message and len(message) > 40 else f": '{message}'" if message else ""))
        ...
        if _tracker is not None and _tracker_lock is not None:
            ...
            for _wtid in _worker_tids:
                try:
                    ...
                    pass
        # A hard interrupt supersedes any pending /steer тАФ the steer was
    ...
    def _neutralize_footer_paths(cls, text: str) -> str:
        ...
        any that exist on disk as native attachments тАФ but it explicitly
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        Every file path that reaches the user-facing text тАФ both the bullet
        path and any path echoed inside the tool's error preview тАФ is
        ...
        for path, info in failed.items():
            ...
            if preview:
                lines.append(f"  тАв `{path}` тАФ [{tool}] {preview}")
            else:
                lines.append(f"  тАв `{path}` тАФ [{tool}] failed")
            shown += 1
        remaining = len(failed) - shown
        if remaining > 0:
            lines.append(f"  тАв тАж and {remaining} more")
    ...
    def _format_turn_completion_explanation(turn_exit_reason: str) -> str:
        ...
        the UI's perspective тАФ the symptom users report in #34452.
        ...
        # which already surfaces its own message) тАФ don't second-guess.
        return ""

    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.apply_pending_steer_to_tool_results``."""
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        best-effort тАФ it never raises into the agent loop.
        """
    ...
    def _capture_credits(self, http_response: Any) -> None:
        ...
        Fail-open throughout тАФ header issues never break the agent loop. The PARSE is
        swallowed (any error тЖТ treated as a miss тЖТ keep last-known). The notice
        ...
        if _fixture is not None:
            ...
            logger.info(
                "credits тЦ╕ [FIXTURE] remaining=%d (%s) ┬╖ paid=%s ┬╖ denom=%s ┬╖ used=%s "
                "(real headers bypassed тАФ `echo clear` / unset HERMES_DEV_CREDITS_FIXTURE to restore)",
        ...
        # тФАтФА Parse (fail-open тЖТ miss; never overwrite good state with None) тФАтФА
        try:
            from agent.credits_tracker import parse_credits_headers
            state = parse_credits_headers(headers, provider=self.provider)
        except Exception:
            return  # parse error тЖТ treat as a miss, keep last-known
        if state is None:
            if _dev:
                logger.info(
                    "credits тЦ╕ response had no valid x-nous-credits-* headers "
                    "(miss тАФ producer off / non-Nous path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # HERMES_DEV_CREDITS: stream each capture to agent.log тАФ watch live with
            # `hermes logs -f` (grep 'credits тЦ╕'). Dev-only; silent for normal users.
            spent = self.get_credits_spent_micros()
            used = state.used_fraction
            logger.info(
                "credits тЦ╕ remaining=%d (%s) ┬╖ paid=%s ┬╖ denom=%s ┬╖ used=%s "
                "┬╖ ╬Фspent=%s ┬╖ age=%s%s",
                ...
                ("%.1f┬в" % (spent / 10000)) if spent is not None else "n/a",
        ...
        # Threshold notices тАФ shared with the cold-start seed (see _emit_credits_notices).
        self._emit_credits_notices()
    ...
    def _emit_credits_notices(self) -> None:
        ...
        session that opens already depleted warns immediately тАФ not only after the first
        inference header. Runs only when a notice consumer is bound (messaging binds none
        тЖТ state still cached for /usage, no policy). WARNS on failure rather than
        ...
        try:
            ...
            # only (":free" suffix + pricing-cache peek) тАФ never a network call.
            ...
            for key in to_clear:        # clears FIRST тАж
                self._emit_notice_clear(key)
            for notice in to_show:      # тАж then shows (depleted lands last in a latest-wins slot)
    ...
    def _credits_notices_enabled(self) -> bool:
        ...
        Read once per agent and cached тАФ the policy runs after every API
    ...
    def shutdown_memory_provider(self, messages: list = None) -> None:
        """Shut down the memory provider and context engine тАФ call at actual session boundaries.

        This calls on_session_end() then shutdown_all() on the memory
        manager, and on_session_end() on the context engine.
        NOT called per-turn тАФ only at CLI exit, /reset, gateway
        session expiry, etc.
        """
    ...
    def commit_memory_session(self, messages: list = None) -> None:
        ...
        session_id тАФ they just flush pending extraction now."""
        if self._memory_manager:
            try:
                self._memory_manager.on_session_end(messages or [])
            except Exception:
                pass
        # Notify context engine of session end too тАФ same lifecycle moment as
    ...
    def _sync_external_memory_for_turn(
        ...
        *,
        ...
        is not durable conversational truth тАФ mirroring it into an
        ...
        providers are strictly best-effort тАФ a misconfigured or offline
    ...
    def release_clients(self) -> None:
        ...
        memory-management reasons (LRU cap or idle TTL) тАФ the session may
        ...
        Safe to call multiple times.  Distinct from close() тАФ which is the
    ...
    def close(self) -> None:
        ...
        # soft-eviction clear тАФ close() is the hard teardown for true session
    ...
    def _hydrate_todo_store(self, history: List[Dict[str, Any]]) -> None:
        ...
        if last_todo_response:
            ...
            if not self.quiet_mode:
                self._vprint(f"{self.log_prefix}ЁЯУЛ Restored {len(last_todo_response)} todo item(s) from history")
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        """Forwarder тАФ see ``agent.system_prompt.build_system_prompt_parts``."""
        from agent.system_prompt import build_system_prompt_parts
        return build_system_prompt_parts(self, system_message=system_message)

    def _build_system_prompt(self, system_message: str = None) -> str:
        """Forwarder тАФ see ``agent.system_prompt.build_system_prompt``."""
    ...
    def _sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.sanitize_api_messages``."""
    ...
    def _is_thinking_only_assistant(
        ...
        *,
        ...
        gateways), the resulting message has only thinking blocks тАФ which
        ...
        rather than fabricating stub text тАФ the message log (UI transcript)
            ...
            for block in content:
                ...
                if btype == "text":
                    ...
                    if isinstance(text, str) and text.strip():
                        return False
                    continue
                # tool_use, image, document, etc. тАФ real payload
                return False
    ...
    def _drop_thinking_only_and_merge_users(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.drop_thinking_only_and_merge_users``."""
    ...
    def _repair_tool_call(self, tool_name: str) -> str | None:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.repair_tool_call``."""
        from agent.agent_runtime_helpers import repair_tool_call
        return repair_tool_call(self, tool_name)

    def _invalidate_system_prompt(self):
        """Forwarder тАФ see ``agent.system_prompt.invalidate_system_prompt``."""
    ...
    def _deterministic_call_id(fn_name: str, arguments: str, index: int = 0) -> str:
        ...
        Deterministic IDs prevent cache invalidation тАФ random UUIDs would
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.create_openai_client``."""
        from agent.agent_runtime_helpers import create_openai_client
        return create_openai_client(self, client_kwargs, reason=reason, shared=shared)

    @staticmethod
    def _force_close_tcp_sockets(client: Any) -> int:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.force_close_tcp_sockets``."""
    ...
    def _cleanup_dead_connections(self) -> bool:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.cleanup_dead_connections``."""
    ...
    def _abort_request_openai_client(self, client: Any, *, reason: str) -> None:
        ...
        тАФ which is where the FD release belongs.
        """
        if client is None:
            return
        try:
    ...
    def _run_codex_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        """Forwarder тАФ see ``agent.codex_runtime.run_codex_stream``."""
        from agent.codex_runtime import run_codex_stream
        return run_codex_stream(self, api_kwargs, client, on_first_delta)

    def _run_codex_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        """Forwarder тАФ see ``agent.codex_runtime.run_codex_create_stream_fallback``."""
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        #
        # When an agent is using a non-singleton credential тАФ e.g. a manual
        ...
        # constructed with an explicit ``api_key=`` arg тАФ force-refreshing
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        if self.provider != "anthropic":
            return False
        # Azure endpoints use static API keys тАФ OAuth token rotation doesn't apply.
        ...
        # Update OAuth flag тАФ token type may have changed (API key тЖФ OAuth).
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            # No URL-specific headers тАФ check profile.default_headers before clearing.
            _ph_headers = None
            try:
        ...
        # User-configured overrides win over URL/profile defaults тАФ keep them
    ...
    def _recover_with_credential_pool(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.recover_with_credential_pool``."""
    ...
    def _rebuild_anthropic_client(self) -> None:
        ...
        correctly тАФ rebuilding with the Bedrock SDK when provider is bedrock,
    ...
    def _interruptible_api_call(self, api_kwargs: dict):
        """Forwarder тАФ see ``agent.chat_completion_helpers.interruptible_api_call``."""
        from agent.chat_completion_helpers import interruptible_api_call
        return interruptible_api_call(self, api_kwargs)

    # тФАтФА Unified streaming API call тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    def _reset_stream_delivery_tracking(self) -> None:
        ...
        # flush the context scrubber.  Order matters тАФ the think
    ...
    def _fire_stream_delta(self, text: str) -> None:
        ...
        if isinstance(text, str):
            ...
            # sends '<think>' and its content as separate deltas тАФ
            ...
            # Only strip leading newlines on the first delta тАФ mid-stream "\n" is legitimate markdown.
    ...
    def _interruptible_streaming_api_call(
        ...
        """Forwarder тАФ see ``agent.chat_completion_helpers.interruptible_streaming_api_call``."""
        from agent.chat_completion_helpers import interruptible_streaming_api_call
        return interruptible_streaming_api_call(self, api_kwargs, on_first_delta=on_first_delta)

    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        """Forwarder тАФ see ``agent.chat_completion_helpers.try_activate_fallback``."""
    ...
    # тФАтФА Per-turn primary restoration тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА

    def _restore_primary_runtime(self) -> bool:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.restore_primary_runtime``."""
    ...
    def _try_recover_primary_transport(
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.try_recover_primary_transport``."""
    ...
    def _prepare_messages_for_non_vision_model(self, api_messages: list) -> list:
        ...
        for msg in transformed:
            if not isinstance(msg, dict):
                continue
            # Reuse the Anthropic text-fallback preprocessor тАФ the behaviour is
    ...
    def _tool_result_content_for_active_model(self, tool_name: str, result: Any) -> Any:
        ...
        if self._model_supports_vision():
            # Vision-capable on paper тАФ but if the provider rejects list-type
            ...
            if not self._provider_supports_vision_tool_messages():
                logger.debug(
                    ...
                    "content тАФ sending text summary",
            ...
            if no_list and key in no_list:
                logger.debug(
                    ...
                    "content this session тАФ sending text summary",
    ...
    def _try_shrink_image_parts_in_messages(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.conversation_compression.try_shrink_image_parts_in_messages``."""
    ...
    def _try_strip_image_parts_from_tool_messages(self, api_messages: list) -> bool:
        ...
        Returns True when at least one tool message was downgraded тАФ the
        ...
        for msg in api_messages:
            ...
            if not had_image:
                # List-type content but no image parts тАФ leave alone (some
            ...
            if text_parts:
                ...
                msg["content"] = (
                    "[image content removed тАФ provider does not accept "
    ...
    def _anthropic_preserve_dots(self) -> bool:
        ...
        return (
            ...
            # AWS Bedrock runtime endpoints тАФ defense-in-depth when
    ...
    def _qwen_prepare_chat_messages_inplace(self, messages: list) -> None:
        """In-place variant тАФ mutates an already-copied message list."""
        if not messages:
            return
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        """Forwarder тАФ see ``agent.chat_completion_helpers.build_api_kwargs``."""
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            opts, ts = cached
            # Non-empty тЖТ permanent. Empty тЖТ 60s TTL.
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        """Forwarder тАФ see ``agent.chat_completion_helpers.build_assistant_message``."""
    ...
    def _needs_thinking_reasoning_pad(self) -> bool:
        ...
        ``_try_activate_fallback()`` mutate any of those. This is hot тАФ the
    ...
    def _copy_reasoning_content_for_api(self, source_msg: dict, api_msg: dict) -> None:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.copy_reasoning_content_for_api``."""
    ...
    def _reapply_reasoning_echo_for_provider(self, api_messages: list) -> int:
        """Forwarder тАФ see ``agent.agent_runtime_helpers.reapply_reasoning_echo_for_provider``."""
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        ...
        the internal message history тАФ this method only modifies the outgoing
        API copy.

        ``extra_content`` (Gemini thought_signature) is also stripped тАФ strict
        providers reject it with "Extra inputs are not permitted" тАФ UNLESS the
    ...
    def _sanitize_tool_call_arguments(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.sanitize_tool_call_arguments``."""
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None, force: bool = False) -> tuple:
        """Forwarder тАФ see ``agent.conversation_compression.compress_context``.
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        """Forwarder тАФ see ``agent.agent_runtime_helpers.invoke_tool``."""
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        """Forwarder тАФ see ``agent.tool_executor.execute_tool_calls_concurrent``."""
    ...
    def _execute_tool_calls_sequential(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        """Forwarder тАФ see ``agent.tool_executor.execute_tool_calls_sequential``."""
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        """Forwarder тАФ see ``agent.chat_completion_helpers.handle_max_iterations``."""
    ...
    def run_conversation(
        ...
        """Forwarder тАФ see ``agent.conversation_loop.run_conversation``."""
    ...
    def _run_codex_app_server_turn(
        ...
        *,
        ...
        """Forwarder тАФ see ``agent.codex_runtime.run_codex_app_server_turn``."""
...
def main(
    ...
    """
    print("ЁЯдЦ AI Agent with Tool Calling")
    print("=" * 50)
    ...
    if list_tools:
        ...
        print("ЁЯУЛ Available Tools & Toolsets:")
        print("-" * 50)
        
        # Show new toolsets system
        print("\nЁЯОп Predefined Toolsets (New System):")
        print("-" * 40)
        ...
        for name, info in basic_toolsets:
            ...
            print(f"  тАв {name:15} - {info['description']}")
            print(f"    Tools: {tools_str}")
        
        # Print composite toolsets
        print("\nЁЯУВ Composite Toolsets (built from other toolsets):")
        ...
        print("\nЁЯОн Scenario-Specific Toolsets:")
        for name, info in scenario_toolsets:
            print(f"  тАв {name:20} - {info['description']}")
            print(f"    Total tools: {info['tool_count']}")
        
        
        # Show legacy toolset compatibility
        print("\nЁЯУж Legacy Toolsets (for backward compatibility):")
        legacy_toolsets = get_available_toolsets()
        for name, info in legacy_toolsets.items():
            status = "тЬЕ" if info["available"] else "тЭМ"
        ...
        print(f"\nЁЯФз Individual Tools ({len(all_tools)} available):")
        for tool_name in sorted(all_tools):
            toolset = get_toolset_for_tool(tool_name)
            print(f"  ЁЯУМ {tool_name} (from {toolset})")
    ...
    if enabled_toolsets:
        ...
        print(f"ЁЯОп Enabled toolsets: {enabled_toolsets_list}")
    
    if disabled_toolsets:
        disabled_toolsets_list = [t.strip() for t in disabled_toolsets.split(",")]
        print(f"ЁЯЪл Disabled toolsets: {disabled_toolsets_list}")
    
    if save_trajectories:
        print("ЁЯТ╛ Trajectory saving: ENABLED")
        print("   - Successful conversations тЖТ trajectory_samples.jsonl")
        print("   - Failed conversations тЖТ failed_trajectories.jsonl")
    ...
    print("\n" + "=" * 50)
    print("ЁЯУЛ CONVERSATION SUMMARY")
    print("=" * 50)
    print(f"тЬЕ Completed: {result['completed']}")
    print(f"ЁЯУЮ API Calls: {result['api_calls']}")
    print(f"ЁЯТм Messages: {len(result['messages'])}")
    ...
    if save_sample:
        ...
        try:
            ...
            print(f"\nЁЯТ╛ Sample trajectory saved to: {sample_filename}")
        except Exception as e:
            print(f"\nтЪая╕П Failed to save sample: {e}")
    
    print("\nЁЯСЛ Agent execution completed!")
