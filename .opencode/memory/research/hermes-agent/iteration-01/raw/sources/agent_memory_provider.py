...
Lifecycle (called by MemoryManager, wired in run_agent.py):
  initialize()          тАФ connect, create resources, warm up
  system_prompt_block()  тАФ static text for the system prompt
  prefetch(query)        тАФ background recall before each turn
  sync_turn(user, asst)  тАФ async write after each turn
  get_tool_schemas()     тАФ tool schemas to expose to the model
  handle_tool_call()     тАФ dispatch a tool call
  shutdown()             тАФ clean exit

Optional hooks (override to opt in):
  on_turn_start(turn, message, **kwargs) тАФ per-turn tick with runtime context
  on_session_end(messages)               тАФ end-of-session extraction
  on_session_switch(new_session_id, **kwargs) тАФ mid-process session_id rotation
  on_pre_compress(messages) -> str       тАФ extract before context compression
  on_memory_write(action, target, content, metadata=None) тАФ mirror built-in memory writes
  on_delegation(task, result, **kwargs)  тАФ parent-side observation of subagent work
"""
...
class MemoryProvider(ABC):
    ...
    def is_available(self) -> bool:
        ...
        Should not make network calls тАФ just check config and installed deps.
        """
    ...
    def prefetch(self, query: str, *, session_id: str = "") -> str:
        ...
        should be fast тАФ use background threads for the actual recall
    ...
    def queue_prefetch(self, query: str, *, session_id: str = "") -> None:
        ...
        by prefetch() on the next turn. Default is no-op тАФ providers
    ...
    def sync_turn(
        ...
        *,
        ...
        Called after each turn. Should be non-blocking тАФ queue for
    ...
    def shutdown(self) -> None:
        """Clean shutdown тАФ flush queues, close connections."""
    ...
    def on_session_end(self, messages: List[Dict[str, Any]]) -> None:
        ...
        NOT called after every turn тАФ only at actual session boundaries
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        gateway equivalents, and context compression тАФ any path that
        ...
        parent_session_id:
            The previous session_id, if meaningful тАФ set for ``/branch``
