"""The agent conversation loop тАФ extracted from ``run_agent.AIAgent``.
...
def _print_nous_entitlement_guidance(agent, capability: str) -> bool:
    ...
    for line in message.splitlines():
        agent._vprint(f"{agent.log_prefix}   ЁЯТб {line}", force=True)
    return True
...
def _restore_or_build_system_prompt(agent, system_message, conversation_history):
    ...
    if conversation_history and agent._session_db:
        try:
            ...
            logger.warning(
                ...
                "(session=%s): %s. Falling back to fresh build тАФ prefix "
                "cache will miss for this turn.",
                agent.session_id, exc,
            )

    if stored_prompt and _stored_prompt_matches_runtime(agent, stored_prompt):
        # Continuing session тАФ reuse the exact system prompt from the
    ...
    if conversation_history and stored_state in ("null", "empty"):
        ...
        # string тАФ either way every turn now rebuilds and the prefix
    ...
    # prompt) тАФ build from scratch.
    agent._cached_system_prompt = agent._build_system_prompt(system_message)

    # Plugin hook: on_session_start тАФ fired once when a brand-new
    ...
    # Cold-start credits seed (L3) тАФ fallback for the first-turn path. The TUI/
    ...
    # gateway path (fresh AIAgent per turn тЖТ reads from this row every
    # subsequent turn).
...
def _content_policy_blocked_result(
    ...
    *,
    ...
    exception-path handler return the identical shape тАФ a failed, non-completed
    turn carrying the user-facing message and a ``content_policy_blocked:``
    prefixed error тАФ so they funnel through this one builder.
    """
    return {
...
def run_conversation(
    ...
    """
    # тФАтФА Per-turn setup (the prologue) тФАтФА
    # All once-per-turn setup тАФ stdio guarding, retry-counter resets, user
    # message sanitization, todo/nudge hydration, system-prompt restore-or-
    # build, crash-resilience persistence, preflight compression, the
    # ``pre_llm_call`` plugin hook, and external-memory prefetch тАФ lives in
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        if agent._interrupt_requested:
            ...
            if not agent.quiet_mode:
                agent._safe_print("\nтЪб Breaking out of tool loop due to interrupt...")
            break
        ...
        if agent._budget_grace_call:
            ...
            if not agent.quiet_mode:
                agent._safe_print(f"\nтЪая╕П  Iteration budget exhausted ({agent.iteration_budget.used}/{agent.iteration_budget.max_total} iterations used)")
            break
        ...
        # тФАтФА Pre-API-call /steer drain тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        # If a /steer arrived during the previous API call (while the model
        # was thinking), drain it now тАФ before we build api_messages тАФ so
        ...
        # tool batch тАФ injecting into a user message would break role
            ...
            for _si in range(len(messages) - 1, -1, -1):
                ...
                if isinstance(_sm, dict) and _sm.get("role") == "tool":
                    ...
                    if isinstance(existing, str):
                        ...
                        # Multimodal content blocks тАФ append text block
                        try:
                    ...
                    break
            if not _injected:
                # No tool message to inject into тАФ put it back so
        ...
        # ``tool тЖТ user`` or ``user тЖТ user`` tail (e.g. after empty-
        ...
        for idx, msg in enumerate(messages):
            ...
            # API-call-time only тАФ the original message in `messages` is
        ...
        # This is intentional тАФ system prompt modifications break the prompt
        ...
        # results before sending to the API.  Runs unconditionally тАФ not
        # gated on context_compressor тАФ so orphans from session loading or
        ...
        # a thinking-only turn. Runs on the per-call copy only тАФ the
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("тЭМ Ollama runtime context is too small for Hermes tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        if not agent.quiet_mode:
            agent._vprint(f"\n{agent.log_prefix}ЁЯФД Making API call #{api_call_count}/{agent.max_iterations}...")
            agent._vprint(f"{agent.log_prefix}   ЁЯУК Request size: {len(api_messages)} messages, ~{approx_tokens:,} tokens (~{total_chars:,} chars)")
            agent._vprint(f"{agent.log_prefix}   ЁЯФз Available tools: {len(agent.tools) if agent.tools else 0}")
        else:
        ...
        while retry_count < max_retries:
            # тФАтФА Nous Portal rate limit guard тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
            ...
            if agent.provider == "nous":
                try:
                    ...
                    if _nous_remaining is not None and _nous_remaining > 0:
                        _nous_msg = (
                            f"Nous Portal rate limit active тАФ "
                            f"resets in {_fmt_nous_remaining(_nous_remaining)}."
                        )
                        agent._buffer_vprint(
                            f"тП│ {_nous_msg} Trying fallback..."
                        )
                        agent._buffer_status(f"тП│ {_nous_msg}")
                        if agent._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            _retry.primary_recovery_attempted = False
                            continue
                        # No fallback available тАФ surface buffered context
            ...
            try:
                ...
                try:
                    ...
                    if has_hook("pre_api_request"):
                        ...
                        # sanitised тАФ secrets are not expected here because
                ...
                # Always prefer the streaming path тАФ even without stream
                ...
                # attempt тАФ switch to non-streaming for the rest of this
                # session instead of re-failing every retry.
                if getattr(agent, "_disable_streaming", False):
                    _use_streaming = False
                # CopilotACPClient communicates via subprocess stdio and
                # returns a plain SimpleNamespace тАФ not an iterable
                ...
                if not agent.quiet_mode:
                    agent._vprint(f"{agent.log_prefix}тП▒я╕П  API call completed in {api_duration:.2f}s")
                ...
                if response_invalid:
                    ...
                    # Stop spinner silently тАФ retry status is now buffered
                    ...
                    # Invalid response тАФ could be rate limiting, provider timeout,
                    ...
                    if agent._fallback_index < len(agent._fallback_chain):
                        agent._buffer_status("тЪая╕П Empty/malformed response тАФ switching to fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                    ...
                    if _resp_error_code == 524:
                        ...
                        _failure_hint = f"fast response ({api_duration:.1f}s) тАФ likely rate limited"
                    elif api_duration > 60:
                        _failure_hint = f"slow response ({api_duration:.0f}s) тАФ likely upstream timeout"
                    else:
                        _failure_hint = f"response time {api_duration:.1f}s"

                    agent._buffer_vprint(f"тЪая╕П  Invalid API response (attempt {retry_count}/{max_retries}): {', '.join(error_details)}")
                    agent._buffer_vprint(f"   ЁЯПв Provider: {provider_name}")
                    cleaned_provider_error = agent._clean_error_message(error_msg)
                    agent._buffer_vprint(f"   ЁЯУЭ Provider message: {cleaned_provider_error}")
                    agent._buffer_vprint(f"   тП▒я╕П  {_failure_hint}")
                    
                    if retry_count >= max_retries:
                        # Try fallback before giving up
                        if agent._has_pending_fallback():
                            agent._buffer_status(f"тЪая╕П Max retries ({max_retries}) for invalid responses тАФ trying fallback...")
                        if agent._try_activate_fallback():
                            retry_count = 0
                            compression_attempts = 0
                            _retry.primary_recovery_attempted = False
                            continue
                        # Terminal тАФ flush buffered retry trace so user sees what happened.
                        agent._flush_status_buffer()
                        agent._emit_status(f"тЭМ Max retries ({max_retries}) exceeded for invalid responses. Giving up.")
                    ...
                    # Backoff before retry тАФ jittered exponential: 5s base, 120s cap
                    wait_time = jittered_backoff(retry_count, base_delay=5.0, max_delay=120.0)
                    agent._buffer_vprint(f"тП│ Retrying in {wait_time:.1f}s ({_failure_hint})...")
                    ...
                    while time.time() < sleep_end:
                        if agent._interrupt_requested:
                            agent._vprint(f"{agent.log_prefix}тЪб Interrupt detected during retry wait, aborting.", force=True)
                            agent._persist_session(messages, conversation_history)
                        ...
                        if _backoff_touch_counter % 150 == 0:  # 150 ├Ч 0.2s = 30s
                ...
                if agent.api_mode == "codex_responses":
                    ...
                    # Bedrock response already normalized at dispatch тАФ use transport
                    ...
                    if agent._should_treat_stop_as_truncated(
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  Treating suspicious Ollama/GLM stop response as truncated",
                            force=True,
                        )
                        finish_reason = "length"

                # тФАтФА Content-policy refusal (HTTP 200) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                # The model тАФ or the provider's safety system тАФ returned a
                # *successful* response whose stop/finish reason is a refusal:
                # Anthropic ``stop_reason="refusal"`` тЖТ ``content_filter``;
                ...
                # retries" тАФ burning paid attempts reproducing a deterministic
                ...
                if finish_reason == "content_filter":
                    ...
                    # Deterministic for the unchanged prompt тАФ never retry.
                    # Try a configured fallback once (a different model may not
                    # refuse); otherwise surface the refusal terminally.
                    if agent._has_pending_fallback():
                        agent._buffer_status(
                            "тЪая╕П Model declined to respond (safety refusal) тАФ trying fallback..."
                        )
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                    ...
                    agent._emit_status(
                        "тЪая╕П The model declined to respond to this request (safety refusal)."
                    )
                    ...
                    _refusal_response = (
                        "тЪая╕П  The model declined to respond to this request "
                        "(safety refusal тАФ not a Hermes/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if finish_reason == "length":
                    if getattr(response, "id", "") == PARTIAL_STREAM_STUB_ID:
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  Stream interrupted by network error "
                            f"(finish_reason='length' on partial-stream-stub)",
                            force=True,
                        )
                    else:
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  Response truncated "
                    ...
                    # тФАтФА Detect thinking-budget exhaustion тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    ...
                    # string for unrelated reasons тАФ treat those as normal
                    ...
                    if _thinking_exhausted:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ЁЯТн Reasoning exhausted the output token budget тАФ "
                        ...
                        _exhaust_response = (
                            "тЪая╕П **Thinking Budget Exhausted**\n\n"
                            "The model used all its output tokens on reasoning "
                            "and had none left for the actual response.\n\n"
                            "To fix this:\n"
                            "тЖТ Lower reasoning effort: `/thinkon low` or `/thinkon minimal`\n"
                            "тЖТ Or switch to a larger/non-reasoning model with `/model`"
                        )
                        agent._cleanup_task_resources(effective_task_id)
                        agent._persist_session(messages, conversation_history)
                        return {
                        ...
                        if assistant_message is not None and not _trunc_has_tool_calls:
                            ...
                            if length_continue_retries < 3:
                                ...
                                if _is_partial_stream_stub and _dropped_tools:
                                    ...
                                    agent._vprint(
                                        f"{agent.log_prefix}тЖ╗ Stream interrupted mid "
                                        f"tool-call ({_tool_list}) тАФ requesting "
                                        f"chunked retry "
                                        f"({length_continue_retries}/3)..."
                                    )
                                elif _is_partial_stream_stub:
                                    agent._vprint(
                                        f"{agent.log_prefix}тЖ╗ Stream interrupted тАФ "
                                        f"requesting continuation "
                                        f"({length_continue_retries}/3)..."
                                    )
                                else:
                                    agent._vprint(
                                        f"{agent.log_prefix}тЖ╗ Requesting continuation "
                                        f"({length_continue_retries}/3)..."
                                    )
                        ...
                        if assistant_message is not None and _trunc_has_tool_calls:
                            ...
                            if truncated_tool_call_retries < 3:
                                ...
                                if _is_stub_stall:
                                    ...
                                    # cap тАФ say so instead of "max output tokens".
                                    agent._buffer_vprint(
                                        f"тЪая╕П  Stream interrupted mid tool-call тАФ "
                                        f"retrying ({truncated_tool_call_retries}/3)..."
                                    )
                                else:
                                    agent._buffer_vprint(
                                        f"тЪая╕П  Truncated tool call detected тАФ "
                                        f"retrying API call "
                            ...
                            if _is_stub_stall:
                                agent._vprint(
                                    f"{agent.log_prefix}тЪая╕П  Stream kept dropping mid tool-call after 3 retries тАФ the action was not executed.",
                                    force=True,
                                )
                            else:
                                agent._vprint(
                                    f"{agent.log_prefix}тЪая╕П  Truncated tool call response detected again тАФ refusing to execute incomplete tool arguments.",
                                    force=True,
                                )
                            agent._cleanup_task_resources(effective_task_id)
                            agent._persist_session(messages, conversation_history)
                    ...
                    if len(messages) > 1:
                        agent._vprint(f"{agent.log_prefix}   тПк Rolling back to last complete assistant turn")
                        ...
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}тЭМ First response truncated - cannot recover", force=True)
                        agent._persist_session(messages, conversation_history)
                        return {
                            "final_response": None,
                            "messages": messages,
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    if getattr(agent.context_compressor, "_context_probed", False):
                        ...
                        if getattr(agent.context_compressor, "_context_probe_persistable", False):
                            ...
                            agent._safe_print(f"{agent.log_prefix}ЁЯТ╛ Cached context length: {ctx:,} tokens for {agent.model}")
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # visible in agent.log тАФ silent loss here is
                    ...
                    # them тАФ not just those where we inject cache_control
                    ...
                    if (cached or written) and not agent.quiet_mode:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}   ЁЯТ╛ Cache: "
                            f"{cached:,}/{prompt:,} tokens "
                            f"({hit_pct:.0f}% hit, {written:,} written)"
                        )
                
                _retry.has_retried_429 = False  # Reset on success
                # Note: don't clear the retry buffer here тАФ an "API call
                ...
                # Clear Nous rate limit state on successful request тАФ
                ...
                agent._vprint(f"{agent.log_prefix}тЪб Interrupted during API call.", force=True)
                agent._persist_session(messages, conversation_history)
                interrupted = True
                ...
                # Stop spinner silently тАФ retry status is buffered and
                ...
                #      (Google Docs, rich-text editors) тАФ sanitize and retry.
                #   2. ASCII codec on systems with LANG=C or non-UTF-8 locale
                #      (e.g. Chromebooks) тАФ any non-ASCII character fails.
                ...
                if isinstance(api_error, UnicodeEncodeError) and getattr(agent, '_unicode_sanitization_passes', 0) < 2:
                    ...
                    # Detect surrogate errors тАФ utf-8 codec refusing to
                    ...
                    # from `reasoning` тАФ fields the canonical list doesn't
                    ...
                    # found anything тАФ _force_ascii_payload / the extended
                    ...
                    if _surrogates_found or _is_surrogate_error:
                        ...
                        if _surrogates_found:
                            agent._buffer_vprint(
                                f"тЪая╕П  Stripped invalid surrogate characters from messages. Retrying..."
                            )
                        else:
                            agent._buffer_vprint(
                                f"тЪая╕П  Surrogate encoding error тАФ retrying after full-payload sanitization..."
                            )
                        continue
                    if _is_ascii_codec:
                        ...
                        # Sanitize the API key тАФ non-ASCII characters in
                        # credentials (e.g. ╩Л instead of v from a bad
                        ...
                        # Entra ID bearer providers are callables тАФ their
                        ...
                        if _raw_key and isinstance(_raw_key, str):
                            ...
                            if _clean_key != _raw_key:
                                ...
                                # Also update the live client тАФ it holds its
                                ...
                                agent._vprint(
                                    f"{agent.log_prefix}тЪая╕П  API key contained non-ASCII characters "
                                    f"(bad copy-paste?) тАФ stripped them. If auth fails, "
                                    f"re-copy the key from your provider's dashboard.",
                                    force=True,
                                )

                        # Always retry on ASCII codec detection тАФ
                        ...
                        if _any_sanitized:
                            agent._vprint(
                                f"{agent.log_prefix}тЪая╕П  System encoding is ASCII тАФ stripped non-ASCII characters from request payload. Retrying...",
                                force=True,
                            )
                        else:
                            agent._vprint(
                                f"{agent.log_prefix}тЪая╕П  System encoding is ASCII тАФ enabling full-payload sanitization for retry...",
                                force=True,
                            )
                        continue

                # тФАтФА Image-rejection recovery тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                ...
                #
                # Detection is best-effort English phrase matching тАФ a
                ...
                _IMAGE_REJECTION_PHRASES = (
                    ...
                    # purpose тАФ keyed on the field-path apostrophe so
                ...
                # said no to images" тАФ those are transient and must
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}тЪая╕П  Server rejected image content тАФ "
                ...
                # тФАтФА Classify the error for structured recovery decisions тФАтФА
                ...
                if (
                    ...
                    if _try_refresh_nous_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}ЁЯФР Nous paid access verified тАФ "
                ...
                if (
                    ...
                    if agent._try_shrink_image_parts_in_messages(
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ЁЯУР Image(s) exceeded provider size limit тАФ "
                ...
                if (
                    ...
                    if agent._try_strip_image_parts_from_tool_messages(api_messages):
                        agent._vprint(
                            f"{agent.log_prefix}ЁЯУР Provider rejected list-type tool content тАФ "
                ...
                # subscriptions never hit this branch тАФ they accept the
                ...
                if (
                    ...
                    if not getattr(agent, "_oauth_1m_beta_disabled", False):
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}ЁЯФХ OAuth subscription doesn't support "
                            f"the 1M-context beta тАФ disabled for this session and retrying...",
                            force=True,
                        )
                        continue
                ...
                if (
                    ...
                    if agent._try_refresh_codex_client_credentials(force=True):
                        ...
                        agent._buffer_vprint(f"ЁЯФР {_label} auth refreshed after 401. Retrying request...")
                        continue
                if (
                    ...
                    if agent._try_refresh_nous_client_credentials(force=True):
                        print(f"{agent.log_prefix}ЁЯФР Nous agent key refreshed after 401. Retrying request...")
                        continue
                    # Credential refresh didn't help тАФ show diagnostic info.
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}ЁЯФР Nous 401 тАФ Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_nous_entitlement_guidance(agent, "Nous model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     тАв Re-authenticate: hermes auth add nous")
                    print(f"{agent.log_prefix}     тАв Check credits / billing: https://portal.nousresearch.com")
                    print(f"{agent.log_prefix}     тАв Verify stored credentials: {_dhh}/auth.json")
                    print(f"{agent.log_prefix}     тАв Switch providers temporarily: /model <model> --provider openrouter")
                if (
                    ...
                    if agent._try_refresh_copilot_client_credentials():
                        agent._buffer_vprint(f"ЁЯФР Copilot credentials refreshed after 401. Retrying request...")
                        continue
                if (
                    ...
                    if agent._try_refresh_anthropic_client_credentials():
                        print(f"{agent.log_prefix}ЁЯФР Anthropic credentials refreshed after 401. Retrying request...")
                        continue
                    # Credential refresh didn't help тАФ show diagnostic info
                    key = agent._anthropic_api_key
                    print(f"{agent.log_prefix}ЁЯФР Anthropic 401 тАФ authentication failed.")
                    if is_token_provider(key):
                        # Azure Foundry Entra ID тАФ the bearer token is
                    ...
                    from hermes_constants import display_hermes_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     тАв Check ANTHROPIC_TOKEN in {_dhh}/.env for Hermes-managed OAuth/setup tokens")
                    print(f"{agent.log_prefix}     тАв Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                    print(f"{agent.log_prefix}     тАв For API keys: verify at https://platform.claude.com/settings/keys")
                    print(f"{agent.log_prefix}     тАв For Claude Code: run 'claude /login' to refresh, then retry")
                    print(f"{agent.log_prefix}     тАв Legacy cleanup: hermes config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     тАв Clear stale keys: hermes config set ANTHROPIC_API_KEY \"\"")
                ...
                if (
                    ...
                    for _m in api_messages:
                        if isinstance(_m, dict) and "reasoning_details" in _m:
                            ...
                            _api_stripped += 1
                    agent._vprint(
                        f"{agent.log_prefix}тЪая╕П  Thinking block signature invalid, "
                ...
                # тФАтФА Invalid encrypted reasoning replay recovery тФАтФАтФАтФАтФАтФАтФА
                ...
                # One-shot тАФ if a second 400 fires we fall through to the
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}тЪая╕П  Encrypted reasoning replay was rejected by the provider тАФ "
                ...
                # тФАтФА llama.cpp grammar-parse recovery тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                ...
                if (
                    ...
                    except Exception as _strip_exc:  # pragma: no cover тАФ defensive
                        logger.warning(
                    ...
                    if _stripped:
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  llama.cpp rejected tool schema grammar тАФ "
                        ...
                        continue
                    # No keywords found to strip тАФ fall through to normal
                    # retry path rather than loop forever on the same error.
                    logger.warning(
                        "%sllama.cpp grammar error but no pattern/format "
                        "keywords to strip тАФ falling through to normal retry",
                        agent.log_prefix,
                    )
                ...
                agent._buffer_vprint(f"тЪая╕П  API call failed (attempt {retry_count}/{max_retries}): {error_type}{_status_code_str}")
                agent._buffer_vprint(f"   ЁЯФМ Provider: {_provider}  Model: {_model}")
                agent._buffer_vprint(f"   ЁЯМР Endpoint: {_base}")
                agent._buffer_vprint(f"   ЁЯУЭ Error: {_error_summary}")
                if status_code and status_code < 500:
                    _err_body = getattr(api_error, "body", None)
                    _err_body_str = str(_err_body)[:300] if _err_body else None
                    if _err_body_str:
                        agent._buffer_vprint(f"   ЁЯУЛ Details: {_err_body_str}")
                agent._buffer_vprint(f"   тП▒я╕П  Elapsed: {elapsed_time:.2f}s  Context: {len(api_messages)} msgs, ~{approx_tokens:,} tokens")

                # Actionable hint for OpenRouter "no tool endpoints" error.
                # Buffered like the rest of the retry trace тАФ surfaced only
                ...
                if (
                    ...
                    agent._buffer_vprint(
                        f"   ЁЯТб No OpenRouter providers for {_model} support tool calling with your current settings."
                    )
                ...
                if agent._interrupt_requested:
                    agent._vprint(f"{agent.log_prefix}тЪб Interrupt detected during error handling, aborting retries.", force=True)
                    agent._persist_session(messages, conversation_history)
                    agent.clear_interrupt()
                    return {
                ...
                # A 413 is a payload-size error тАФ the correct response is to
                # compress history and retry, not abort immediately.
                status_code = getattr(api_error, "status_code", None)

                # тФАтФА Respect disabled auto-compaction on overflow тФАтФАтФАтФАтФАтФА
                # Ported from anomalyco/opencode#30749.  When the user has
                # turned auto-compaction off (``compression.enabled: false``),
                # NO automatic compaction trigger may fire тАФ including the
                ...
                # (``force=True``) is unaffected тАФ it never reaches this loop.
                _overflow_reasons = {
                ...
                if (
                    ...
                    agent._flush_status_buffer()
                    agent._vprint(
                        f"{agent.log_prefix}тЭМ Context overflow, but auto-compaction is disabled "
                        f"(compression.enabled: false).",
                        force=True,
                    )
                    agent._vprint(
                        f"{agent.log_prefix}   ЁЯТб Run /compress to compact manually, /new to start fresh, "
                        f"switch to a larger-context model, or reduce attachments.",
                        force=True,
                    )
                    logger.error(
                        f"{agent.log_prefix}Context overflow ({classified.reason.value}) with "
                        f"auto-compaction disabled тАФ not compressing."
                    )
                    agent._persist_session(messages, conversation_history)
                    return {
                        "messages": messages,
                ...
                # тФАтФА Anthropic Sonnet long-context tier gate тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                # Anthropic returns HTTP 429 "Extra usage is required for
                # long context requests" when a Claude Max (or similar)
                # subscription doesn't include the 1M-context tier.  This
                # is NOT a transient rate limit тАФ retrying or switching
                ...
                if classified.reason == FailoverReason.long_context_tier:
                    ...
                    if old_ctx > _reduced_ctx:
                        ...
                        # Context probing flags тАФ only set on built-in
                        # compressor (plugin engines manage their own).
                        if hasattr(compressor, "_context_probed"):
                            compressor._context_probed = True
                            # Don't persist тАФ this is a subscription-tier
                        ...
                        agent._buffer_vprint(
                            f"тЪая╕П  Anthropic long-context tier "
                            f"requires extra usage тАФ reducing context: "
                            f"{old_ctx:,} тЖТ {_reduced_ctx:,} tokens"
                        )
                    ...
                    if compression_attempts <= max_compression_attempts:
                        ...
                        # Compression created a new session тАФ clear history
                        # so _flush_messages_to_session_db writes compressed
                        # messages to the new session, not skipping them.
                        conversation_history = None
                        if len(messages) < original_len or old_ctx > _reduced_ctx:
                            agent._buffer_status(
                                f"ЁЯЧЬя╕П Context reduced to {_reduced_ctx:,} tokens "
                ...
                if is_rate_limited and agent._fallback_index < len(agent._fallback_chain):
                    ...
                    if not pool_may_recover:
                        if classified.reason == FailoverReason.billing:
                            agent._buffer_status(
                                "тЪая╕П Billing or credits exhausted тАФ switching to fallback provider..."
                            )
                        else:
                            agent._buffer_status("тЪая╕П Rate limited тАФ switching to fallback provider...")
                ...
                # тФАтФА Nous Portal: record rate limit & skip retries тФАтФАтФАтФАтФА
                ...
                # Compression can't help тАФ the floor is the system prompt
                # itself, not the conversation тАФ so surface a clear "not
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}   ЁЯТб GitHub Models free tier (models.inference.ai.azure.com) caps every",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      setup` тЖТ GitHub Copilot), or pick any other provider.",
                        force=True,
                    )
                ...
                if is_payload_too_large:
                    compression_attempts += 1
                    if compression_attempts > max_compression_attempts:
                        # Terminal тАФ surface the buffered retry trace.
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}тЭМ Max compression attempts ({max_compression_attempts}) reached for payload-too-large error.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ЁЯТб Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                    ...
                    agent._buffer_status(f"тЪая╕П  Request payload too large (413) тАФ compression attempt {compression_attempts}/{max_compression_attempts}...")
                    ...
                    if len(messages) < original_len:
                        agent._buffer_status(f"ЁЯЧЬя╕П Compressed {original_len} тЖТ {len(messages)} messages, retrying...")
                        time.sleep(2)  # Brief pause between compression retries
                        _retry.restart_with_compressed_messages = True
                        break
                    else:
                        # Terminal тАФ surface buffered context so the user
                        # sees what compression attempts were made.
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}тЭМ Payload too large and cannot compress further.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ЁЯТб Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                ...
                if is_context_length_error:
                    ...
                    # тФАтФА Distinguish two very different errors тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    ...
                    #    Do NOT shrink context_length тАФ the window is unchanged.
                    #
                    ...
                    if available_out is not None:
                        ...
                        agent._buffer_vprint(
                            f"тЪая╕П  Output cap too large for current prompt тАФ "
                        ...
                        if compression_attempts > max_compression_attempts:
                            agent._flush_status_buffer()
                            agent._vprint(f"{agent.log_prefix}тЭМ Max compression attempts ({max_compression_attempts}) reached.", force=True)
                            agent._vprint(f"{agent.log_prefix}   ЁЯТб Try /new to start a fresh conversation, or /compress to retry compression.", force=True)
                    ...
                    if new_ctx is not None:
                        ...
                        agent._buffer_vprint(f"тЪая╕П  Context length exceeded тАФ using provider limit: {old_ctx:,} тЖТ {new_ctx:,} tokens")
                        ...
                        agent._buffer_vprint(
                            f"тЪая╕П  Context length exceeded, but provider did not report a max context length; "
                            f"keeping context_length at {old_ctx:,} tokens and compressing."
                        )
                    ...
                    agent._buffer_status(f"ЁЯЧЬя╕П Context too large (~{approx_tokens:,} tokens) тАФ compressing ({compression_attempts}/{max_compression_attempts})...")
                    ...
                    if len(messages) < original_len or new_ctx and new_ctx < old_ctx:
                        ...
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}тЭМ Context length exceeded and cannot compress further.", force=True)
                        agent._vprint(f"{agent.log_prefix}   ЁЯТб The conversation has accumulated too much content. Try /new to start fresh, or /compress to manually trigger compression.", force=True)
                ...
                # Exclude UnicodeEncodeError тАФ it's a ValueError subclass
                # but is handled separately by the surrogate sanitization
                # path above.  Exclude json.JSONDecodeError тАФ also a
                ...
                is_local_validation_error = (
                    ...
                    # backend response.completed.output=null) тАФ not local
                ...
                #   тАв credential-pool rotation (line ~2031) has already
                #     fired for billing and either ``continue``d or
                #     returned (False, ...) тАФ pool is exhausted or absent.
                #   тАв the eager-fallback branch above (line ~2422) also
                ...
                # balance with no recovery mechanism left тАФ see #31273
                ...
                if is_client_error:
                    # Try fallback before aborting тАФ a different provider may
                    ...
                    if agent._has_pending_fallback():
                        if classified.reason == FailoverReason.content_policy_blocked:
                            agent._buffer_status("тЪая╕П Provider safety filter blocked this request тАФ trying fallback...")
                        else:
                            agent._buffer_status(f"тЪая╕П Non-retryable error (HTTP {status_code}) тАФ trying fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                    ...
                    # Terminal тАФ flush buffered context so the user sees
                    # what was tried before the abort.
                    agent._flush_status_buffer()
                    if classified.reason == FailoverReason.content_policy_blocked:
                        agent._emit_status(
                            f"тЭМ Provider safety filter blocked this request: "
                            f"{agent._summarize_api_error(api_error)}"
                        )
                    else:
                        agent._emit_status(
                            f"тЭМ Non-retryable error (HTTP {status_code}): "
                            f"{agent._summarize_api_error(api_error)}"
                        )
                    agent._vprint(f"{agent.log_prefix}тЭМ Non-retryable client error (HTTP {status_code}). Aborting.", force=True)
                    agent._vprint(f"{agent.log_prefix}   ЁЯФМ Provider: {_provider}  Model: {_model}", force=True)
                    agent._vprint(f"{agent.log_prefix}   ЁЯМР Endpoint: {_base}", force=True)
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   ЁЯТб Codex OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (Codex CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `hermes auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   ЁЯТб xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `hermes model`.", force=True)
                            else:  # nous
                                agent._vprint(f"{agent.log_prefix}   ЁЯТб Nous Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    agent._vprint(f"{agent.log_prefix}      тЪая╕П  Note: `{_model}` looks like an OpenRouter slug (`:free` suffix).", force=True)
                            ...
                            agent._vprint(f"{agent.log_prefix}   ЁЯТб Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      тАв Is the key valid? Run: hermes setup", force=True)
                            agent._vprint(f"{agent.log_prefix}      тАв Does your account have access to {_model}?", force=True)
                            if base_url_host_matches(str(_base), "openrouter.ai"):
                                agent._vprint(f"{agent.log_prefix}      тАв Check credits: https://openrouter.ai/settings/credits", force=True)
                    else:
                        agent._vprint(f"{agent.log_prefix}   ЁЯТб This type of error won't be fixed by retrying.", force=True)
                    # Content-policy blocks deserve their own actionable
                    # guidance тАФ neither "fix your API key" nor "retry won't
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        agent._vprint(
                            f"{agent.log_prefix}   ЁЯТб The provider's safety filter rejected this specific prompt.",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}      тАв Try rephrasing the request, narrowing the context, or splitting into smaller steps.",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}      тАв Configure a fallback provider so future blocks route automatically:",
                            force=True,
                        )
                        agent._vprint(
                            f"{agent.log_prefix}        hermes fallback add   (interactive picker тАФ same as `hermes model`)",
                            force=True,
                        )
                    ...
                    if status_code == 400 and (approx_tokens > 50000 or len(api_messages) > 80):
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  Skipping session persistence "
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        _policy_response = (
                            "тЪая╕П  The model provider's safety filter blocked this request "
                ...
                if retry_count >= max_retries:
                    ...
                    if agent._has_pending_fallback():
                        agent._buffer_status(f"тЪая╕П Max retries ({max_retries}) exhausted тАФ trying fallback...")
                    if agent._try_activate_fallback():
                        retry_count = 0
                        compression_attempts = 0
                        _retry.primary_recovery_attempted = False
                        continue
                    # Terminal тАФ flush buffered retry/fallback trace.
                    agent._flush_status_buffer()
                    _final_summary = agent._summarize_api_error(api_error)
                    _billing_guidance = ""
                    if classified.reason == FailoverReason.billing:
                        agent._emit_status(f"тЭМ Billing or credits exhausted тАФ {_final_summary}")
                        ...
                        agent._emit_status(f"тЭМ Rate limited after {max_retries} retries тАФ {_final_summary}")
                    else:
                        agent._emit_status(f"тЭМ API failed after {max_retries} retries тАФ {_final_summary}")
                    agent._vprint(f"{agent.log_prefix}   ЁЯТА Final error: {_final_summary}", force=True)
                    ...
                    if _is_stream_drop:
                        agent._vprint(
                            f"{agent.log_prefix}   ЁЯТб The provider's stream "
                    ...
                    if _is_stream_drop:
                        _final_response += (
                            ...
                            "dropping тАФ this often happens when generating "
                ...
                if is_rate_limited:
                    agent._buffer_status(f"тП▒я╕П Rate limited. Waiting {wait_time:.1f}s (attempt {retry_count + 1}/{max_retries})...")
                else:
                    agent._buffer_status(f"тП│ Retrying in {wait_time:.1f}s (attempt {retry_count}/{max_retries})...")
                logger.warning(
        ...
        if _retry.restart_with_length_continuation:
            ...
            # Retry 1 тЖТ 2├Ч base, retry 2 тЖТ 3├Ч base, capped at 32 768.
        ...
        if response is None:
            ...
            print(f"{agent.log_prefix}тЭМ All API retries exhausted with no successful response.")
            agent._persist_session(messages, conversation_history)
            break
        ...
        try:
            ...
            # Normalize content to string тАФ some OpenAI-compatible servers
            ...
            if assistant_message.content is not None and not isinstance(assistant_message.content, str):
                ...
                if isinstance(raw, dict):
                    ...
                    # Multimodal content list тАФ extract text parts
                    parts = []
                    for part in raw:
            ...
            if assistant_message.content and not agent.quiet_mode:
                if agent.verbose_logging:
                    agent._vprint(f"{agent.log_prefix}ЁЯдЦ Assistant: {assistant_message.content}")
                else:
                    agent._vprint(f"{agent.log_prefix}ЁЯдЦ Assistant: {assistant_message.content[:100]}{'...' if len(assistant_message.content) > 100 else ''}")
            ...
            # This means the model ran out of output tokens mid-reasoning тАФ retry up to 2 times
            if has_incomplete_scratchpad(assistant_message.content or ""):
                agent._incomplete_scratchpad_retries += 1
                
                agent._buffer_vprint(f"тЪая╕П  Incomplete <REASONING_SCRATCHPAD> detected (opened but never closed)")
                
                if agent._incomplete_scratchpad_retries <= 2:
                    agent._buffer_vprint(f"ЁЯФД Retrying API call ({agent._incomplete_scratchpad_retries}/2)...")
                    # Don't add the broken message, just retry
                    continue
                else:
                    # Max retries - discard this turn and save as partial
                    agent._flush_status_buffer()
                    agent._vprint(f"{agent.log_prefix}тЭМ Max retries (2) for incomplete scratchpad. Saving as partial.", force=True)
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                if agent._codex_incomplete_retries < 3:
                    if not agent.quiet_mode:
                        agent._vprint(f"{agent.log_prefix}тЖ╗ Codex response incomplete; continuing turn ({agent._codex_incomplete_retries}/3)")
                    agent._session_messages = messages
                    continue
            ...
            if assistant_message.tool_calls:
                if not agent.quiet_mode:
                    agent._vprint(f"{agent.log_prefix}ЁЯФз Processing {len(assistant_message.tool_calls)} tool call(s)...")
                ...
                for tc in assistant_message.tool_calls:
                    if tc.function.name not in agent.valid_tool_names:
                        ...
                        if repaired:
                            print(f"{agent.log_prefix}ЁЯФз Auto-repaired tool name: '{tc.function.name}' -> '{repaired}'")
                ...
                if invalid_tool_calls:
                    ...
                    # Return helpful error to model тАФ model can agent-correct next turn
                    available = ", ".join(sorted(agent.valid_tool_names))
                    invalid_name = invalid_tool_calls[0]
                    invalid_preview = invalid_name[:80] + "..." if len(invalid_name) > 80 else invalid_name
                    agent._buffer_vprint(f"тЪая╕П  Unknown tool '{invalid_preview}' тАФ sending error to model for agent-correction ({agent._invalid_tool_retries}/3)")

                    if agent._invalid_tool_retries >= 3:
                        agent._flush_status_buffer()
                        agent._vprint(f"{agent.log_prefix}тЭМ Max retries (3) for invalid tool calls exceeded. Stopping as partial.", force=True)
                ...
                if invalid_json_args:
                    ...
                    if _truncated:
                        agent._vprint(
                            f"{agent.log_prefix}тЪая╕П  Truncated tool call arguments detected "
                            f"(finish_reason={finish_reason!r}) тАФ refusing to execute.",
                            force=True,
                        )
                    ...
                    agent._buffer_vprint(f"тЪая╕П  Invalid JSON in tool call arguments for '{tool_name}': {error_msg}")

                    if agent._invalid_json_retries < 3:
                        agent._buffer_vprint(f"ЁЯФД Retrying API call ({agent._invalid_json_retries}/3)...")
                        # Don't add anything to messages, just retry the API call
                        continue
                    else:
                        # Instead of returning partial, inject tool error results so the model can recover.
                        # Using tool results (not user messages) preserves role alternation.
                        agent._buffer_vprint(f"тЪая╕П  Injecting recovery tool results for invalid JSON...")
                ...
                # тФАтФА Post-call guardrails тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                ...
                if turn_content and agent._has_content_after_think_block(turn_content):
                    ...
                    if _all_housekeeping and agent._has_stream_consumers():
                        ...
                        if clean:
                            agent._vprint(f"  тФК ЁЯТм {clean}")
                
                # Pop thinking-only prefill message(s) before appending
                # (tool-call path тАФ same rationale as the final-response path).
                _had_prefill = False
                while (
                    messages
                ...
                # the whole conversation тАФ a model that intermittently
                # empties (empty тЖТ prefill тЖТ tools тЖТ empty тЖТ prefill тЖТ
                ...
                # Successful tool execution тАФ reset the post-tool nudge
                ...
                # Only signal the display callback тАФ TTS (_stream_callback)
                ...
                if agent._tool_guardrail_halt_decision is not None:
                    ...
                    agent._emit_status(
                        f"тЪая╕П Tool guardrail halted {decision.tool_name}: {decision.code}"
                    )
                    messages.append({"role": "assistant", "content": final_response})
                    # Emit the halt message to the client so it's not
                    # indistinguishable from a crash.  The stream display
                    # was flushed (callback(None)) before tool execution,
                    # but the callback is still alive тАФ fire the text
                ...
                # assistant turn тАФ a tight lower bound for the next prompt.
                ...
                if _compressor.last_prompt_tokens > 0:
                    # Only use prompt_tokens тАФ completion/reasoning
                    ...
                    # Include tool schemas тАФ with 50+ tools enabled
                ...
                if agent.compression_enabled and _compressor.should_compress(_real_tokens):
                    agent._safe_print("  тЯ│ compacting contextтАж")
                    messages, active_system_prompt = agent._compress_context(
                        messages, system_message,
                        approx_tokens=agent.context_compressor.last_prompt_tokens,
                        task_id=effective_task_id,
                    )
                    # Compression created a new session тАФ clear history so
                ...
                if not agent._has_content_after_think_block(final_response):
                    # тФАтФА Partial stream recovery тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    ...
                    if agent._has_content_after_think_block(_partial_streamed):
                        ...
                        logger.info(
                            ...
                            "тАФ using as final response",
                            len(_recovered),
                        )
                        agent._emit_status(
                            "тЖ╗ Stream interrupted тАФ using delivered content "
                            "as final response"
                        )
                    ...
                    # the empty follow-up means the model choked тАФ let the
                    ...
                    if fallback and getattr(agent, '_last_content_tools_all_housekeeping', False):
                        ...
                        logger.info("Empty follow-up after tool calls тАФ using prior turn content as final response")
                        agent._emit_status("тЖ╗ Empty response after tool calls тАФ using earlier content as final answer")
                        agent._last_content_with_tools = None
                        agent._last_content_tools_all_housekeeping = False
                        agent._empty_content_retries = 0
                        # Do NOT modify the assistant message content тАФ the
                    ...
                    # тФАтФА Post-tool-call empty response nudge тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    # The model returned empty after executing tool calls.
                    # This covers two cases:
                    #  (a) No prior-turn content at all тАФ model went silent
                    ...
                    if (
                        ...
                        and not _has_inline_thinking  # thinking model still working тАФ let prefill handle
                    ):
                        ...
                        agent._last_content_tools_all_housekeeping = False
                        logger.info(
                            "Empty response after tool calls тАФ nudging model "
                            "to continue processing"
                        )
                        agent._buffer_status(
                            "тЪая╕П Model returned empty after tool calls тАФ "
                            "nudging to continue"
                        )
                        # Append the empty assistant message first so the
                        # message sequence stays valid:
                        #   tool(result) тЖТ assistant("(empty)") тЖТ user(nudge)
                        # Without this, we'd have tool тЖТ user which most
                    ...
                    # тФАтФА Thinking-only prefill continuation тФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    ...
                    # continue тАФ the model will see its own reasoning
                    ...
                    if _has_structured and agent._thinking_prefill_retries < 2:
                        ...
                        logger.info(
                            "Thinking-only response (no visible content) тАФ "
                            "prefilling to continue (%d/2)",
                            agent._thinking_prefill_retries,
                        )
                        agent._buffer_status(
                            f"тЖ╗ Thinking-only response тАФ prefilling to continue "
                    ...
                    # тФАтФА Empty response retry тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
                    ...
                    # prefill exhaustion тАФ models like mimo-v2-pro
                    ...
                    if _truly_empty and (not _has_structured or _prefill_exhausted) and agent._empty_content_retries < 3:
                        ...
                        logger.warning(
                            "Empty response (no content or reasoning) тАФ "
                            "retry %d/3 (model=%s)",
                            agent._empty_content_retries, agent.model,
                        )
                        agent._buffer_status(
                            f"тЪая╕П Empty response from model тАФ retrying "
                            f"({agent._empty_content_retries}/3)"
                        )
                        continue

                    # тФАтФА Exhausted retries тАФ try fallback provider тФАтФА
                    ...
                    if _truly_empty and agent._fallback_chain:
                        logger.warning(
                            "Empty response after %d retries тАФ "
                            "attempting fallback (model=%s, provider=%s)",
                            agent._empty_content_retries, agent.model,
                            agent.provider,
                        )
                        agent._buffer_status(
                            "тЪая╕П Model returning empty responses тАФ "
                            "switching to fallback provider..."
                        )
                        if agent._try_activate_fallback():
                            agent._empty_content_retries = 0
                            agent._buffer_status(
                                f"тЖ╗ Switched to fallback: {agent.model} "
                                f"({agent.provider})"
                            )
                            logger.info(
                    ...
                    if reasoning_text:
                        ...
                        agent._emit_status(
                            "тЪая╕П Model produced reasoning but no visible "
                        ...
                        agent._emit_status(
                            "тЭМ Model returned no content after all retries"
                ...
                agent._empty_content_retries = 0
                agent._thinking_prefill_retries = 0
                # Successful content reached тАФ drop any buffered retry
                ...
                if not agent.quiet_mode:
                    agent._safe_print(f"ЁЯОЙ Conversation completed after {api_call_count} OpenAI-compatible API call(s)")
                break
            ...
            # тАФ users would see a one-line summary on screen with no way to
