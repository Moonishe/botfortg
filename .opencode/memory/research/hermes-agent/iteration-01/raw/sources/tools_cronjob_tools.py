...
#      Strict scanning is appropriate тАФ a legit cron prompt has no business
...
#      every time a skill *describes* a command тАФ see #3968 follow-up: the
...
# Strict patterns тАФ applied to the user prompt only.
...
# Looser pattern set тАФ applied to the assembled prompt when skills are
...
# Unicode emoji sequences (for example ЁЯСитАНЁЯСйтАНЁЯСз, ЁЯП│я╕ПтАНЁЯМИ, тЭдя╕ПтАНЁЯй╣, ЁЯзСтАНЁЯТ╗).
...
def _strip_invisible_unicode(prompt: str) -> tuple[str, list[str]]:
    ...
    skill body is already vetted at install time by ``skills_guard.py`` тАФ a
    ...
    for idx, ch in enumerate(prompt):
        if ch in _CRON_INVISIBLE_CHARS:
            if ch == '\u200d' and _zwj_has_emoji_neighbour(prompt, idx):
                cleaned.append(ch)  # legitimate emoji joiner тАФ keep
                continue
...
def _scan_cron_prompt(prompt: str) -> str:
    ...
    Strict pattern set тАФ used at job create/update time and as a runtime
...
def _scan_cron_skill_assembled(assembled: str) -> tuple[str, str]:
    ...
    Looser pattern set тАФ only catches unambiguous prompt-injection
    ...
    ``_scan_cron_prompt`` тАФ that path is the actual injection surface.
...
def _resolve_model_override(model_obj: Optional[Dict[str, Any]]) -> tuple:
    ...
    # Bare "custom" is usually an incomplete spec тАФ the canonical form is
    ...
    # provider below тАФ otherwise pinning to ``model.provider`` (e.g. codex)
...
def _normalize_deliver_param(value: Any) -> Optional[str]:
    ...
    Some callers тАФ MCP clients passing arrays, scripts building the payload as a
    list тАФ supply ``["telegram"]``.  ``create_job``/``update_job`` store it as-is,
...
def cronjob(
    ...
    try:
        ...
        if normalized == "create":
            ...
            #   - no_agent=True тЖТ script is the job; prompt/skills are optional
            #     (and irrelevant to execution).
            #   - no_agent=False (default) тЖТ at least one of prompt/skills must
            #     be set, same as before.
            if _no_agent:
                if not script:
                    return tool_error(
                        "create with no_agent=True requires a script тАФ "
        ...
        if normalized == "update":
            ...
            if workdir is not None:
                ...
                # otherwise pass raw тАФ update_job() validates / normalizes.
                updates["workdir"] = _normalize_optional_job_value(workdir) or None
            if no_agent is not None:
                # Toggling no_agent on/off at update time. If flipping to True,
                # we need a script to already exist on the job (or be part of
                # the same update) тАФ otherwise the next tick would error out.
...
To stop a job the user no longer wants: first action='list' to find the job_id, then action='remove' with that job_id. Never guess job IDs тАФ always list first.
...
present тАФ they cannot ask questions or request clarification.
        ...
        "properties": {
            ...
            "model": {
                ...
                "properties": {
                    "provider": {
                        "type": "string",
                        "description": "Provider name (e.g. 'openrouter', 'anthropic', or 'custom:<name>' for a provider defined in custom_providers config тАФ always include the ':<name>' suffix, never pass the bare 'custom'). Omit to use and pin the current provider."
                    },
                    "model": {
                        "type": "string",
            ...
            "no_agent": {
                ...
                "default": False,
                "description": (
                    "Default: False (LLM-driven job тАФ the agent runs the prompt each tick). "
                    ...
                    "(b) EMPTY stdout means SILENT тАФ nothing is sent to the user and they won't see anything happened, so design your script to stay quiet when there's nothing to report (the watchdog pattern); "
                    ...
                    "WHEN TO USE False (default): anything that needs reasoning тАФ summarize a feed, draft a daily briefing, pick interesting items, rephrase data for a human, follow conditional logic based on content."
                ),
            },
            "context_from": {
                "type": "array",
                ...
                "description": (
                    ...
                    "Note: injects the most recent completed output тАФ does not wait for "
            ...
            "enabled_toolsets": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Optional list of toolset names to restrict the job's agent to (e.g. [\"web\", \"terminal\", \"file\", \"delegation\"]). When set, only tools from these toolsets are loaded, significantly reducing input token overhead. When omitted, all default tools are loaded. Infer from the job's prompt тАФ e.g. use \"web\" if it calls web_search, \"terminal\" if it runs scripts, \"file\" if it reads files, \"delegation\" if it calls delegate_task. On update, pass an empty array to clear."
            },
            "workdir": {
                "type": "string",
                "description": "Optional absolute path to run the job from. When set, AGENTS.md / CLAUDE.md / .cursorrules from that directory are injected into the system prompt, and the terminal/file/code_exec tools use it as their working directory тАФ useful for running a job inside a specific project repo. Must be an absolute path that exists. When unset (default), preserves the original behaviour: no project context files, tools use the scheduler's cwd. On update, pass an empty string to clear. Jobs with workdir run sequentially (not parallel) to keep per-job directories isolated."
            },
        },
        "required": ["action"]
    }
...
def check_cronjob_requirements() -> bool:
    ...
    ``yes``, ``on``) тАФ false-like values (``0``, ``false``, ``no``, ``off``)
