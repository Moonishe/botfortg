"""MemoryManager тАФ orchestrates memory providers for the agent.

Single integration point in run_agent.py. Replaces scattered per-backend
code with one manager that delegates to registered providers.

Only ONE external plugin provider is allowed at a time тАФ attempting to
...
# teardown indefinitely тАФ the worker threads are daemon, so anything still
        ...
        while buf:
            if self._in_span:
                ...
                if idx == -1:
                    ...
                    return "".join(out)
                # Found close тАФ skip span content + tag, continue
                buf = buf[idx + len(self._CLOSE_TAG):]
                self._in_span = False
            else:
                idx = self._find_boundary_open_tag(buf)
                if idx == -1:
                    # No open tag тАФ hold back a potential partial open tag
                    held = (
...
def build_memory_context_block(raw_context: str) -> str:
    ...
    return (
        ...
        "NOT new user input. Treat as authoritative reference data тАФ "
...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        Only **one** external (non-builtin) provider is allowed тАФ a second
        ...
        if not is_builtin:
            if self._has_external:
                ...
                logger.warning(
                    "Rejected memory provider '%s' тАФ external provider '%s' is "
        ...
        # Core tool names are reserved тАФ a memory provider must never register
        ...
        # table at all тАФ matching the built-ins-always-win invariant used by
        # the TTS/browser/search provider registries.
        from toolsets import _HERMES_CORE_TOOLS

        _core_tool_names = set(_HERMES_CORE_TOOLS)

        # Index tool names тЖТ provider for routing
        for schema in provider.get_tool_schemas():
            tool_name = schema.get("name", "")
            if tool_name in _core_tool_names:
                logger.warning(
                    "Memory provider '%s' tool '%s' shadows a reserved core "
                    "tool name; registration ignored. Core tools always win тАФ "
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        just the user's instruction here, once, for every provider тАФ so this
        ...
        - Bare skill invocations (no instruction) return None тЖТ callers skip
    ...
    def sync_all(
        ...
        *,
        ...
        broken provider can never stall the turn тАФ the sync simply
    ...
    def _submit_background(self, fn) -> None:
        ...
        runs inline as a last-resort fallback тАФ losing the async benefit
        ...
        if executor is None:
            # Executor unavailable (shut down / creation failed) тАФ run
    ...
    def flush_pending(self, timeout: Optional[float] = None) -> bool:
        ...
        try:
            ...
            # Executor already shut down тАФ nothing pending.
            return True
        try:
    ...
    def get_all_tool_schemas(self) -> List[Dict[str, Any]]:
        ...
        skipped тАФ they are rejected from the routing table in
    ...
    def on_session_switch(
        ...
        **kwargs,
        ...
        context compression тАФ any path that reassigns
    ...
    def _drain_sync_executor(self) -> None:
        ...
        try:
            ...
            # do NOT block here тАФ cancel_futures cancels not-yet-started
