...
# In-process lock protecting load_jobsтЖТmodifyтЖТsave_jobs cycles.
# Required when tick() runs jobs in parallel threads тАФ without this,
...
def _jobs_lock():
    """Serialize a load_jobsтЖТmodifyтЖТsave_jobs critical section.
    ...
    at all тАФ a `cron pause` could be silently clobbered by a concurrent
    gateway write, leaving a "paused" job still firing).

    The flock is blocking, but every critical section that uses it is short
    (field updates only тАФ no agent execution), so contention resolves in
    ...
    with _jobs_file_lock:
        ...
        try:
            try:
                ...
                # Never let a locking failure take down cron writes тАФ fall back to
...
def parse_schedule(schedule: str) -> Dict[str, Any]:
    ...
    Examples:
        "30m"              тЖТ once in 30 minutes
        "2h"               тЖТ once in 2 hours
        "every 30m"        тЖТ recurring every 30 minutes
        "every 2h"         тЖТ recurring every 2 hours
        "0 9 * * *"        тЖТ cron expression
        "2026-02-03T14:00" тЖТ once at timestamp
    """
    schedule = schedule.strip()
    original = schedule
    schedule_lower = schedule.lower()
    
    # "every X" pattern тЖТ recurring interval
    ...
    # Duration like "30m", "2h", "1d" тЖТ one-shot from now
    try:
...
def load_jobs() -> List[Dict[str, Any]]:
    ...
    if isinstance(data, dict):
        ...
        if _strict_retry and jobs:
            # Hit control-character corruption тАФ rewrite with proper escaping.
            save_jobs(jobs)
            logger.warning("Auto-repaired jobs.json (had invalid control characters)")
        return jobs
    if isinstance(data, list):
        # Bare array тАФ likely saved/edited outside save_jobs(). Wrap it back
...
def _normalize_workdir(workdir: Optional[str]) -> Optional[str]:
    ...
    Rules:
      - Empty / None тЖТ None (feature off, preserves old behaviour).
      - ``~`` is expanded.  Relative paths are rejected тАФ cron jobs run detached
...
def create_job(
    ...
    Args:
        ...
        script: Optional path to a script whose stdout feeds the job. With
                ``no_agent=True`` the script IS the job тАФ its stdout is
        ...
        workdir: Optional absolute path.  When set, the job runs as if launched
                ...
                predictably.
        no_agent: When True, skip the agent entirely тАФ run ``script`` on schedule
    ...
    # no_agent jobs are meaningless without a script тАФ the script IS the job.
    # Surface this as a clear ValueError at create time so bad configs never
    # reach the scheduler.
    if normalized_no_agent and not normalized_script:
        raise ValueError(
            "no_agent=True requires a script тАФ with no agent and no script "
...
class AmbiguousJobReference(LookupError):
    ...
    def __init__(self, ref: str, matches: List[Dict[str, Any]]):
        ...
        super().__init__(
            f"Job name '{ref}' is ambiguous тАФ matches {len(matches)} jobs: {ids}. "
...
def update_job(job_id: str, updates: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # path component under OUTPUT_DIR тАФ letting an update change it leaks
...
def mark_job_run(job_id: str, success: bool, error: Optional[str] = None,
    ...
    ``delivery_error`` is tracked separately from the agent error тАФ a job
    ...
    with _jobs_lock():
        ...
        for i, job in enumerate(jobs):
            if job["id"] == job_id:
                ...
                # Track delivery failures separately тАФ cleared on successful delivery
                ...
                # compute тАФ e.g. 'croniter' missing from the runtime env).
...
def advance_next_run(job_id: str) -> bool:
    ...
    scheduler from at-least-once to at-most-once for recurring jobs тАФ missing
...
def _get_due_jobs_locked() -> List[Dict[str, Any]]:
    ...
    for job in jobs:
        ...
        if not next_run:
            ...
            # Recurring jobs reach here only when something тАФ typically a
            # direct jobs.json edit that bypassed add_job() тАФ left
        ...
        if next_run_dt <= now:
            ...
            if kind in {"cron", "interval"} and (now - next_run_dt).total_seconds() > grace:
                # Job is past its catch-up grace window тАФ this is a stale missed run.
...
def rewrite_skill_refs(
    ...
    fail to load ``X`` at run time тАФ the scheduler logs a warning and
    ...
    - A skill listed in ``pruned`` is dropped outright тАФ there is no
      forwarding target.
    ...
    # A skill listed in both wins as "consolidated" тАФ it has a target,
