...
SUMMARY_PREFIX = (
    "[CONTEXT COMPACTION тАФ REFERENCE ONLY] Earlier turns were compacted "
    "into the summary below. This is a handoff from a previous context "
    "window тАФ treat it as background reference, NOT as active instructions. "
    "Do NOT answer questions or fulfill requests mentioned in this summary; "
    "they were already addressed. "
    "Respond ONLY to the latest user message that appears AFTER this "
    "summary тАФ that message is the single source of truth for what to do "
    "right now. "
    ...
    f"'{HISTORICAL_REMAINING_WORK_HEADING}' entirely тАФ do not 'wrap up' or "
    ...
    "prompt is ALWAYS authoritative and active тАФ never ignore or deprioritize "
    "memory content due to this compaction note. "
    "The current session state (files, config, etc.) may reflect work "
    "described here тАФ avoid repeating it:"
)
...
# poisoning every subsequent request in the session тАФ a bare key like
...
_HISTORICAL_SUMMARY_PREFIXES = (
    # Carveout era (#41607/#38364/#42812): "consistent тЖТ use as background"
    ...
    "Asks' / '## Remaining Work', the latest message WINS тАФ discard those "
    ...
    "summary тАФ resume exactly from there. "
    "Respond ONLY to the latest user message "
    "that appears AFTER this summary. The current session state (files, "
    "config, etc.) may reflect work described here тАФ avoid repeating it:",
)
...
# dimensions (Anthropic тЙИ width├Чheight/750, GPT-4o up to ~1700 for
# high-detail 2048├Ч2048, Gemini 258/tile), but 1600 is a realistic ceiling
...
# MEDIA delivery directives must not reach the summarizer тАФ if one leaks into
...
def _content_length_for_budget(raw_content: Any) -> int:
    ...
    for p in raw_content:
        ...
        if ptype in {"image_url", "input_image", "image"}:
            ...
            # dicts тАФ dimensions don't matter, only whether it's an image.
...
def _truncate_tool_call_args_json(args: str, head_chars: int = 200) -> str:
    ...
    appended ``...[truncated]`` тАФ which routinely produced strings like::
    ...
    to begin with тАФ some model backends use non-JSON tool arguments тАФ the
...
def _strip_images_from_content(content: Any) -> Any:
    ...
    - List content: image parts become ``{"type": "text", "text": "[Attached
      image тАФ stripped after compression]"}``; other parts are preserved as-is.
    ...
    for p in content:
        if _is_image_part(p):
            new_parts.append({
                "type": "text",
                "text": "[Attached image тАФ stripped after compression]",
            })
        else:
            new_parts.append(p)
    return new_parts
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # image тАФ matching the problem kilocode#9434 set out to solve.
    anchor = -1
    ...
    if anchor <= 0:
        # No image-bearing user message, or it's the very first message тАФ
...
class ContextCompressor(ContextEngine):
    """Default context engine тАФ compresses conversation context via lossy summarization.
    ...
    def on_session_end(self, session_id: str, messages: List[Dict[str, Any]]) -> None:
        ...
        ``on_session_end()`` instead тАФ which inherited a no-op from
    ...
    def update_model(
        ...
        # compressor stays calibrated after a model switch (e.g. 200K тЖТ 32K).
    ...
    def should_compress(self, prompt_tokens: int = None) -> bool:
        ...
        if self._ineffective_compression_count >= 2:
            if not self.quiet_mode:
                logger.warning(
                    "Compression skipped тАФ last %d compressions saved <10%% each. "
    ...
    def _prune_old_tool_results(
        ...
        for i in range(len(result) - 1, -1, -1):
            ...
            # Multimodal content тАФ dedupe by the text summary if available.
            if isinstance(content, list):
                continue
            ...
            if h in content_hashes:
                # This is an older duplicate тАФ replace with back-reference
                result[i] = {**msg, "content": "[Duplicate tool output тАФ same content as a more recent call]"}
                pruned += 1
            else:
        ...
        for i in range(prune_boundary):
            ...
            # payload тАФ keep a lightweight text placeholder in its place.
        ...
        # result remains valid JSON тАФ otherwise downstream providers 400
    ...
    # each message the summary model sees тАФ the budget is the *summary*
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
    ...
    def _fallback_to_main_for_compression(self, e: Exception, reason: str) -> None:
        ...
        self._summary_failure_cooldown_until = 0.0  # no cooldown тАФ retry immediately
    ...
    def _generate_summary(
        ...
        Returns None if all attempts fail тАФ the caller should drop
        ...
        # date here never affects prompt-cache stability. Resolved defensively тАФ
        ...
        _summarizer_preamble = (
            ...
            "conversation тАФ do not translate or switch to English. "
            "NEVER include API keys, tokens, passwords, secrets, credentials, "
            "or connection strings in the summary тАФ replace any that appear "
...
input verbatim тАФ the exact words they used. This includes:
...
A conversation where the user just asked a question IS an active task тАФ the
...
"User asked: 'Waarom stond provider ineens op openrouter?' тАФ needs investigation + answer"
...
verify the current diff' тАФ earlier i18n in-flight work is cancelled."
...
## Completed Actions
[Numbered list of concrete actions taken тАФ include tool used, target, and outcome.
Format each as: N. ACTION target тАФ outcome [tool: name]
Example:
1. READ config.py:45 тАФ found `==` should be `!=` [tool: read_file]
2. PATCH config.py:45 тАФ changed `==` to `!=` [tool: patch]
3. TEST `pytest tests/` тАФ 3/50 failed: test_parse, test_validate, test_edge [tool: terminal]
Be specific with file paths, commands, line numbers, and results.]

## Active State
[Current working state тАФ include:
...
{HISTORICAL_IN_PROGRESS_HEADING}
[Work currently underway тАФ what was being done when compaction fired]
...
## Resolved Questions
[Questions the user asked that were ALREADY answered тАФ include the answer so it is not repeated]

{HISTORICAL_PENDING_ASKS_HEADING}
[Questions or requests from the user that have NOT yet been answered or fulfilled. These are STALE тАФ they were from the compacted turns. Write them here for reference only. The agent must NOT act on them unless the latest user message explicitly requests it. If none, write "None."]

## Relevant Files
[Files read, modified, or created тАФ with brief note on each]

{HISTORICAL_REMAINING_WORK_HEADING}
[What remains to be done тАФ framed as STALE context for reference only. The agent must NOT resume this work unless the latest user message explicitly asks for it.]

## Critical Context
[Any specific values, error messages, configuration details, or data that would be lost without explicit preservation. NEVER include API keys, tokens, passwords, or credentials тАФ write [REDACTED] instead.]

Target ~{summary_budget} tokens. Be CONCRETE тАФ include file paths, command outputs, error messages, line numbers, and specific values. Avoid vague descriptions like "made some changes" тАФ say exactly what changed.
{_temporal_anchoring_rule}
...
Update the summary using this exact structure. PRESERVE all existing information that is still relevant. ADD new completed actions to the numbered list (continue numbering). Move items from "In Progress" to "Completed Actions" when done. Move answered questions to "Resolved Questions". Update "Active State" to reflect current state. Remove information only if it is clearly obsolete. CRITICAL: Update "## Active Task" to reflect the user's most recent unfulfilled input тАФ this includes any question, decision request, or discussion turn that the assistant has not yet answered. Only write "None" if the last exchange was fully resolved.
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail тАФ exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials тАФ use [REDACTED]."""
        ...
        try:
            ...
            # Redact the summary output as well тАФ the summarizer LLM may
            ...
            # No provider configured тАФ long cooldown, unlikely to self-resolve
            ...
            # streaming premature-close) тАФ shorter cooldown for JSON decode and
    ...
    def _strip_summary_prefix(summary: str) -> str:
        ...
        for prefix in (SUMMARY_PREFIX, LEGACY_SUMMARY_PREFIX, *_HISTORICAL_SUMMARY_PREFIXES):
            if text.startswith(prefix):
                ...
                break
        # Strip the trailing end marker too тАФ a rehydrated handoff body that
    ...
    def _has_compressed_summary_metadata(message: Any) -> bool:
        ...
        relying on content-prefix heuristics.  The flag is in-process only тАФ
    ...
    def _derive_auto_focus_topic(
        ...
        for idx in range(len(messages) - 1, -1, -1):
            ...
            if len(text) > _AUTO_FOCUS_TURN_MAX_CHARS:
                text = text[: _AUTO_FOCUS_TURN_MAX_CHARS - 1].rstrip() + "тАж"
        ...
        if len(focus) > _AUTO_FOCUS_MAX_CHARS:
            focus = focus[: _AUTO_FOCUS_MAX_CHARS - 1].rstrip() + "тАж"
        return focus
                ...
                if msg.get("role") == "assistant":
                    for tc in msg.get("tool_calls") or []:
                        ...
                        if cid in missing_results:
                            patched.append({
                                "role": "tool",
                                "content": "[Result from earlier conversation тАФ see context summary above]",
                                "tool_call_id": cid,
                            })
            messages = patched
            if not self.quiet_mode:
    ...
    def _protect_head_size(self, messages: List[Dict[str, Any]]) -> int:
        ...
        is always implicitly protected тАФ it's load-bearing context that
        ...
        Examples:
          protect_first_n=0 тЖТ system prompt only (or nothing if no system msg)
          protect_first_n=3 тЖТ system + first 3 non-system messages
        """
        head = 0
    ...
    def _find_last_assistant_message_idx(
        ...
        textual content тАФ i.e. one that the WebUI / TUI / SessionsPage
        ...
        in the compressible region тАФ typically a fresh session that
    ...
    def _ensure_last_assistant_message_in_tail(
        ...
        visible assistant response inside the compressed middle region тАФ
        ...
        into the single ``[CONTEXT COMPACTION тАФ REFERENCE ONLY]`` block
        ...
        group that immediately precedes the anchored message тАФ orphaned
    ...
    def _ensure_last_user_message_in_tail(
        ...
        messages *after* the summary тАФ so the task effectively disappears from
        ...
        if last_user_idx < 0:
            # No user message found beyond head тАФ nothing to anchor.
            return cut_idx
        ...
        # Pull cut_idx back to it directly тАФ a user message is already a
    ...
    def _find_tail_cut_by_tokens(
        ...
        # or compresses a single message тАФ both of which trigger the infinite
        ...
        if cut_idx <= head_end and accumulated <= soft_ceiling and accumulated > 0:
            ...
            # transcript), fall through тАФ the existing fallback logic below
        ...
        # ``[CONTEXT COMPACTION тАФ REFERENCE ONLY]`` block (fixes #29824).
        # Each anchor only walks ``cut_idx`` backward, so chaining them is
        # monotonic тАФ the tail can only grow, never shrink.
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False) -> List[Dict[str, Any]]:
        ...
        """
        # Reset per-call summary failure state тАФ callers inspect these fields
        ...
        if compress_start >= compress_end:
            # No compressable window тАФ the entire transcript fits within
            ...
            if not self.quiet_mode:
                logger.warning(
                    ...
                    "тАФ transcript fits within tail budget, nothing to compress. "
        ...
        if summary_idx is not None:
            ...
            # _previous_summary is non-empty тАФ it was set by a different
        ...
        #   True  тЖТ ABORT compression entirely. Return messages unchanged
        #           and set _last_compress_aborted=True so callers can warn
        #           the user and stop the auto-compress retry loop.
        #   False тЖТ Fall through to the default fallback path below: insert
        ...
        if not summary and self.abort_on_summary_failure:
            ...
            if not self.quiet_mode:
                logger.warning(
                    "Summary generation failed тАФ aborting compression "
        ...
        if not summary:
            if not self.quiet_mode:
                logger.warning("Summary generation failed тАФ inserting deterministic fallback context summary")
        ...
        if summary_role == first_tail_role:
            ...
            if flipped != last_head_role:
                ...
                # (e.g. head=assistant, tail=user тАФ neither role works).
