...
# expanded text is what flows into the agent loop тАФ and into memory providers
...
def extract_user_instruction_from_skill_message(content: Any) -> Optional[str]:
    ...
    Returns:
        ...
        - ``None`` when the content is skill scaffolding with no user
          ...
          memory providers should skip the turn in that case тАФ there is no
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    skill_dir = None
    # Prefer the absolute skill_dir returned by skill_view() тАФ this is
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    try:
        ...
        pass  # Non-critical тАФ skill still loads without config injection
...
def _build_skill_message(
    ...
    # тФАтФА Template substitution and inline-shell expansion тФАтФА
    ...
    # тФАтФА Inject the absolute skill directory so the agent can reference
    #    bundled scripts without an extra skill_view() round-trip. тФАтФА
    if skill_dir:
        parts.append("")
    ...
    # тФАтФА Inject resolved skill config values тФАтФА
    ...
    if supporting and skill_dir:
        try:
            ...
            # Skill is from an external dir тАФ use the skill name instead
...
def reload_skills() -> Dict[str, Any]:
    ...
    тАФ they don't need to be in the system prompt for the model to use them.
    ...
    Returns:
        ...
        ``description:`` field тАФ the same string the system prompt renders
...
def resolve_skill_command_key(command: str) -> Optional[str]:
    ...
    Skills are always stored with hyphens тАФ ``scan_skill_commands`` normalizes
...
def build_skill_invocation_message(
    ...
    try:
        ...
        pass  # Non-critical тАФ skill invocation proceeds regardless
