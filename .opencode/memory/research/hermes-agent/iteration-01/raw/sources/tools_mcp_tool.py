...
# the terminal while prompt_toolkit / Rich is rendering the TUI тАФ which
...
def _get_mcp_stderr_log() -> Any:
    ...
    with _mcp_stderr_log_lock:
        ...
        except Exception as exc:  # pragma: no cover тАФ best-effort fallback
...
# These are WARNING-level тАФ we log but don't block, since false positives
...
def _scan_mcp_description(server_name: str, tool_name: str, description: str) -> List[str]:
    ...
    if findings:
        logger.warning(
            "MCP server '%s' tool '%s': suspicious description content тАФ %s. "
            "Description: %.200s",
...
# ---------------------------------------------------------------------------
# MCP ImageContent block тЖТ Hermes MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    try:
        ...
        # without gateway deps). Fall back to silently dropping тАФ callers
        # get any text blocks that did parse.
        logger.debug("MCP image caching skipped тАФ gateway.platforms.base unavailable")
        return ""
...
def _validate_remote_mcp_url(server_name: str, url: Any) -> str:
    ...
    - Non-http(s) schemes (``file://``, ``ws://``, ``stdio:`` тАФ stdio servers
    ...
    except Exception as exc:  # urlparse is very permissive тАФ belt and braces
        raise InvalidMcpUrlError(
    ...
    # Reject that тАФ we need a real host.
...
def _resolve_client_cert(server_name: str, config: dict):
    ...
    no client certificate is configured:
      ...
      - A ``(cert_path, key_path, password)`` tuple when ``client_cert`` is
        a 3-element list/tuple тАФ the third element is the key passphrase.
    ...
    # Tuple/list form for client_cert тАФ (cert, key) or (cert, key, password).
...
class MCPServerTask:
    ...
    def _advertises_tools(self) -> bool:
        ...
        them raises ``McpError(-32601 Method not found)`` тАФ which previously
    ...
    async def _refresh_tools(self):
        ...
        тАФ atomic from the event loop's perspective.
        """
        from tools.registry import registry

        if not self._advertises_tools():
            # A server that doesn't implement tools/* should never send
            # tools/list_changed, but guard anyway тАФ calling tools/list
        ...
        async with self._refresh_lock:
            ...
            if changes:
                logger.warning(
                    "MCP server '%s': tools changed dynamically тАФ %s. "
    ...
    async def _wait_for_lifecycle_event(self) -> str:
        ...
        try:
            while True:
                ...
                # Timeout тАФ no lifecycle event fired.  Send a keepalive
                ...
                # ``ping`` request for them instead тАФ otherwise every
    ...
    async def _run_stdio(self, config: dict):
        ...
        try:
            async with stdio_client(server_params, errlog=_errlog) as (
                ...
                if new_pids:
                    # Capture pgid while the child is alive тАФ once it exits we
                ...
                with _lock:
                    ...
                    for pid in new_pids:
                        ...
                        if not pid_alive and pgid is not None and _killpg is not None:
                            ...
                            # signal 0 тАФ succeeds iff any pgroup member is alive.
                            try:
                                _killpg(pgid, 0)
                                pgroup_alive = True
                        ...
                        if pid_alive or pgroup_alive:
                            ...
                            # Nothing left to reap тАФ drop the pgid entry so
    ...
    async def _preflight_content_type(
        ...
        *,
        ...
        here catches that in тЙд ``timeout`` seconds and raises
        ...
        through silently тАФ the probe is strictly best-effort, and the real
        ...
        try:
            ...
            return  # No httpx тЖТ skip probe; SDK import would have failed first.
        ...
        try:
            ...
            return  # DNS/connect/timeout/transport error тАФ let the SDK try.
        ...
        if not ct_base:
            return  # No content type advertised тАФ don't second-guess the SDK.
        ...
        raise NonMcpEndpointError(
            ...
            "points at a web page rather than an MCP endpoint тАФ check it "
    ...
    async def _run_http(self, config: dict):
        ...
        if config.get("transport") == "sse":
            ...
            if _oauth_auth is not None:
                ...
                # forwarded тАФ SSE OAuth would silently fail with 401s.
            ...
            async with sse_client(**_sse_kwargs) as (read_stream, write_stream):
                async with ClientSession(
                    ...
                    if reason == "reconnect":
                        logger.info(
                            "MCP server '%s': reconnect requested тАФ "
        ...
        if _MCP_NEW_HTTP:
            ...
            # Caller owns the client lifecycle тАФ the SDK skips cleanup when
    ...
    async def _discover_tools(self):
        ...
        which previously aborted the connection тАФ those servers could never
        ...
        if not self._advertises_tools():
            logger.info(
                "MCP server '%s': does not advertise 'tools' capability тАФ "
    ...
    async def run(self, config: dict):
        ...
        # means a typo in config.yaml fails fast with a clear error тАФ and
        ...
        if self._is_http():
            ...
            # before surfacing an opaque CancelledError. Probing here тАФ once,
            # outside the SDK task group тАФ fails fast and non-retryably with
        ...
        while True:
            try:
                ...
                #    touch the retry counters тАФ this is not a failure.
                if self._shutdown_event.is_set():
                    break
                logger.info(
                ...
                # still detect a transient in-flight state тАФ it'll be
                # re-set after the fresh session initializes.
                continue
            except asyncio.CancelledError:
                # Task was cancelled (shutdown, gateway restart, explicit
                # task.cancel()). Don't treat this as a connection failure тАФ
...
#
# State machine:
#   closed    тАФ error count below threshold; all calls go through.
#   open      тАФ threshold reached; calls short-circuit until the
#               cooldown elapses.
#   half-open тАФ cooldown elapsed; the next call is a probe that
#               actually hits the session. Probe success тЖТ closed.
#               Probe failure тЖТ reopens (cooldown re-armed).
#
...
# this state тАФ they keep the count and timestamp in sync.
...
def _get_auth_error_types() -> tuple:
    ...
    Cached after first call. Includes:
      - ``mcp.client.auth.OAuthFlowError`` / ``OAuthTokenError`` тАФ raised by
        the SDK's auth flow when discovery, refresh, or full re-auth fails.
      - ``mcp.client.auth.UnauthorizedError`` (older MCP SDKs) тАФ kept as an
        optional import for forward/backward compatibility.
      - ``tools.mcp_oauth.OAuthNonInteractiveError`` тАФ raised by our callback
        handler when no user is present to complete a browser flow.
      - ``httpx.HTTPStatusError`` тАФ caller must additionally check
...
def _handle_auth_error_and_retry(
    ...
    if recovered:
        ...
        # server is viable again, so close the circuit breaker here тАФ
    ...
    return json.dumps({
        "error": (
            ...
            f"this tool тАФ ask the user to re-authenticate."
        ),
        "needs_reauth": True,
        "server": server_name,
    }, ensure_ascii=False)
...
# was garbage-collected.  The caller's OAuth token is still valid тАФ
...
def _is_session_expired_error(exc: BaseException) -> bool:
    ...
    state while the OAuth token remains valid тАФ idle TTL, server
    ...
    needed is a transport reconnect тАФ tear down and rebuild the
...
def _handle_session_expired_and_retry(
    ...
    the OAuth manager's ``handle_401`` тАФ the access token is still
...
def _mcp_loop_exception_handler(loop, context):
    ...
    if isinstance(exc, RuntimeError) and "Event loop is closed" in str(exc):
        return  # benign shutdown race тАФ suppress
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    ...
    context on the MCP loop and reset when it completes тАФ concurrent calls
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # loop thread, so they copy the loop thread's context тАФ not the
    ...
    # task's own context (task-local тАФ concurrent calls carrying different
...
def _load_mcp_config() -> Dict[str, dict]:
    ...
    try:
        ...
        # with all customizations disabled тАФ no MCP servers connect.
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
            ...
            if age < _CIRCUIT_BREAKER_COOLDOWN_SEC:
                ...
                return json.dumps({
                    "error": (
                        ...
                        f"Do NOT retry this tool yet тАФ use alternative "
                        f"approaches or ask the user to check the MCP server."
                    )
                }, ensure_ascii=False)
            # Cooldown elapsed тЖТ fall through as a half-open probe.
        ...
        async def _call():
            ...
            # the two тАФ plugs into existing infrastructure.
            parts: List[str] = []
            for block in (result.content or []):
        ...
        try:
            ...
            try:
                ...
                if "error" in parsed:
                    ...
                    _reset_server_error(server_name)  # success тАФ reset
            except (json.JSONDecodeError, TypeError):
            ...
            # still valid тАФ only the server-side session is stale.
...
# Source of truth: MCP spec тАФ capabilities.resources / capabilities.prompts
...
def _register_server_tools(name: str, server: MCPServerTask, config: dict) -> List[str]:
    ...
    #   tools.include тАФ whitelist: only these tool names are registered
    #   tools.exclude тАФ blacklist: all tools EXCEPT these are registered
    #   include takes precedence over exclude
    #   Neither set тЖТ register all tools (backward-compatible default)
    tools_filter = config.get("tools") or {}
    ...
    for mcp_tool in server._tools:
        ...
        if existing_toolset and not existing_toolset.startswith("mcp-"):
            logger.warning(
                "MCP server '%s': tool '%s' (тЖТ '%s') collides with built-in "
                "tool in toolset '%s' тАФ skipping to preserve built-in",
...
def get_mcp_status() -> List[dict]:
    ...
    for name, cfg in configured.items():
        ...
        if server and server.session is not None:
            ...
            # A server with enabled: false is intentionally not connected тАФ it is
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``hermes tools`` interactive configuration тАФ connects to each
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    did not terminate the process тАФ common on Linux when stdio children escape
    ...
    With ``include_active=True`` also kills every PID in ``_stdio_pids`` тАФ
    ...
    # dance entirely тАФ otherwise every MCP-free shutdown pays a 2s sleep tax.
    if not pids:
        return
    ...
    def _send_signal(pid: int, sig: int, server_name: str) -> None:
        ...
        if pgid is not None and killpg is not None:
            try:
                ...
                # Pgroup gone (all members exited) or refused тАФ fall back to
    ...
    for pid, server_name in pids.items():
        if not _pid_exists(pid):
            continue  # Good тАФ exited after SIGTERM
...
def _stop_mcp_loop(*, only_if_idle: bool = False) -> bool:
    ...
    if loop is not None:
        ...
        # graceful shutdown are now orphaned тАФ include active PIDs too
