...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    That file is still the user's only copy of their intended overrides тАФ if
    ...
    backup is best-effort тАФ any failure (permissions, symlink, disk full) is
        ...
        for existing in sibling_baks:
            try:
                if existing.stat().st_size == st.st_size:
                    # Same size as the current broken file тАФ likely the same
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    msg = (
        ...
        f"Falling back to default config тАФ every user override "
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"тЪая╕П  hermes config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
# Env var names that influence how the next subprocess executes тАФ
# never writable through ``save_env_value``. Anything that controls
# the loader, interpreter, shell, or replacement editor counts:
#
# * ``LD_PRELOAD`` / ``LD_LIBRARY_PATH`` / ``LD_AUDIT`` тАФ Linux dynamic
#   loader. ``DYLD_*`` тАФ macOS equivalent. Planting a path here means
#   the next ``subprocess.run([...])`` Hermes makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` тАФ Python interpreter init. Hermes itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` тАФ Node interpreter; affects npm,
#   ``hermes update``, the TUI build.
# * ``PATH`` тАФ too broad to allow. The dashboard never needs to rewrite
...
# * ``GIT_SSH_COMMAND`` / ``GIT_EXEC_PATH`` тАФ git rewrites that fire
#   on every plugin install / ``hermes update``.
# * ``BROWSER`` / ``EDITOR`` / ``VISUAL`` / ``PAGER`` тАФ commands the
#   shell or CLI invokes implicitly. Wrong values here = RCE on next
#   ``$EDITOR``.
# * ``SHELL`` тАФ what subprocess uses with ``shell=True`` (we try to
#   avoid that, but defense in depth).
# * ``HERMES_HOME`` / ``HERMES_PROFILE`` / ``HERMES_CONFIG`` /
#   ``HERMES_ENV`` тАФ Hermes runtime location flags. Writing these into
...
#
# This is enforced on *write* only тАФ values already in ``.env`` (set
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    # Hermes runtime location тАФ never via dashboard env writer.
...
# load repopulates automatically тАФ no explicit invalidation hook.
_LOAD_CONFIG_CACHE: Dict[str, Tuple[int, int, Dict[str, Any]]] = {}
# (path, mtime_ns, size) -> cached raw yaml dict. Same pattern as
# _LOAD_CONFIG_CACHE but for read_raw_config() тАФ used when callers want
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Deprecated tool-progress env vars тАФ replaced by display.tool_progress in
    ...
    "MATRIX_RECOVERY_KEY",
    # Langfuse observability plugin тАФ optional tuning keys + standard SDK vars.
    # Activation is via plugins.enabled (opt-in through `hermes plugins enable
    # observability/langfuse` or `hermes tools тЖТ Langfuse`); credentials gate
...
#   - The pre-existing fallback message ("тЬЧ Not a git repository. Please
#     reinstall: curl ... install.sh") is actively misleading inside Docker
#     тАФ that script installs a *new* host-side Hermes, it doesn't update
...
тЬЧ ``hermes update`` doesn't apply inside the Docker container.

Hermes Agent runs as a published image (nousresearch/hermes-agent), not a
git checkout тАФ the container has no working tree to pull into.  Update by
...
Notes:
  тАв If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container тАФ pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/nousresearch/hermes-agent/tags
  тАв Your config and session history live under ``$HERMES_HOME`` (``/opt/data``
    in the container, typically bind-mounted from the host) and persist
    across image upgrades тАФ re-pulling doesn't lose any state.
  тАв Running a fork?  Build your own image with this repo's ``Dockerfile``
...
# Re-export from hermes_constants тАФ canonical definition lives there.
...
def _chown_to_hermes_uid(path) -> None:
    ...
    No-op when:
      ...
      - The current process isn't root (chown will EPERM тАФ silently ignored)
    ...
    try:
        ...
        # both of which are non-fatal тАФ the dir is still created and
...
def _secure_dir(path):
    ...
    Skipped in managed mode тАФ the NixOS module sets group-readable
    ...
    vars are set (#34107 тАФ Docker deployments need this so profile subdirs
...
def _secure_file(path):
    ...
    Skipped in managed mode тАФ the NixOS activation script sets
    group-readable permissions (0640) on config files.

    Skipped in containers тАФ Docker/Podman volume mounts often need broader
...
def _ensure_hermes_home_managed(home: Path):
    ...
    # Inside umask(0o007) scope тАФ SOUL.md will be created as 0660
    _ensure_default_soul_md(home)
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # coding conversation mid-reasoning runs 60тАУ150s per call, so a 60s
        ...
        # Values: "auto" (default тАФ applies to gpt/codex models), true/false
        # (force on/off for all models), or a list of model-name substrings
        # to match (e.g. ["gpt", "codex", "gemini", "qwen"]).
        "tool_use_enforcement": "auto",
        # Universal "finish the job" guidance тАФ short prompt block applied to
        ...
        # Local-environment toolchain probe тАФ surfaces Python/pip/uv/PEP-668
        # state in the system prompt when something non-default is detected
        # (e.g. python3 has no pip module, pipтЖТpython version mismatch, PEP
        ...
        # (docker/modal/ssh тАФ they have their own probe).  Set False to
        # disable entirely.
        ...
        # тАФ proxy, credential handling, mount layout тАФ without editing the
        # identity slot (SOUL.md). Empty by default. The HERMES_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture тАФ on interactive coding surfaces (CLI, TUI, desktop
        ...
        # interrupted тАФ process the unfinished tool result(s) first]"
    ...
    "terminal": {
        ...
        #   auto    тАФ host keeps the real OS-user HOME; containers use
        #             HERMES_HOME/home for persistent state (default)
        #   real    тАФ force the real OS-user HOME
        #   profile тАФ force HERMES_HOME/home when it exists (old strict
        ...
        # a bash login shell would skip тАФ most commonly ``~/.bashrc``
        ...
        # behaviour тАФ disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        ...
        # captures PATH additions, shell functions, and aliases тАФ which a
        ...
        # docker_env lets you specify exact key-value pairs тАФ useful when Hermes
        ...
        # Container resource limits (docker, singularity, modal, daytona тАФ ignored for local/ssh)
        "container_cpu": 1,
        "container_memory": 5120,       # MB (default 5GB)
        "container_disk": 51200,        # MB (default 50GB)
        "container_persistent": True,   # Persist filesystem across sessions
        # Docker volume mounts тАФ share host directories with the container.
        ...
        # Persistent shell тАФ keep a long-lived bash shell across execute() calls
    ...
    "web": {
        "backend": "",           # shared fallback тАФ applies to both search and extract
    ...
    "browser": {
        ...
        # "auto"       тАФ use Chrome (default, don't pass --engine at all)
        # "lightpanda" тАФ use Lightpanda (1.3-5.8x faster navigation, no screenshots)
        # "chrome"     тАФ explicitly request Chrome
        # Also settable via AGENT_BROWSER_ENGINE env var.
        "engine": "auto",
        "auto_local_for_private_urls": True,  # When a cloud provider is set, auto-spawn local Chromium for LAN/localhost URLs instead of sending them to the cloud
        "cdp_url": "",  # Optional persistent CDP endpoint for attaching to an existing Chromium/Chrome
        # CDP supervisor тАФ dialog + frame detection via a persistent WebSocket.
    ...
    # Filesystem checkpoints тАФ automatic snapshots before destructive file ops.
    ...
    # 100K chars тЙИ 25тАУ35K tokens across typical tokenisers.
    ...
    #                    (default 50_000 тЙИ 12-15K tokens).
    # - max_lines:       read_file pagination cap тАФ the maximum `limit`
    ...
    "compression": {
        ...
        "codex_gpt55_autoraise": True,  # When True, gpt-5.5 on the ChatGPT Codex OAuth
                                      ...
                                      # exact route is affected тАФ gpt-5.5 on OpenAI's
    ...
    "bedrock": {
        "region": "",  # AWS region for Bedrock API calls (empty = AWS_REGION env var тЖТ us-east-1)
        "discovery": {
        ...
        "guardrail": {
            # Amazon Bedrock Guardrails тАФ content filtering and safety policies.
    ...
    # Auxiliary model config тАФ provider:model for each side task.
    ...
    #
    # Each aux task is independent тАФ main-agent provider_routing and
    ...
    "auxiliary": {
        "vision": {
            ...
            "timeout": 120,        # seconds тАФ LLM API call timeout; vision payloads need generous timeout
            "extra_body": {},      # OpenAI-compatible provider-specific request fields
            "download_timeout": 30,  # seconds тАФ image HTTP download timeout; increase for slow connections
        },
        "web_extract": {
            "provider": "auto",
            "model": "",
            "base_url": "",
            "api_key": "",
            "timeout": 360,        # seconds (6min) тАФ per-attempt LLM summarization timeout; increase for slow local models
            "extra_body": {},
        },
        "compression": {
            "provider": "auto",
            ...
            "base_url": "",
            "api_key": "",
            "timeout": 120,        # seconds тАФ compression summarises large contexts; increase for local models
            "extra_body": {},
        },
        # Note: session_search no longer uses an auxiliary LLM (PR #27590 тАФ
        ...
        # Triage specifier тАФ flesh out a rough one-liner in the Kanban
        ...
        # Kanban decomposer тАФ decomposes a triage task into a graph of
        ...
        # Profile describer тАФ auto-generates a 1-2 sentence description
        ...
        # Curator тАФ skill-usage review fork. Timeout is generous because the
        # review pass can take several minutes on reasoning models (umbrella
        # building over hundreds of candidate skills). "auto" = use main chat
        # model; override via `hermes model` тЖТ auxiliary тЖТ Curator to route
        ...
        # Monitor тАФ urgency/importance classifier used by the important-mail
    ...
    "display": {
        ...
        #   "cli" тАФ the classic prompt_toolkit REPL (default, preserves prior behavior)
        #   "tui" тАФ the modern Ink TUI (same as passing `--tui`)
        ...
        #   "off"     тАФ no chat notification (the review still runs and writes)
        #   "on"      тАФ generic "ЁЯТ╛ Memory updated" line (default)
        #   "verbose" тАФ include a compact content preview of what changed
        ...
        # restored).  When false, no credits notices are emitted тАФ balance data
        ...
        # abnormally with no usable reply тАФ empty content after retries, a
        ...
        "tool_progress_overrides": {},  # DEPRECATED тАФ use display.platforms instead
        ...
        # Auto-delete system-notice replies (e.g. "тЬи New session started!",
        # "тЩ╗ Restarting gatewayтАж", "тЪб StoppedтАж") after N seconds on platforms
        ...
        # replies wrapped with gateway.platforms.base.EphemeralReply тАФ agent
        ...
        # platform ("telegram", "discord", "slack", тАж) mapping to a dict of
        ...
        # streaming.enabled master switch still gates everything тАФ these
    ...
    "dashboard": {
        ...
        # settings. Each can be overridden by an environment variable тАФ
        # ``HERMES_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``HERMES_DASHBOARD_PORTAL_URL`` respectively тАФ and the env var
        ...
        "oauth": {
            "client_id": "",  # agent:{instance_id} тАФ Portal provisions this
            "portal_url": "",  # blank тЖТ use plugin default (production Portal)
        },
        # Username/password gate configuration тАФ read by the bundled
        ...
        # either ``password_hash`` (preferred тАФ no plaintext at rest) or
        ...
        # empty (the default) to keep the plugin a no-op тАФ loopback /
        ...
        # is generated тАФ fine for a single process, but sessions then
        ...
        "basic_auth": {
            "username": "",  # blank тЖТ plugin no-op (no password provider)
            "password_hash": "",  # scrypt$... (preferred тАФ no plaintext at rest)
            "password": "",  # plaintext fallback (hashed in-memory at load)
            "secret": "",  # token-signing key; blank тЖТ random per-process
            "session_ttl_seconds": 0,  # 0 тЖТ plugin default (12h)
        },
        # Public URL override (env: ``HERMES_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority тАФ scheme + host +
        # optional path prefix (e.g. ``https://example.com/hermes``) тАФ
        ...
        # the operator has declared the public URL тАФ we no longer need
    ...
    "tts": {
        ...
        "xai": {
            "voice_id": "eve",  # or custom voice ID тАФ see https://docs.x.ai/developers/model-capabilities/audio/custom-voices
            "language": "en",
            "sample_rate": 24000,
            "bit_rate": 128000,
        },
    ...
    "memory": {
        ...
        #   false (default) тАФ write freely; the gate is off (pre-gate behaviour)
        #   true            тАФ require approval: foreground writes prompt inline
    ...
    # Subagent delegation тАФ override the provider:model used by delegate_task
    ...
    "delegation": {
        ...
        "api_mode": "",    # wire protocol for delegation.base_url: "chat_completions",
                           ...
                           # from URL (e.g. /anthropic suffix тЖТ anthropic_messages). Set this
        ...
        # and _get_orchestrator_enabled).  Floored at 1, no upper ceiling тАФ
        # raise deliberately, each level multiplies API cost.
        "max_spawn_depth": 1,        # depth (1 = flat [default], 2 = orchestratorтЖТleaf, 3+ = deeper)
        "orchestrator_enabled": True,  # kill switch for role="orchestrator"
        # When a subagent hits a dangerous-command approval prompt, the parent's
        # prompt_toolkit TUI owns stdin тАФ a thread-local input() call from the
        ...
        #   false (default) тЖТ auto-deny with a logger.warning audit line (safe)
        #   true             тЖТ auto-approve "once" with a logger.warning audit line
        # Flip to true only if you trust delegated work to run dangerous cmds
        # without human review (cron pipelines, batch automation, etc.).
        "subagent_auto_approve": False,
    },

    # Ephemeral prefill messages file тАФ JSON list of {role, content} dicts
    # injected at the start of every API call for few-shot priming.
    # Never saved to sessions, logs, or trajectories.
    "prefill_messages_file": "",

    # Goals тАФ persistent cross-turn goals (Ralph-style loop).
    ...
    # fail OPEN (continue) so a flaky judge never wedges progress тАФ the
    ...
    # Skills тАФ external skill directories for sharing skills across tools/agents.
    # Each path is expanded (~, ${VAR}) and resolved.  Read-only тАФ skill creation
    ...
    "skills": {
        ...
        # state, detected tool versions, тАж).  Off by default because any
        ...
        # security.  Turn on if you want the belt-and-suspenders тАФ a
        ...
        #   false (default) тАФ write freely; the gate is off (pre-gate behaviour)
        #   true            тАФ require approval: stage the write for review
        ...
        #                     /skills diff <id> (full diff тАФ CLI/dashboard/file,
        #                     never crammed into a chat bubble), apply with
        #                     /skills approve <id> or drop with /skills reject <id>.
        "write_approval": False,
    },

    # Curator тАФ background skill maintenance.
    #
    ...
    # and patch drift. Runs inactivity-triggered from session start тАФ no
    # cron daemon.
    #
    ...
    "curator": {
        ...
        # without use. Archived skills are recoverable тАФ no auto-deletion.
        ...
        # archived. Hub-installed skills are NEVER pruned here тАФ they have an
    ...
    "discord": {
        ...
        "reactions": True,             # Add ЁЯСА/тЬЕ/тЭМ reactions to messages during processing
        "channel_prompts": {},         # Per-channel ephemeral system prompts (forum parents apply to child threads)
        # Opt-in DM role-based auth (#12136). By default, DISCORD_ALLOWED_ROLES
        # authorizes only guild messages in the role's own guild тАФ DMs require
        ...
        # stop-and-swap тАФ the Grok-voice-mode feel. discord.py ships no mixer;
        ...
        "voice_fx": {
            ...
            "ambient_gain": 0.18,     # idle bed loudness, 0.0тАУ1.0
            "duck_gain": 0.06,        # ambient loudness while speech plays
            "speech_gain": 1.0,       # TTS / ack loudness, 0.0тАУ1.0
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "тЪХ *Hermes Agent*" header.
        # Set to "" (empty string) to disable the header entirely.
        # Supports \n for newlines, e.g. "ЁЯдЦ *My Bot*\nтФАтФАтФАтФАтФАтФА\n"
    },

    # Telegram platform settings (gateway mode)
    "telegram": {
        "reactions": False,            # Add ЁЯСА/тЬЕ/тЭМ reactions to messages during processing
    ...
    #   manual тАФ always prompt the user (default)
    #   smart  тАФ use auxiliary LLM to auto-approve low-risk commands, prompt for high-risk
    #   off    тАФ skip all approval prompts (equivalent to --yolo)
    #
    # cron_mode тАФ what to do when a cron job hits a dangerous command:
    #   deny    тАФ block the command and let the agent find another way (default, safe)
    #   approve тАФ auto-approve all dangerous commands in cron jobs
    "approvals": {
        "mode": "manual",
        "timeout": 60,
        "cron_mode": "deny",
        ...
        # prompt), so the next message re-sends full input tokens тАФ this can
        ...
        # through tools.slash_confirm тАФ native yes/no buttons on Telegram,
    ...
    # Shell-script hooks тАФ declarative bridge that invokes shell scripts
    ...
    # Custom personalities тАФ add your own entries here
    ...
    "security": {
        ...
        # ``pip install`` for everything beyond the base set тАФ appropriate
    ...
    # Kanban multi-agent coordination тАФ controls the dispatcher loop that
    ...
    "kanban": {
        # Run the dispatcher inside the gateway process. On by default тАФ
        # the cost is ~300┬╡s every `dispatch_interval_seconds` when idle,
        ...
        # Unset (None) means "no per-profile cap" тАФ backward-compatible
    ...
    # execute_code settings тАФ controls the tool used for programmatic tool calls.
    "code_execution": {
        # Execution mode:
        #   project (default) тАФ scripts run in the session's working directory
        #     with the active virtualenv/conda env's python, so project deps
        #     (pandas, torch, project packages) and relative paths resolve.
        #   strict            тАФ scripts run in an isolated temp directory with
    ...
    # in the model-facing tools array with three bridge tools тАФ
    # tool_search / tool_describe / tool_call тАФ and surfaced on demand.
    #
    ...
    "tools": {
        "tool_search": {
            # "auto" (default) тАФ activate only when deferrable tool schemas
            #   exceed ``threshold_pct`` of the active model's context length,
            #   so small toolsets pay no overhead.
            # "on"  тАФ always activate when there is at least one deferrable
            #   tool. Use when you have many MCP servers and want maximum
            #   token reduction unconditionally.
            # "off" тАФ disable entirely. Tools-array assembly is a pass-through.
            "enabled": "auto",
    ...
    # Logging тАФ controls file logging to ~/.hermes/logs/.
    ...
    # Network settings тАФ workarounds for connectivity issues.
    "network": {
    ...
    # Gateway settings тАФ control how messaging platforms (Telegram, Discord,
    ...
    "gateway": {
        ...
        # sent. Off by default тАФ when off, the model sees clean message text.
        ...
        # тАФ we accept any document type the user uploads, and the agent
        ...
        # operator allowlist тАФ useful for ``pandoc -o /tmp/report.pdf`` or
    ...
    # the feature discoverable in config.yaml тАФ it does not change behavior.
    #
    ...
    "streaming": {
        ...
        "enabled": False,
        # Transport selection:
        #   "auto"  тАФ prefer native draft streaming where the platform
        ...
        #   "draft" тАФ explicitly request native drafts; falls back to edit
        #             when the platform/chat doesn't support them.
        #   "edit"  тАФ progressive editMessageText only (legacy behavior).
        #   "off"   тАФ disable streaming entirely (same as enabled: false).
        "transport": "auto",
        # Minimum seconds between progressive edits тАФ tuned for Telegram's
    ...
    # Session storage тАФ controls automatic cleanup of ~/.hermes/state.db.
    ...
    "sessions": {
        ...
        # Only touches ended sessions тАФ active sessions are always preserved.
        ...
        # reclaim disk space on DELETE тАФ freed pages are just reused on
        # subsequent INSERTs тАФ so without VACUUM the file stays bloated
        ...
        # and token counts), so this is off by default тАФ the snapshots had
    ...
    "updates": {
        ...
        # can be restored with ``hermes import <path>``.  Off by default тАФ
        ...
        # floored to 1 тАФ the backup just created is always preserved.  To
        ...
        # source tree when it runs NON-interactively тАФ i.e. triggered from
        ...
        # as they always have.
        #   "stash"   тАФ auto-stash the changes, pull, then auto-restore them
        #               on top of the updated code (the safe default; nothing
        #               is ever lost тАФ conflicts are preserved in a git stash).
        #   "discard" тАФ auto-stash the changes and throw the stash away after
        ...
        #               ignored paths тАФ node_modules, venv, build outputs тАФ
        #               are never touched.
        "non_interactive_local_changes": "stash",
    },

    # Language Server Protocol тАФ semantic diagnostics from real
    ...
    # tier тАФ handy for Telegram/Discord chats where the cwd is the
    # user's home directory.
    "lsp": {
        # Master toggle.  Setting this to false disables the entire
        # subsystem тАФ no servers spawn, no background event loop, no
        # cost.
        "enabled": True,
        ...
        # ``"auto"`` тАФ try to install via npm/go/pip into
        #              ``<HERMES_HOME>/lsp/bin/`` on first use.
        # ``"manual"`` тАФ only use binaries already on PATH.
        # ``"off"`` тАФ alias for ``manual``.
        ...
        #   disabled: true
        #     тАФ skip this server even when its extensions match
        #   command: ["full/path/to/server", "--stdio"]
        #     тАФ pin a custom binary path; bypasses auto-install
        #   env: {"KEY": "value"}
        #     тАФ extra env vars passed to the spawned process
        #   initialization_options: {...}
        #     тАФ merged into the LSP ``initializationOptions``
    ...
    "x_search": {
        ...
        # 60-120s for complex queries тАФ the default is generous.
        "timeout_seconds": 180,
    ...
    "secrets": {
        "bitwarden": {
            ...
            # bws binary is never auto-installed тАФ same as not having
            # this section at all.
            "enabled": False,
            ...
            # True because the point of using BSM is centralized rotation тАФ
...
# dict is intentionally empty тАФ no single env var is universally required.
REQUIRED_ENV_VARS = {}

# Optional environment variables that enhance functionality
OPTIONAL_ENV_VARS = {
    # тФАтФА Provider (handled in provider selection, not shown in checklists) тФАтФА
    "NOUS_BASE_URL": {
    ...
    "HERMES_GEMINI_CLIENT_ID": {
        ...
        "prompt": "Google OAuth client ID (optional тАФ leave empty to use the public default)",
    ...
    "OLLAMA_API_KEY": {
        "description": "Ollama Cloud API key (ollama.com тАФ cloud-hosted open models)",
    ...
    "BROWSERBASE_API_KEY": {
        "description": "Browserbase API key for cloud browser (optional тАФ local browser works without this)",
    ...
    "BROWSERBASE_PROJECT_ID": {
        "description": "Browserbase project ID (optional тАФ only needed for cloud browser)",
    ...
    "BROWSER_USE_API_KEY": {
        "description": "Browser Use API key for cloud browser (optional тАФ local browser works without this)",
    ...
    # тФАтФА Bundled skills (opt-in: only needed if the user uses that skill) тФАтФА
    # These use category="skill" (distinct from "tool") so the sandbox
    # env blocklist in tools/environments/local.py does NOT rewrite them тАФ
    ...
    # тФАтФА Langfuse observability тФАтФА
    ...
    # тФАтФА Messaging platforms тФАтФА
    "TELEGRAM_BOT_TOKEN": {
    ...
    "SLACK_APP_TOKEN": {
        "description": "Slack app-level token (xapp-) for Socket Mode. Get from Basic Information тЖТ "
    ...
    "MATRIX_RECOVERY_KEY": {
        "description": "Matrix recovery key for cross-signing verification after device key rotation (from Element: Settings тЖТ Security тЖТ Recovery Key)",
    ...
    "BLUEBUBBLES_PASSWORD": {
        "description": "BlueBubbles server password (from BlueBubbles Server тЖТ Settings тЖТ API)",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hermes API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only тАФ all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
    ...
    # тФАтФА Agent settings тФАтФА
    # NOTE: MESSAGING_CWD was removed here тАФ use terminal.cwd in config.yaml
    ...
    # HERMES_TOOL_PROGRESS and HERMES_TOOL_PROGRESS_MODE are deprecated тАФ
...
# Tool Gateway env vars are always visible тАФ they're useful for
...
def _set_nested(config, dotted_key: str, value):
    ...
    Supports both dict and list navigation:
      _set_nested(c, "a.b.c", 1)     тЖТ c["a"]["b"]["c"] = 1
      _set_nested(c, "a.0.b", 1)     тЖТ c["a"][0]["b"] = 1
      _set_nested(c, "providers.1", "x") тЖТ c["providers"][1] = "x"
    ...
    (we do not grow lists тАФ the user is navigating into structure they
    wrote themselves).  If a segment targets a non-container leaf
    (scalar), the leaf is replaced with a fresh dict so the write can
    proceed тАФ this preserves the pre-existing behavior for bare scalar
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _CAMEL_ALIASES: Dict[str, str] = {
        ...
        "apiMode": "api_mode",
        "keyEnv": "key_env",
        "apiKeyEnv": "key_env",  # alias тАФ OpenClaw-compatible + docs variant
...
_VALID_CUSTOM_PROVIDER_FIELDS = {
    ...
    # тАФ include it here so the set accurately describes the supported schema.
    "key_env",
}
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    # тФАтФА custom_providers must be a list, not a dict тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    cp = config.get("custom_providers")
    if cp is not None:
        if isinstance(cp, dict):
            issues.append(ConfigIssue(
                "error",
                "custom_providers is a dict тАФ it must be a YAML list (items prefixed with '-')",
                "Change to:\n"
                "  custom_providers:\n"
                "    - name: my-provider\n"
                "      base_url: https://...\n"
    ...
    # тФАтФА fallback_model: single dict OR list of dicts (chain) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    fb = config.get("fallback_model")
    if fb is not None:
        if isinstance(fb, list):
            # Chain fallback тАФ validate each entry
            ...
            if not fb.get("provider"):
                issues.append(ConfigIssue(
                    "warning",
                    "fallback_model is missing 'provider' field тАФ fallback will be disabled",
                    "Add: provider: openrouter (or another provider)",
                ))
            if not fb.get("model"):
                issues.append(ConfigIssue(
                    "warning",
                    "fallback_model is missing 'model' field тАФ fallback will be disabled",
                    "Add: model: anthropic/claude-sonnet-4 (or another model)",
                ))

    # тФАтФА Check for fallback_model accidentally nested inside custom_providers тФАтФА
    ...
    # тФАтФА model section: should exist when custom_providers is configured тФАтФА
    model_cfg = config.get("model")
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section тАФ Hermes won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
    ...
    # тФАтФА Root-level keys that look misplaced тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    for key in config:
        if key.startswith("_"):
            continue
        if key not in _KNOWN_ROOT_KEYS and key in _CUSTOM_PROVIDER_LIKE_FIELDS:
            issues.append(ConfigIssue(
                "warning",
                f"Root-level key '{key}' looks misplaced тАФ should it be under 'model:' or inside a 'custom_providers' entry?",
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines = ["\033[33mтЪа Config issues detected in config.yaml:\033[0m"]
    for ci in issues:
        marker = "\033[31mтЬЧ\033[0m" if ci.severity == "error" else "\033[33mтЪа\033[0m"
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    These env vars are deprecated тАФ the canonical setting is terminal.cwd
    ...
    if messaging_cwd:
        lines.append(
            f"  \033[33mтЪа\033[0m MESSAGING_CWD={messaging_cwd} found in .env тАФ "
            f"this is deprecated."
        )
    if terminal_cwd_env and not config_has_explicit_cwd:
        # TERMINAL_CWD in env but not from config bridge тАФ likely from .env
        lines.append(
            f"  \033[33mтЪа\033[0m TERMINAL_CWD={terminal_cwd_env} found in .env тАФ "
            f"this is deprecated."
        )
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.hermes")
        lines.insert(0, "\033[33mтЪа Deprecated .env settings detected:\033[0m")
        lines.append(
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    # тФАтФА Always: sanitize .env (split concatenated keys) тФАтФА
    try:
        fixes = sanitize_env_file()
        if fixes and not quiet:
            print(f"  тЬУ Repaired .env file ({fixes} corrupted entries fixed)")
    except Exception:
    ...
    # тФАтФА Version 3 тЖТ 4: migrate tool progress from .env to config.yaml тФАтФА
    if current_ver < 4:
        config = load_config()
        ...
        if "tool_progress" not in display:
            ...
            save_config(config)
            if not quiet:
                print(f"  тЬУ Migrated tool progress to config.yaml: {display['tool_progress']}")
    
    # тФАтФА Version 4 тЖТ 5: add timezone field тФАтФА
    if current_ver < 5:
        config = load_config()
        if "timezone" not in config:
            ...
            if not quiet:
                ...
                print(f"  тЬУ Added timezone to config.yaml: {tz_display}")

    # тФАтФА Version 8 тЖТ 9: clear ANTHROPIC_TOKEN from .env тФАтФА
    ...
    if current_ver < 9:
        try:
            ...
            if old_token:
                ...
                if not quiet:
                    print("  тЬУ Cleared ANTHROPIC_TOKEN from .env (no longer used)")
        except Exception:
            pass

    # тФАтФА Version 11 тЖТ 12: migrate custom_providers list тЖТ providers dict тФАтФА
    if current_ver < 12:
        config = load_config()
        ...
        if isinstance(custom_list, list) and custom_list:
            ...
            if migrated_count > 0:
                ...
                # Remove the old list тАФ runtime reads via get_compatible_custom_providers()
                config.pop("custom_providers", None)
                save_config(config)
                if not quiet:
                    print(f"  тЬУ Migrated {migrated_count} custom provider(s) to providers: section")
                    for key in list(providers_dict.keys())[-migrated_count:]:
                        ep = providers_dict[key]
                        print(f"    тЖТ {key}: {ep.get('api', '')}")

    # тФАтФА Version 12 тЖТ 13: clear dead LLM_MODEL / OPENAI_MODEL from .env тФАтФА
    ...
    # Stale entries cause user confusion тАФ see issue report.
    if current_ver < 13:
        for dead_var in ("LLM_MODEL", "OPENAI_MODEL"):
            try:
                ...
                if old_val:
                    ...
                    if not quiet:
                        print(f"  тЬУ Cleared {dead_var} from .env (no longer used тАФ config.yaml is source of truth)")
            except Exception:
                pass

    # тФАтФА Version 13 тЖТ 14: migrate legacy flat stt.model to provider section тФАтФА
        ...
        if isinstance(raw_stt, dict) and "model" in raw_stt:
            ...
            if provider in {"local", "local_command"}:
                ...
                if legacy_model in _local_models:
                    # Check raw config тАФ only set if user didn't already
                ...
                # else: drop it тАФ it was an OpenAI model name, local section
                # already defaults to "base" via DEFAULT_CONFIG
            else:
                # Cloud provider тАФ put it in that provider's section only
            ...
            save_config(config)
            if not quiet:
                print(f"  тЬУ Migrated legacy stt.model to provider-specific config")

    # тФАтФА Version 14 тЖТ 15: add explicit gateway interim-message gate тФАтФА
    if current_ver < 15:
        ...
        if "interim_assistant_messages" not in display:
            ...
            save_config(config)
            if not quiet:
                print("  тЬУ Added display.interim_assistant_messages=true")

    # тФАтФА Version 15 тЖТ 16: migrate tool_progress_overrides into display.platforms тФАтФА
    if current_ver < 16:
        config = read_raw_config()
        display = config.get("display", {})
        if not isinstance(display, dict):
        ...
        if isinstance(old_overrides, dict) and old_overrides:
            ...
            if not quiet:
                ...
                print(f"  тЬУ Migrated tool_progress_overrides тЖТ display.platforms: {migrated}")
            results["config_added"].append("display.platforms (migrated from tool_progress_overrides)")

    # тФАтФА Version 16 тЖТ 17: remove legacy compression.summary_* keys тФАтФА
    if current_ver < 17:
        config = read_raw_config()
        ...
        if isinstance(comp, dict):
            ...
            if migrated_keys or s_model is not None or s_provider is not None or s_base_url is not None:
                ...
                if not quiet:
                    if migrated_keys:
                        print(f"  тЬУ Migrated compression.summary_* тЖТ auxiliary.compression: {', '.join(migrated_keys)}")
                    else:
                        print("  тЬУ Removed unused compression.summary_* keys")

    # тФАтФА Version 20 тЖТ 21: plugins are now opt-in; grandfather existing user plugins тФАтФА
    ...
    #
    # Bundled plugins (shipped in the repo itself) are NOT grandfathered тАФ
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            if not quiet:
                if grandfathered:
                    print(
                        f"  тЬУ Plugins now opt-in: grandfathered "
                        f"{len(grandfathered)} existing plugin(s) into plugins.enabled"
                    )
                else:
                    print(
                        "  тЬУ Plugins now opt-in: no existing plugins to grandfather. "
                        "Use `hermes plugins enable <name>` to activate."
                    )

    # тФАтФА Version 22 тЖТ 23: seed curator defaults + create logs/curator/ тФАтФА
    ...
    #      тАФ only keys the user hasn't already overridden.
    #   2. Writes the `auxiliary.curator` aux-task slot (provider, model,
    #      base_url, api_key, timeout, extra_body) тАФ canonical slot for
    #      routing the curator fork to a cheaper aux model.
    #   3. Creates `~/.hermes/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hermes_home() тАФ old profiles that predate this
    ...
    if current_ver < 23:
        ...
        # (1) Top-level curator section тАФ only add missing keys
        ...
        if touched:
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        "  тЬУ Seeded curator defaults in config.yaml: "
            ...
            if added_aux:
                ...
                if not quiet:
                    print(
                        "  тЬУ Seeded auxiliary.curator defaults in config.yaml: "
                        f"{', '.join(added_aux)}"
                    )

    # тФАтФА Version 24 тЖТ 25: lower model_catalog TTL 24h тЖТ 1h тФАтФА
    ...
    # stale window. Only rewrite the OLD default (24) тАФ never clobber a
    ...
    if current_ver < 25:
        ...
        if isinstance(raw_mc, dict) and raw_mc.get("ttl_hours") == 24:
            ...
            save_config(config)
            results["config_added"].append("model_catalog.ttl_hours 24тЖТ1")
            if not quiet:
                print("  тЬУ Lowered model_catalog.ttl_hours to 1 (hourly picker refresh)")

    # тФАтФА Version 28 тЖТ 29: rename memory/skills write_mode тЖТ write_approval тФАтФА
    ...
    # it maps to true; everything else (on/off/unset) тЖТ false. The old
    # "off = block all writes" mode is dropped тАФ memory_enabled: false disables
    ...
    if current_ver < 29:
        ...
        for subsystem in ("memory", "skills"):
            ...
            touched = True
            results["config_added"].append(
                f"{subsystem}.write_mode тЖТ write_approval={sub['write_approval']}"
            )
        if touched:
            save_config(config)
            if not quiet:
                print("  тЬУ Renamed write_mode тЖТ write_approval (boolean gate)")

    # тФАтФА Post-migration: disable exfiltration-shaped MCP stdio entries тФАтФА
        ...
        if _validate_mcp_server_entry:
            ...
            for server_name, entry in raw_mcp_servers.items():
                ...
                if not quiet:
                    for issue in issues:
                        print(f"  тЪа {issue}")
                    print(f"  тЪа Disabled MCP server '{server_name}' pending review")
            if mcp_touched:
                config["mcp_servers"] = raw_mcp_servers
                save_config(config)

    if current_ver < latest_ver and not quiet:
        print(f"Config version: {current_ver} тЖТ {latest_ver}")
    
    # Check for missing required env vars
    missing_env = get_missing_env_vars(required_only=True)
    
    if missing_env and not quiet:
        print("\nтЪая╕П  Missing required environment variables:")
        for var in missing_env:
            print(f"   тАв {var['name']}: {var['description']}")
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            for name, info in new_and_unset:
                print(f"    тАв {name} тАФ {info.get('description', '')}")
            print()
            try:
    ...
    # тФАтФА Skill-declared config vars тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        for var in missing_skill_config:
            ...
            print(f"    тАв {var['key']} тАФ {var['description']} (from skill: {skill_name})")
        print()
        try:
        ...
        if answer in {"y", "yes"}:
            ...
            for var in missing_skill_config:
                ...
                if value:
                    ...
                    print(f"  тЬУ Saved {var['key']} = {value}")
                else:
                    results["warnings"].append(
                        f"Skipped {var['key']} тАФ skill '{var.get('skill', '?')}' may ask for it later"
                    )
                print()
            save_config(config)
        else:
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ``model.*`` key is empty тАФ they never override an existing value.
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    default)`` semantics тАФ ``default`` is only returned when the key is
...
def read_raw_config() -> Dict[str, Any]:
    ...
    Cached on the config file's (mtime_ns, size) тАФ same strategy as
...
def load_config() -> Dict[str, Any]:
    ...
    defensive deepcopy тАФ that path matters in agent-loop hot spots like
...
def load_config_readonly() -> Dict[str, Any]:
    ...
    caller** тАФ only use this when you are absolutely sure your code path
    ...
    safety guarantee is purely documented, not enforced тАФ be careful.
    """
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        if cache_key is not None:
            ...
            # calls will see тАФ keeps "two readonly calls return the same
        ...
        # file missing), it's also fine тАФ callers get an isolated object.
        return expanded


_SECURITY_COMMENT = """
# тФАтФА Security тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
# Secret redaction is ON by default тАФ strings that look like API keys,
...
_FALLBACK_COMMENT = """
# тФАтФА Fallback Model тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
# Automatic provider failover when primary is unavailable.
# Uncomment and configure to enable. Triggers on rate limits (429),
# overload (529), service errors (503), or connection failures.
#
# Supported providers:
#   openrouter   (OPENROUTER_API_KEY)  тАФ routes to any model
#   openai-codex (OAuth тАФ hermes auth) тАФ OpenAI Codex
#   nous         (OAuth тАФ hermes auth) тАФ Nous Portal
#   zai          (ZAI_API_KEY)         тАФ Z.AI / GLM
#   kimi-coding  (KIMI_API_KEY)        тАФ Kimi / Moonshot
#   kimi-coding-cn (KIMI_CN_API_KEY)   тАФ Kimi / Moonshot (China)
#   minimax      (MINIMAX_API_KEY)     тАФ MiniMax
#   minimax-cn   (MINIMAX_CN_API_KEY)  тАФ MiniMax (China)
#   bedrock      (AWS IAM / boto3)     тАФ AWS Bedrock (Converse API)
#
...
def load_env() -> Dict[str, str]:
    ...
    panels). Sanitisation is O(lines ├Ч known-keys), so re-parsing the
...
# Editing .env bumps mtime тЖТ next load_env() rebuilds. invalidate_env_cache()
...
def _check_non_ascii_credential(key: str, value: str) -> str:
    ...
    API keys and tokens must be pure ASCII тАФ they are sent as HTTP header
    ...
    try:
        value.encode("ascii")
        return value  # all ASCII тАФ nothing to do
...
def save_env_value(key: str, value: str):
    ...
    value = value.replace("\n", "").replace("\r", "")
    # API keys / tokens must be ASCII тАФ strip non-ASCII with a warning.
...
def reload_env() -> int:
    ...
    the .env file (but only vars known to Hermes тАФ OPTIONAL_ENV_VARS and
    _EXTRA_ENV_KEYS тАФ to avoid clobbering unrelated environment).
    """
    env_vars = load_env()
    known_keys = set(OPTIONAL_ENV_VARS.keys()) | _EXTRA_ENV_KEYS
    count = 0
...
def redact_key(key: str) -> str:
    ...
    Thin wrapper over :func:`agent.redact.mask_secret` тАФ preserves the
...
def show_config():
    ...
    print()
    print(color("тФМтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФР", Colors.CYAN))
    print(color("тФВ              тЪХ Hermes Configuration                    тФВ", Colors.CYAN))
    print(color("тФФтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФШ", Colors.CYAN))
    
    # Paths
    print()
    print(color("тЧЖ Paths", Colors.CYAN, Colors.BOLD))
    ...
    # API Keys
    print()
    print(color("тЧЖ API Keys", Colors.CYAN, Colors.BOLD))
    ...
    # Model settings
    print()
    print(color("тЧЖ Model", Colors.CYAN, Colors.BOLD))
    ...
    try:
        ...
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                тЪа .env has stale HERMES_MAX_ITERATIONS={_env_ghost} "
    ...
    # Display
    print()
    print(color("тЧЖ Display", Colors.CYAN, Colors.BOLD))
    display = config.get('display', {})
    ...
    # Terminal
    print()
    print(color("тЧЖ Terminal", Colors.CYAN, Colors.BOLD))
    ...
    # Timezone
    print()
    print(color("тЧЖ Timezone", Colors.CYAN, Colors.BOLD))
    ...
    # Compression
    print()
    print(color("тЧЖ Context Compression", Colors.CYAN, Colors.BOLD))
    ...
    if has_overrides:
        print()
        print(color("тЧЖ Auxiliary Models (overrides)", Colors.CYAN, Colors.BOLD))
    ...
    # Messaging
    print()
    print(color("тЧЖ Messaging Platforms", Colors.CYAN, Colors.BOLD))
    ...
    try:
        ...
        if skill_vars:
            ...
            print()
            print(color("тЧЖ Skill Settings", Colors.CYAN, Colors.BOLD))
...
def edit_config():
    ...
    if not editor:
        # Try common editors тАФ order is platform-aware so Windows users
...
def set_config_value(key: str, value: str):
    ...
    if key.upper() in api_keys or key.upper().endswith(('_API_KEY', '_TOKEN')) or key.upper().startswith('TERMINAL_SSH'):
        ...
        print(f"тЬУ Set {key} in {get_env_path()}")
        return
    ...
    print(f"тЬУ Set {key} = {value} in {config_path}")
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        print()
        print(color("ЁЯФД Checking configuration for updates...", Colors.CYAN, Colors.BOLD))
        print()
        ...
        if not missing_env and not missing_config and current_ver >= latest_ver:
            print(color("тЬУ Configuration is up to date!", Colors.GREEN))
            print()
            return
        ...
        if required_missing:
            print(f"\n  тЪая╕П  {len(required_missing)} required API key(s) missing:")
            for var in required_missing:
                print(f"     тАв {var['name']}")
        
        if optional_missing:
            print(f"\n  тД╣я╕П  {len(optional_missing)} optional API key(s) not configured:")
            for var in optional_missing:
                tools = var.get("tools", [])
                tools_str = f" (enables: {', '.join(tools[:2])})" if tools else ""
                print(f"     тАв {var['name']}{tools_str}")
        ...
        if results["env_added"] or results["config_added"]:
            print(color("тЬУ Configuration updated!", Colors.GREEN))
        
        if results["warnings"]:
            print()
            for warning in results["warnings"]:
                print(color(f"  тЪая╕П  {warning}", Colors.YELLOW))
        ...
        print()
        print(color("ЁЯУЛ Configuration Status", Colors.CYAN, Colors.BOLD))
        print()
        
        current_ver, latest_ver = check_config_version()
        if current_ver >= latest_ver:
            print(f"  Config version: {current_ver} тЬУ")
        else:
            print(color(f"  Config version: {current_ver} тЖТ {latest_ver} (update available)", Colors.YELLOW))
        
        print()
        print(color("  Required:", Colors.BOLD))
        for var_name in REQUIRED_ENV_VARS:
            if get_env_value(var_name):
                print(f"    тЬУ {var_name}")
            else:
                print(color(f"    тЬЧ {var_name} (missing)", Colors.RED))
        ...
        for var_name, info in OPTIONAL_ENV_VARS.items():
            if get_env_value(var_name):
                ...
                tools_str = f" тЖТ {', '.join(tools[:2])}" if tools else ""
                print(color(f"    тЧЛ {var_name}{tools_str}", Colors.DIM))
...
# тФАтФА Profile-driven env var injection тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
...
def _inject_profile_env_vars() -> None:
    ...
    Called once at module load time. Idempotent тАФ repeated calls are no-ops.
    """
...
# тФАтФА Platform-plugin env var injection тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
