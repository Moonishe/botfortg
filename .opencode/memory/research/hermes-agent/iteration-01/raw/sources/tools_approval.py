...
# instantly bypass all approval checks тАФ a prompt-injection escalation path.
...
def _is_gateway_approval_context() -> bool:
    ...
    ``approvals.cron_mode`` config, not interactive resolve тАФ letting cron
...
# time by _normalize_command_for_detection() тАФ see the rewrite step there тАФ so
...
# `cp`, etc. targeting it are gated too тАФ otherwise the deny is unpaired
...
# alone.
#
# The list is deliberately tiny тАФ only things with no recovery path:
...
# them through тАФ that's what yolo is for.
#
...
HARDLINE_PATTERNS = [
    ...
    # System shutdown / reboot тАФ anchor to command position (start of line,
...
# =========================================================================
# Sudo stdin guard тАФ block password guessing via "sudo -S"
# =========================================================================
...
# Treat this as an unconditional block тАФ there is never a legitimate
...
def _check_sudo_stdin_guard(command: str) -> tuple:
    ...
    internally тАФ that path is legitimate and handled elsewhere.  This guard
...
def _hardline_block_result(description: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "be executed via the agent тАФ not even with --yolo, /yolo, "
...
def _sudo_stdin_block_result(description: str) -> dict:
    ...
    return {
        ...
        "message": (
            ...
            "Do not pipe passwords to 'sudo -S' тАФ this is a brute-force "
...
DANGEROUS_PATTERNS = [
    ...
    # find -exec rm / -execdir rm тАФ the -execdir variant (same semantics,
    ...
    # Docker container lifecycle тАФ any user with docker.sock mounted (a common
    ...
    # project-relative env/config тАФ so `cp evil ~/.ssh/authorized_keys` (key
    ...
    # DESTINATION (last arg) only тАФ `cp evil ~/.ssh/authorized_keys` is gated,
    ...
    # anywhere in the args, not just the first token тАФ `perl -e '...'` (code
    # eval, no -i) does not trip because it has no `-...i` flag token.
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HERMES_CONFIG_PATH}|{_HERMES_ENV_PATH})', "in-place edit of Hermes config/env (perl/ruby)"),
    # Script execution via heredoc тАФ bypasses the -e/-c flag patterns above.
    ...
    # Script execution after chmod +x тАФ catches the two-step pattern where
    ...
    # matching, so case variants of S/s and A/a collapse тАФ both forms
    ...
    # Combined short-flag form: -nS, -ns, -sa, -las тАФ sudo flags packed
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # Strip shell backslash-escapes: r\m тЖТ rm. Prevents \-injection bypass.
    command = re.sub(r'\\([^\n])', r'\1', command)
    # Strip empty-string literals that split tokens: r''m тЖТ rm, r"\"m тЖТ rm.
    ...
    # after this module is imported тАФ as the hermetic test conftest does.
...
# subagents, execute_code RPC handlers) can block concurrently тАФ each gets
...
class _ApprovalEntry:
    ...
    def __init__(self, data: dict):
        ...
        self.data = data          # command, description, pattern_keys, тАж
        self.result: Optional[str] = None  # "once"|"session"|"always"|"deny"


_gateway_queues: dict[str, list] = {}        # session_key тЖТ [_ApprovalEntry, тАж]
_gateway_notify_cbs: dict[str, object] = {}  # session_key тЖТ callable(approval_data)
...
def register_gateway_notify(session_key: str, cb) -> None:
    ...
    *approval_data* contains ``command``, ``description``, and
    ``pattern_keys``.  The callback bridges syncтЖТasync (runs in the agent
...
Assess the ACTUAL risk of this command. Many flagged commands are false positives тАФ for example, `python -c "print('hello')"` is flagged as "script execution via -c flag" but is completely harmless.
...
def check_dangerous_command(command: str, env_type: str,
    ...
    if not is_cli and not is_gateway:
        ...
        logger.warning(
            ...
            "(pattern: %s): %s тАФ set HERMES_INTERACTIVE or HERMES_GATEWAY_SESSION to require approval.",
    ...
    if is_gateway or env_var_enabled("HERMES_EXEC_ASK"):
        ...
        return {
            ...
            "description": description,
            "message": (
                f"тЪая╕П This command is potentially dangerous ({description}). "
...
def _format_tirith_description(tirith_result: dict) -> str:
    ...
    return "Security scan тАФ " + "; ".join(parts)
...
def _await_gateway_decision(session_key: str, notify_cb, approval_data: dict,
    ...
    elapses тАФ firing pre/post approval hooks and cleaning up the queue entry.
    ...
    # Notify the user (bridges sync agent thread тЖТ async gateway)
    try:
    ...
    # inactivity tracker тАФ otherwise the gateway watchdog kills the agent
...
def check_all_command_guards(command: str, env_type: str,
    ...
    # Tirith check тАФ wrapper guarantees no raise for expected failures.
    ...
    try:
        ...
        pass  # tirith module not installed тАФ allow
    ...
    # Tirith block/warn тЖТ approvable warning with rich findings.
    ...
    if approval_mode == "smart":
        ...
        # verdict == "escalate" тЖТ fall through to manual prompt
    ...
    # Gateway/async approval тАФ block the agent thread until the user
    ...
    if is_gateway or is_ask:
        ...
        if notify_cb is not None:
            ...
            if not resolved or choice is None or choice == "deny":
                ...
                # deny is also a hard halt тАФ both produce a BLOCKED outcome
            ...
            # User approved тАФ persist based on scope (same logic as CLI)
            for key, _, is_tirith in warnings:
                if choice == "session" or (choice == "always" and is_tirith):
                    ...
                    save_permanent_allowlist(_permanent_approved)
                # choice == "once": no persistence тАФ command allowed this
        ...
        return {
            ...
            "description": combined_desc,
            "message": (
                f"тЪая╕П {combined_desc}. Asking the user for approval.\n\n**Command:**\n```\n{command}\n```"
            ),
        }
...
def check_execute_code_guard(code: str, env_type: str) -> dict:
    ...
    execute_code runs arbitrary local Python тАФ the script can call
    ...
    approved тАФ matching the existing terminal auto-approve contract. The
    ...
    # Isolated backends already sandbox the child тАФ matches the container skip
    ...
    # Check session/permanent approval тАФ same gate as check_all_command_guards.
    ...
    if approval_mode == "smart":
        ...
        # verdict == "escalate" тЖТ fall through to manual approval
    ...
    if notify_cb is None:
        ...
        return {
            ...
            "description": description,
            "message": (
                f"тЪая╕П {description}. Asking the user for approval.\n\n"
                f"**Code:**\n```python\n{code}\n```"
            ),
        }
    ...
    # Approved тАФ persist based on scope (same logic as check_all_command_guards).
    if choice == "session":
        approve_session(session_key, pattern_key)
    elif choice == "always":
        approve_session(session_key, pattern_key)
        approve_permanent(pattern_key)
        save_permanent_allowlist(_permanent_approved)
    # choice == "once": no persistence тАФ approval lasts this single call only.
