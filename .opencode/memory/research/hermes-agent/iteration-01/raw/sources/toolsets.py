...
_HERMES_CORE_TOOLS = [
    ...
    # via check_fn in tools/read_terminal_tool.py тАФ hidden outside the GUI).
    "read_terminal",
    # File manipulation
    ...
    # Kanban multi-agent coordination тАФ only in schema when the agent is
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `hermes tools` тЖТ X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "image_url) тАФ the active backend auto-routes. Configure via "
            "``hermes tools`` тЖТ Video Generation."
        ),
    ...
    "computer_use": {
        "description": (
            "Background macOS desktop control via cua-driver тАФ screenshots, "
    ...
    # "honcho" toolset removed тАФ Honcho is now a memory provider plugin.
    ...
    "kanban": {
        "description": (
            "Kanban multi-agent coordination тАФ only active when the agent "
    ...
    # Coding posture (base Hermes тАФ CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    "hermes-acp": {
        "description": "Editor integration (VS Code, Zed, JetBrains) тАФ coding-focused tools without messaging, audio, or clarify UI",
        "tools": [
            "web_search", "web_extract",
            "terminal", "process",
            "read_file", "write_file", "patch", "search_files",
    ...
    "hermes-api-server": {
        "description": "OpenAI-compatible API server тАФ full agent tools accessible via HTTP (no interactive UI tools like clarify or send_message)",
        "tools": [
            # Web
            "web_search", "web_extract",
            # Terminal + process management
    ...
    "hermes-cron": {
        ...
        # core tools users see interactively тАФ then `hermes tools` filters
    ...
    "hermes-yuanbao": {
        "description": "Yuanbao Bot хЕГхоЭц╢ИцБпх╣│хП░х╖ехЕ╖щЫЖ - ч╛дф┐бцБпуАБцИРхСШцЯешпвуАБчзБшБКуАБш┤┤ч║╕шбицГЕ",
        "tools": _HERMES_CORE_TOOLS + [
            "yb_query_group_info",
            "yb_query_group_members",
            "yb_send_dm",
...
def resolve_toolset(name: str, visited: Set[str] = None) -> List[str]:
    ...
    # Silently return [] тАФ either this is a diamond (not a bug, tools already
...
def _get_plugin_toolset_names() -> Set[str]:
    ...
    ``TOOLSETS`` dict тАФ i.e. they were added by plugins at load time.
    """
    try:
        from tools.registry import registry
        return {
