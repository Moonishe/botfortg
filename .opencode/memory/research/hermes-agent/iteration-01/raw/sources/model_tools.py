...
def _get_worker_loop():
    ...
    the coroutine, then *closes* the loop тАФ but cached httpx/AsyncOpenAI
...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        # Inside an async context (gateway, RL env) тАФ run in a fresh thread
        ...
        # only works on not-yet-started futures тАФ it's a no-op on a running
        ...
        try:
            ...
            if loop_ready.wait(timeout=1.0) and worker_loop is not None:
                try:
                    ...
                    # Loop already closed тАФ nothing to cancel.
                    pass
            raise
        finally:
    ...
    # lifetime тАФ preventing "Event loop is closed" on GC cleanup.
...
# the first user message тАФ freezing Discord/Telegram heartbeats for up to
...
def get_tool_definitions(
    ...
    if quiet_mode:
        ...
        if cached is not None:
            ...
            # Return a shallow copy of the list but share the dict references тАФ
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        for toolset_name in effective_enabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if not quiet_mode:
                    print(f"тЬЕ Enabled toolset '{toolset_name}': {', '.join(resolved) if resolved else 'no tools'}")
            elif toolset_name in _LEGACY_TOOLSET_MAP:
                legacy_tools = _LEGACY_TOOLSET_MAP[toolset_name]
                tools_to_include.update(legacy_tools)
                if not quiet_mode:
                    print(f"тЬЕ Enabled legacy toolset '{toolset_name}': {', '.join(legacy_tools)}")
            elif not quiet_mode:
                print(f"тЪая╕П  Unknown toolset: {toolset_name}")
    else:
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if not quiet_mode:
                    print(f"ЁЯЪл Disabled toolset '{toolset_name}': {', '.join(resolved) if resolved else 'no tools'}")
            elif toolset_name in _LEGACY_TOOLSET_MAP:
                legacy_tools = _LEGACY_TOOLSET_MAP[toolset_name]
                tools_to_include.difference_update(legacy_tools)
                if not quiet_mode:
                    print(f"ЁЯЪл Disabled legacy toolset '{toolset_name}': {', '.join(legacy_tools)}")
            elif not quiet_mode:
                print(f"тЪая╕П  Unknown toolset: {toolset_name}")

    # Plugin-registered tools are now resolved through the normal toolset
    # path тАФ validate_toolset() / resolve_toolset() / get_all_toolsets()
    ...
    # other tools by name тАФ otherwise the model sees tools mentioned in
    ...
    if not quiet_mode:
        if filtered_tools:
            ...
            print(f"ЁЯЫая╕П  Final tool selection ({len(filtered_tools)} tools): {', '.join(tool_names)}")
        else:
            print("ЁЯЫая╕П  No tools selected (all filtered out or unavailable)")
    ...
    # silently accept тАФ bare "type": "object" with no properties,
    ...
    except Exception as e:  # pragma: no cover тАФ defensive
        logger.warning("Schema sanitization skipped: %s", e)

    # тФАтФА Tool Search (progressive disclosure) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    #
    # This is deliberately the last step before returning тАФ sanitization
    ...
    try:
        ...
        if not skip_tool_search_assembly and ts_cfg.enabled != "off":
            ...
            if assembly.activated and not quiet_mode:
                print(
                    f"ЁЯФО Tool Search: {assembly.deferred_count} MCP/plugin tools deferred "
                    f"(~{assembly.deferred_tokens} tokens) behind tool_search/describe/call. "
                    f"Threshold ~{assembly.threshold_tokens} tokens."
                )
            filtered_tools = assembly.tool_defs
    except Exception as e:  # pragma: no cover тАФ never break tool loading
...
def _resolve_active_context_length() -> int:
    ...
    Returns 0 when the model can't be resolved тАФ ``should_activate`` falls
...
# becomes part of the conversation. It's defense-in-depth тАФ the json layer
# already prevents framing escape тАФ but cheap and worth having.
#
...
def coerce_tool_args(tool_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for key, value in list(args.items()):
        ...
        # ``None`` itself is preserved тАФ we don't know whether the model
            ...
            if isinstance(value, str):
                ...
                if value.strip().startswith("["):
                    logger.warning(
                        ...
                        "but could not be parsed тАФ model may have emitted a "
...
def _coerce_value(value: str, expected_type, schema: dict | None = None):
    ...
    if isinstance(expected_type, list):
        # Union type тАФ try each in order, return first successful coercion
...
def _coerce_json(value: str, expected_python_type: type):
    ...
    if isinstance(parsed, expected_python_type):
        ...
        return parsed
    logger.warning(
        "coerce_tool_args: JSON-parsed value is %s, expected %s тАФ skipping coercion",
...
def _coerce_number(value: str, integer_only: bool = False):
    ...
    try:
        ...
        return value
    # Guard against inf/nan тАФ not JSON-serializable, keep original string
    ...
    if f == int(f):
        return int(f)
    if integer_only:
        # Schema wants an integer but value has decimals тАФ keep as string
        return value
    return f
...
def _emit_post_tool_call_hook(
    *,
    ...
    No-ops cheaply when no plugin has registered for ``post_tool_call`` тАФ
...
def handle_function_call(
    ...
    """
    # Coerce string arguments to their schema-declared types (e.g. "42"тЖТ42)
    function_args = coerce_tool_args(function_name, function_args)
    if not isinstance(function_args, dict):
        function_args = {}
    _tool_middleware_trace = list(tool_request_middleware_trace or [])

    # тФАтФА Tool Search bridge dispatch тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    # tool_search and tool_describe are pure catalog reads тАФ handle them
    ...
    try:
        ...
        # checked тАФ e.g. run_agent._invoke_tool passes skip=True to
        ...
        # pass. When skip=True, the caller already fired it тАФ do nothing
        # here.
