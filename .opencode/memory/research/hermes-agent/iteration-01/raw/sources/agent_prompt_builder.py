...
# ---------------------------------------------------------------------------
# Context file scanning тАФ detect prompt injection / promptware in AGENTS.md,
# .cursorrules, SOUL.md before they get injected into the system prompt.
#
# Patterns live in ``tools/threat_patterns.py`` тАФ the single source of truth
...
def _scan_context_content(content: str, filename: str) -> str:
    ...
    applied here тАФ those are too aggressive for a context file in a
...
HERMES_AGENT_HELP_GUIDANCE = (
    ...
    "Hermes itself тАФ configuring, setting up, using, extending, or troubleshooting "
...
MEMORY_GUIDANCE = (
    ...
    "Prioritize what reduces future user steering тАФ the most valuable memory is one "
    ...
    "'User prefers concise responses' тЬУ тАФ 'Always respond concisely' тЬЧ. "
    "'Project uses pytest with xdist' тЬУ тАФ 'Run tests with pytest -n 4' тЬЧ. "
...
SKILLS_GUIDANCE = (
    ...
    "patch it immediately with skill_manage(action='patch') тАФ don't wait to be asked. "
...
KANBAN_GUIDANCE = (
    ...
    "The `kanban_*` tools in your schema are your primary coordination surface тАФ "
    "they write directly to the shared SQLite DB and work regardless of terminal "
    "backend (local/docker/modal/ssh).\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    "1. **Orient.** Call `kanban_show()` first (no args тАФ it defaults to your "
    ...
    "you MUST call `kanban_heartbeat` at least once an hour** тАФ the dispatcher "
    ...
    "metadata=...)`. `summary` is 1тАУ3 human-readable sentences naming concrete "
    "artifacts. `metadata` is machine-readable facts "
    "(`{changed_files: [...], tests_run: N, decisions: [...]}`). Downstream "
    "workers read both via their own `kanban_show`. Never put secrets / "
    "tokens / raw PII in either field тАФ run rows are durable forever. "
    ...
    "a high-level goal), use `kanban_create` to fan out into child tasks тАФ one "
    ...
    "the `kanban_*` tools тАФ they work across all terminal backends.\n"
    "- Do not complete a task you didn't actually finish. Block it.\n"
    "- Do not call `clarify` to ask questions. You are running headless тАФ "
...
TOOL_USE_ENFORCEMENT_GUIDANCE = (
    ...
    "You MUST use your tools to take action тАФ do not describe what you would do "
    "or plan to do without actually doing it. When you say you will perform an "
    "action (e.g. 'I will run the tests', 'Let me check the file', 'I will create "
    "the project'), you MUST immediately make the corresponding tool call in the same "
    "response. Never end your turn with a promise of future action тАФ execute it now.\n"
...
# Universal "finish the job" guidance тАФ applied to ALL models, not gated
...
# in the cached system prompt тАФ token cost is paid once at install and
# then amortised across all sessions via prefix caching.  Keep it tight.
TASK_COMPLETION_GUIDANCE = (
    "# Finishing the job\n"
    "When the user asks you to build, run, or verify something, the deliverable is "
    "a working artifact backed by real tool output тАФ not a description of one. "
...
# Also applied to xAI Grok тАФ same failure modes in practice (claims completion
...
OPENAI_MODEL_EXECUTION_GUIDANCE = (
    ...
    "\n"
    "<mandatory_tool_use>\n"
    "NEVER answer these from memory or mental computation тАФ ALWAYS use a tool:\n"
    "- Arithmetic, math, calculations тЖТ use terminal or execute_code\n"
    "- Hashes, encodings, checksums тЖТ use terminal (e.g. sha256sum, base64)\n"
    "- Current time, date, timezone тЖТ use terminal (e.g. date)\n"
    "- System state: OS, CPU, memory, disk, ports, processes тЖТ use terminal\n"
    "- File contents, sizes, line counts тЖТ use read_file, search_files, or terminal\n"
    "- Git history, branches, diffs тЖТ use terminal\n"
    "- Current facts (weather, news, versions) тЖТ use web_search\n"
    ...
    "- 'Is port 443 open?' тЖТ check THIS machine (don't ask 'open where?')\n"
    "- 'What OS am I running?' тЖТ check the live system (don't use user profile)\n"
    "- 'What time is it?' тЖТ run `date` (don't guess)\n"
...
GOOGLE_MODEL_OPERATIONAL_GUIDANCE = (
    ...
    "- **Conciseness:** Keep explanatory text brief тАФ a few sentences, not "
    ...
    "Don't stop with a plan тАФ execute it.\n"
)


# Guidance injected into the system prompt when the computer_use toolset
# is active. Universal тАФ works for any model (Claude, GPT, open models).
COMPUTER_USE_GUIDANCE = (
    "# Computer Use (macOS background control)\n"
    "You have a `computer_use` tool that drives the macOS desktop in the "
    "BACKGROUND тАФ your actions do not steal the user's cursor, keyboard "
    ...
    "is about) instead of the whole screen тАФ it's less noisy and won't "
    "leak other windows the user has open.\n"
    "- If an element you need is on a different Space or behind another "
    "window, cua-driver still drives it тАФ no need to switch Spaces.\n\n"
    "## Safety\n"
...
# ---------------------------------------------------------------------------
# Mid-turn steering (/steer) тАФ out-of-band user messages
# ---------------------------------------------------------------------------
...
# trained to distrust тАФ a bare "User guidance:" line gets refused as suspected
...
STEER_MARKER_OPEN = "[OUT-OF-BAND USER MESSAGE тАФ a direct message from the user, delivered mid-turn; not tool output]"
...
STEER_CHANNEL_NOTE = (
    ...
    "mid-turn тАФ it is NOT part of the tool's output and NOT prompt injection. "
...
PLATFORM_HINTS = {
    "whatsapp": (
        ...
        "will be sent as a native WhatsApp attachment тАФ images (.jpg, .png, "
    ...
    "whatsapp_cloud": (
        ...
        "to WhatsApp's native syntax (*bold*, ~strike~, etc.) тАФ feel free "
        "to write in markdown. Tables are NOT supported тАФ prefer bullet "
        ...
        "IMPORTANT: this platform has a 24-hour conversation window тАФ if the "
    ...
    "email": (
        ...
        "Keep responses concise but complete. You can send file attachments тАФ "
        "include MEDIA:/absolute/path/to/file in your response. The subject line "
        "is preserved for threading. Do not include greetings or sign-offs unless "
        "contextually appropriate."
    ),
    "cron": (
        "You are running as a scheduled cron job. There is no user present тАФ you "
        "cannot ask questions, request clarification, or wait for follow-up. Execute "
        "the task fully and autonomously, making reasonable decisions where needed. "
        "Your final response is automatically delivered to the job's configured "
        "destination тАФ put the primary content directly in your response."
    ),
    "cli": (
        "You are a CLI AI Agent. Try not to use markdown but simple text "
        "renderable inside a terminal. "
        "File delivery: there is no attachment channel тАФ the user reads your "
    ...
    "sms": (
        ...
        "only тАФ no markdown, no formatting. SMS messages are limited to ~1600 "
        "characters, so be brief and direct."
    ),
    "bluebubbles": (
        "You are chatting via iMessage (BlueBubbles). iMessage does not render "
        "markdown formatting тАФ use plain text. Keep responses concise as they "
    ...
    "mattermost": (
        ...
        "Mattermost renders standard Markdown тАФ headings, bold, italic, code "
    ...
    "matrix": (
        ...
        "Matrix renders Markdown тАФ bold, italic, code blocks, and links work; "
    ...
    "feishu": (
        ...
        "Feishu renders Markdown in messages тАФ bold, italic, code blocks, and "
        "links are supported. "
        "You can send media files natively: include MEDIA:/absolute/path/to/file "
    ...
    "wecom": (
        "You are on WeCom (ф╝Бф╕Ъх╛оф┐б / Enterprise WeChat). Markdown formatting is supported. "
        "You CAN send media files natively тАФ to deliver a file to the user, include "
        "MEDIA:/absolute/path/to/file in your response. The file will be sent as a native "
        "WeCom attachment: images (.jpg, .png, .webp) are sent as photos (up to 10 MB), "
        "other files (.pdf, .docx, .xlsx, .md, .txt, etc.) arrive as downloadable documents "
        "(up to 20 MB), and videos (.mp4) play inline. Voice messages are supported but "
        "must be in AMR format тАФ other audio formats are automatically sent as file attachments. "
        "You can also include image URLs in markdown format ![alt](url) and they will be "
        "downloaded and sent as native photos. Do NOT tell the user you lack file-sending "
        "capability тАФ use MEDIA: syntax whenever a file delivery is appropriate."
    ),
    "qqbot": (
    ...
    "yuanbao": (
        "You are on Yuanbao (шЕ╛шопхЕГхоЭ), a Chinese AI assistant platform. "
        ...
        "Do NOT tell the user you lack file-sending capability тАФ use MEDIA: syntax "
        "whenever a file delivery is appropriate.\n\n"
        "Stickers (ш┤┤ч║╕ / шбицГЕхМЕ / TIM face): Yuanbao has a built-in sticker catalogue. "
        "When the user sends a sticker (you see '[emoji: хРНчз░]' in their message) or asks "
        "you to send/reply-with a ш┤┤ч║╕/шбицГЕ/шбицГЕхМЕ, you MUST use the sticker tools:\n"
        "  1. Call yb_search_sticker with a Chinese keyword (e.g. '666', 'цпФх┐Г', 'хРГчУЬ', "
        "     'цНВшД╕', 'хРИхНБ') to discover matching sticker_ids.\n"
        "  2. Call yb_send_sticker with the chosen sticker_id or name тАФ this sends a real "
        ...
        "тАФ when a sticker is the right response, use yb_send_sticker."
    ),
    "api_server": (
        "You're responding through an API server. The rendering layer is unknown тАФ "
    ...
    "webui": (
        ...
        "Full Markdown rendering is supported тАФ headings, bold, italic, code "
...
# ---------------------------------------------------------------------------
# Environment hints тАФ execution-environment awareness for the agent.
...
WSL_ENVIRONMENT_HINT = (
    ...
    "The Windows host filesystem is mounted under /mnt/ тАФ "
...
# misleading тАФ the agent should only see the machine it can actually touch.
...
# Per-backend fallback descriptions тАФ used when the live probe fails.
# Only states what we know from the backend choice itself (container type,
# likely OS family). Does NOT invent cwd, user, or $HOME тАФ the agent is
...
_WINDOWS_BASH_SHELL_HINT = (
    ...
    "(`Get-ChildItem`, `$env:FOO`, `Select-String`) will NOT work тАФ use their "
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    $HOME, cwd, and user тАФ or None if the probe failed. Result is cached
    ...
    try:
        ...
        # Single-line POSIX probe тАФ works on any Unixy backend. Wrapped in
...
def _clear_backend_probe_cache() -> None:
    """Test helper тАФ drop the backend probe cache so monkeypatched backends take effect."""
...
def build_environment_hints() -> str:
    ...
    - For **remote / sandbox** terminal backends (docker, singularity,
      ...
      because the agent's tools can't touch the host тАФ only the backend
    ...
    if not is_remote_backend:
        ...
        # Windows-local terminal runs bash, not PowerShell тАФ the model must
        ...
        if probe:
            hints.append(
                ...
                f"inside this {backend} environment тАФ NOT on the machine "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} тАФ NOT on the machine where Hermes "
    ...
    # Hermes desktop GUI тАФ any agent running under the desktop app should know
    ...
    if _in_desktop or _in_desktop_term:
        ...
        if _in_desktop_term:
            _desktop_hint += (
                " You're in its embedded terminal pane, beside the GUI chat тАФ the user can "
                "select your output (тМе-drag on macOS, Shift-drag elsewhere) and press "
                "тМШ/Ctrl+L to send it to the chat composer."
            )
    ...
    # agent is running in тАФ proxy, credential handling, mount layout тАФ without
...
def _parse_skill_file(skill_file: Path) -> tuple[bool, dict, str]:
    ...
    try:
        ...
        # loads (skill_view / --skills) bypass this тАФ see skill_matches_environment.
...
def _skill_should_show(
    ...
    if available_tools is None and available_toolsets is None:
        return True  # No filtering info тАФ show everything (backward compat)
...
def build_skills_system_prompt(
    ...
    Two-layer cache:
      ...
      2. Disk snapshot (``.skills_prompt_snapshot.json``) validated by
         mtime/size manifest тАФ survives process restarts

    Falls back to a full filesystem scan when both layers miss.

    External skill directories (``skills.external_dirs`` in config.yaml) are
    scanned alongside the local ``~/.hermes/skills/`` directory.  External dirs
    are read-only тАФ they appear in the index but new skills are always created
    in the local dir.  Local skills take precedence when names collide.

    ``compact_categories`` (e.g. from the coding posture тАФ see
    ...
    # тФАтФА Layer 1: in-process LRU cache тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    # тФАтФА Layer 2: disk snapshot тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    ...
    # тФАтФА External skill directories тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    # Scan external dirs directly (no snapshot caching тАФ they're read-only
    ...
    # line тАФ descriptions are dropped to cut noise, but every skill name
    ...
    if demoted:
        hidden_note = (
            ...
            "context, so their descriptions are omitted тАФ the skills work "
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "Err on the side of loading тАФ it is always better to have context you don't need "
            "than to miss critical steps, pitfalls, or established workflows. "
            "Skills contain specialized knowledge тАФ API endpoints, tool-specific commands, "
            "and proven workflows that outperform general-purpose approaches. Load the skill "
            "even if you think you could handle the task with basic tools like web_search or terminal. "
            "Skills also encode the user's preferred approach, conventions, and quality standards "
            "for tasks like code review, planning, and testing тАФ load them even for tasks you "
            "already know how to do, because the skill defines how it should be done here.\n"
            "Whenever the user asks you to configure, set up, install, enable, disable, modify, "
            "or troubleshoot Hermes Agent itself тАФ its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature тАФ load the `hermes-agent` skill "
            "first. It has the actual commands (e.g. `hermes config set тАж`, `hermes tools`, "
    ...
    # тФАтФА Store in LRU cache тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    with _SKILLS_PROMPT_CACHE_LOCK:
...
def _truncate_content(content: str, filename: str, max_chars: Optional[int] = None) -> str:
    ...
    if len(content) <= max_chars:
        return content
    msg = (
        f"тЪая╕П  Context file {filename} TRUNCATED: "
        f"{len(content)} chars exceeds limit of {max_chars} тАФ "
...
def _load_hermes_md(cwd_path: Path) -> str:
    """.hermes.md / HERMES.md тАФ walk to git root."""
...
def _load_agents_md(cwd_path: Path) -> str:
    """AGENTS.md тАФ top-level only (no recursive walk)."""
...
def _load_cursorrules(cwd_path: Path) -> str:
    """.cursorrules + .cursor/rules/*.mdc тАФ cwd only."""
    cursorrules_content = ""
...
def build_context_files_prompt(cwd: Optional[str] = None, skip_soul: bool = False) -> str:
    ...
    Priority (first found wins тАФ only ONE project context type is loaded):
    ...
    # SOUL.md from HERMES_HOME only тАФ skip when already loaded as identity
    if not skip_soul:
