...
mode parameter):
  ...
  1. DISCOVERY тАФ pass ``query``. Runs FTS5, dedupes hits by session lineage,
  ...
  2. SCROLL тАФ pass ``session_id`` + ``around_message_id``. Returns a window
  ...
  3. BROWSE тАФ no args. Returns recent sessions chronologically (titles,
     previews, timestamps).
...
No LLM calls anywhere тАФ every shape returns actual messages from the DB.
...
# delegate subagent runs are tagged "subagent" тАФ neither belongs in the
...
def _shape_message(m: Dict[str, Any], anchor_id: Optional[int] = None) -> Dict[str, Any]:
    ...
    # (absent content is meaningful тАФ tool-call-only assistant turns).
...
def _resolve_profile_db(profile: str):
    ...
    runs in profile A. ``read_only=True`` (mode=ro) takes no write lock тАФ safe
...
def _locate_session_db(session_id: str):
    ...
    bare id тАФ we find it wherever it actually lives instead of failing.
    """
...
def _read_session(db, session_id: str, head: int = 20, tail: int = 10) -> str:
    ...
    Serves the linked-session case тАФ the user dropped an @session reference and
...
def _scroll(
    ...
    No FTS5, no bookends тАФ just the slice. The discovery shape's lineage
    ...
    # Reject scrolling inside the active session lineage тАФ those messages are
    # already in context.
...
def _discover(
    ...
    # row тАФ only that pairs validly with the FTS5 match id for the anchored
...
def session_search(
    ...
    Read:      pass ``session_id`` (no anchor) тАФ dumps the whole session.
    Browse:    pass nothing.
    ...
    anchor is set тАФ the agent has asked for a specific slice.
    """
    if db is None:
        try:
            from hermes_state import SessionDB
    ...
    # Session ids never contain "/", so a slash unambiguously means profile/id тАФ
    ...
    # Scroll shape takes precedence тАФ explicit anchor beats any query.
    ...
    # Read shape: a session_id with no anchor тЖТ dump the whole session.
    if isinstance(session_id, str) and session_id.strip():
        ...
        # Miss in the target profile тАФ the model may have dropped the owning
    ...
    # Browse shape: no query тЖТ recent sessions.
...
SESSION_SEARCH_SCHEMA = {
    ...
    "description": (
        ...
        "FTS5-backed retrieval over the SQLite message store. No LLM calls тАФ every "
        "shape returns actual messages from the DB.\n\n"
        "FOUR CALLING SHAPES\n\n"
        "  1) DISCOVERY тАФ pass `query`:\n"
        ...
        "     Bookends + window together let you reconstruct goal тЖТ match тЖТ resolution "
        "without paying for the whole transcript.\n\n"
        "  2) SCROLL тАФ pass `session_id` + `around_message_id`:\n"
        "     session_search(session_id=\"...\", around_message_id=12345, window=10)\n"
        "     Returns a window of ┬▒`window` messages centered on the anchor. No FTS5, "
        "no bookends тАФ just the slice. Use after a discovery call when you need more "
        ...
        "       - The boundary message appears in both windows тАФ orientation marker.\n"
        "       - When messages_before or messages_after is < window, you're at the "
        "start or end of the session.\n\n"
        "  3) READ тАФ pass `session_id` only (no around_message_id):\n"
        ...
        "FTS5 SYNTAX\n\n"
        "  AND is the default тАФ multi-word queries require all terms. Use OR explicitly "
        ...
        "\"find the session where Z\" question тАФ before gh, web search, or filesystem "
        ...
        "properties": {
            ...
            "limit": {
                ...
                "description": (
                    ...
                    "Bump to 5тАУ10 when the topic likely spans several sessions and you "
            ...
            "sort": {
                ...
                "description": (
                    ...
                    "to keep relevance-only ordering (suitable for exploratory recall тАФ "
