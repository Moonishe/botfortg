...
# ---------------------------------------------------------------------------
# Write-path deny list тАФ blocks writes to sensitive system/credential files
# ---------------------------------------------------------------------------
...
def _detect_line_ending(sample: str) -> Optional[str]:
    ...
    patch operations тАФ without this the agent's bare-LF tool args
    silently normalize Windows-line-ending files, and patch produces
    mixed endings when only a substituted region changes.
    """
    if not sample:
        return None
    # Look at the first chunk тАФ enough to tell, cheap to scan.
    head = sample[:4096]
    if "\r\n" in head:
        return "\r\n"
    if "\n" in head:
...
# write when the original file had one тАФ exactly mirroring the line-ending
...
def _split_tool_diagnostics(output: str) -> tuple[str, str]:
    ...
    that look like real search output тАФ a match line (``file:line:content``),
    ...
    exit-2 guard distinguish a pure failure (no usable payload тЖТ surface the
    error) from a partial failure (some files matched, one was unreadable тЖТ
...
#   "cannot find package" тАФ already partially handled by
...
# extensions тАФ the ``lsp_diagnostics`` channel carries the real signal.
...
# couldn't actually run тАФ e.g. ``npx tsc`` when tsc isn't installed in
...
def _lint_json_inproc(content: str) -> tuple[bool, str]:
    ...
    except Exception as e:  # noqa: BLE001 тАФ any parse failure is a lint failure
...
def _lint_yaml_inproc(content: str) -> tuple[bool, str]:
    ...
    Skipped gracefully if PyYAML isn't installed тАФ YAML parsing is optional.
    """
    try:
        import yaml as _yaml
    except ImportError:
        # PyYAML not available тАФ skip silently, caller treats as no linter.
...
def _lint_python_inproc(content: str) -> tuple[bool, str]:
    ...
    ast module rejects тАФ matching py_compile's scope but with no
...
# present тАФ no subprocess overhead, microseconds per call.  Each callable
...
class ShellFileOperations(FileOperations):
    ...
    def __init__(self, terminal_env, cwd: str = None):
        ...
        Args:
            ...
            cwd: Optional explicit fallback cwd when the terminal env has
                 no cwd attribute (rare тАФ most backends track cwd live).
        ...
        Note:
            ...
            when the env has no cwd at all тАФ it is NOT the authoritative
    ...
    def _exec(self, command: str, cwd: str = None, timeout: int = None,
        ...
        Cwd resolution order (critical тАФ see class docstring):
        ...
        terminal's current directory тАФ not the directory this file_ops was
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        (the model hand-counted and was off-by-one, 3/4) тАФ so we keep the
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        #    silent best-effort тАФ a perms-copy failure must not abort the
    ...
    def _detect_file_line_ending(self, path: str, pre_content: Optional[str] = None) -> Optional[str]:
        ...
        for lint/LSP purposes), inspect that тАФ zero extra exec calls.
        ...
        # File may not exist (new write) тАФ `head` exits 0 with empty
    ...
    def read_file(self, path: str, offset: int = 1, limit: int = 500) -> ReadResult:
        ...
        # Images are never inlined тАФ redirect to the vision tool
    ...
    def _suggest_similar_files(self, path: str) -> ReadResult:
        ...
        scored: list = []  # (score, filepath) тАФ higher is better
    ...
    def read_file_raw(self, path: str) -> ReadResult:
        ...
        if self._is_likely_binary(path, sample_output):
            return ReadResult(
                ...
                error="Binary file тАФ cannot display as text."
            )
        ...
        # back out тАФ it re-probes the on-disk file, which still has the
        # marker тАФ so the round-trip preserves it.
    ...
    def delete_file(self, path: str) -> WriteResult:
        ...
        don't ship ``rm``. Directories are rejected here тАФ use
    ...
    def _python_delete(self, path: str, recursive: bool) -> WriteResult:
        ...
        # We can't shell out to ``rm`` here тАФ it doesn't exist on Windows
        ...
        snippet = (
            ...
            "    else:\n"
            # NOTE: avoid ``unlink(missing_ok=True)`` тАФ that kwarg lands in
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        files. The content never appears in the shell command string тАФ
        ...
        write are surfaced тАФ pre-existing problems are filtered out so
        ...
        #      map тАФ pre-existing diagnostics below the edit point shift
        ...
        # тФАтФА Line-ending preservation (Roo Code pattern) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        ...
        # тФАтФА BOM preservation тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        # If the file on disk started with a UTF-8 BOM, keep it. read_file
        # strips the BOM so the agent never sees it, which means the
        # content it hands back to write_file / patch has no BOM either тАФ
        ...
        # of a half-written corrupt file. Same-directory is load-bearing тАФ
        ...
        # Semantic diagnostics from the LSP layer тАФ separate channel.
    ...
    def patch_replace(self, path: str, old_string: str, new_string: str,
        ...
        # тФАтФА Line-ending preservation тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        ...
        # Post-write verification тАФ re-read the file and confirm the bytes we
    ...
    def _check_lint(self, path: str, content: Optional[str] = None) -> LintResult:
        ...
        TOML) when possible тАФ those parse via the Python stdlib in
        ...
        if inproc is not None:
            # Need content тАФ either passed in or read from disk.
            if content is None:
                read_cmd = f"cat {self._escape_shell_arg(path)} 2>/dev/null"
                read_result = self._exec(read_cmd)
                if read_result.exit_code != 0:
        ...
        if ext in _SHELL_LINTER_LSP_REDUNDANT and self._lsp_will_handle(path):
            return LintResult(
                skipped=True,
                message=f"LSP server handles {ext} тАФ shell linter skipped",
            )
        ...
        if result.exit_code != 0 and _looks_like_linter_unusable(base_cmd, result.stdout):
            ...
            # not a real lint failure тАФ surface it as ``skipped`` so the
            # write doesn't get flagged AND so the LSP tier still runs.
            from tools.ansi_strip import strip_ansi
            cleaned = strip_ansi(result.stdout).strip()
            # Collapse to a single line тАФ the npx banner is multi-line ASCII.
            first_line = next(
    ...
    def _check_lint_delta(self, path: str, pre_content: Optional[str],
        ...
        Args:
            ...
            pre_content: File content BEFORE the write.  Pass None for new
                         files or when the pre-state isn't available тАФ the
        ...
        if pre.success or pre.skipped or not pre.output:
            # Pre-write was clean (or we couldn't lint it) тАФ post errors
        ...
        # later ones тАФ if the pre-existing error blocks parsing before
        ...
        # new on top тАФ the agent knows it's inherited state, not fresh
        ...
        if not post_lines:
            # Every error in post was also in pre тАФ this edit didn't make
            # anything obviously worse, but the file remains broken and
            # the agent should know.
            return LintResult(
                success=False,
                output=post.output,
                message="Pre-existing lint errors тАФ this edit didn't introduce new ones but the file is still broken.",
            )
    ...
    def _lsp_local_only(self) -> bool:
        ...
        LSP servers run on the host process тАФ they need access to the
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        Safe to call on remote backends тАФ the registry is purely
    ...
    def _lsp_will_handle(self, path: str) -> bool:
        ...
        Stronger than :meth:`_lsp_handles_extension` тАФ that one only checks
        ...
        runs as before тАФ never suppress lint based on an LSP probe that
    ...
    def _snapshot_lsp_baseline(self, path: str) -> None:
        ...
        Best-effort.  Silent on every failure path тАФ LSP is an
        enrichment layer and must never break a write.

        Skipped entirely on non-local backends (Docker, Modal, SSH,
        etc.) тАФ the server can't see files inside the sandbox.
        """
        if not self._lsp_local_only():
            return
        try:
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        тАФ the calling tier already returned a clean syntax result, so
        ``""`` here just means "no extra info to add".

        Skipped entirely on non-local backends (Docker, Modal, SSH,
        etc.) тАФ same reasoning as ``_snapshot_lsp_baseline``.
        """
        if not self._lsp_local_only():
            return ""
        try:
        ...
        # Build a line-shift map when we have both pre and post тАФ it
