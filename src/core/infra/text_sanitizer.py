...
class _Cleaner(HTMLParser):
    ...
    def handle_starttag(self, tag: str, attrs):
        ...
        if tag in _BLOCK_TO_NEWLINE:
            # Nested <ul> inside <ol>: skip newline — parent structure already set
            if tag == "ul" and self._in_ol:
                return
            if self.parts and not self.parts[-1].endswith("\n"):
                self.parts.append("\n")
            return
        ...
        if norm == "a":
            ...
            for k, v in attrs:
                if k.lower() == "href" and v:
                    ...
                    break
            if href:
                # Remove all whitespace so "java\tscript:" cannot bypass
                # the scheme check.
                href_lower = "".join(href.split()).lower()
