...
# prompt_toolkit is an optional CLI dependency тАФ only needed for
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("fast", "Toggle fast mode тАФ OpenAI Priority Processing / Anthropic Fast Mode (Normal/Fast)", "Configuration",
...
def is_gateway_known_command(name: str | None) -> bool:
    ...
    ``command:<name>`` hooks тАФ plugin commands get the same lifecycle
    events as built-ins.
    """
    if not name:
        return False
...
# "all resolvable commands" тАФ which is the real bypass set (see
...
def should_bypass_active_session(command_name: str | None) -> bool:
    ...
    a "busy тАФ wait or /stop first" response. In both paths the command
    ...
    the pending queue тАФ which meant a mid-run /model (or /reasoning,
...
def telegram_bot_commands() -> list[tuple[str, str]]:
    ...
    for cmd in COMMAND_REGISTRY:
        if not _is_gateway_available(cmd, overrides):
            continue
        # Built-in arg-taking commands are included тАФ their handlers show
...
_TELEGRAM_MENU_PRIORITY = (
    ...
    "sessions",
    "model",
    # Maintenance / diagnostics тАФ the ones that prompted this priority list.
    "debug",
    "restart",
    "update",
    "verbose",
...
# Backward-compat alias тАФ tests and external code may reference the old name.
...
def _sanitize_telegram_name(raw: str) -> str:
    ...
    Steps: lowercase тЖТ replace hyphens with underscores тЖТ strip all other
    invalid characters тЖТ collapse consecutive underscores тЖТ strip leading/
        ...
        if len(name) > _CMD_NAME_LIMIT:
            ...
            if candidate in used:
                ...
                for digit in range(10):
                    ...
                    # All 10 digit slots exhausted тАФ skip entry
                    continue
            name = candidate
        if name in used:
            continue
...
def _collect_gateway_skill_entries(
    ...
    # Plugins have no cmd_key тАФ use empty string as placeholder
    ...
    # Skills fill remaining slots тАФ only tier that gets trimmed
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    User-installed hub skills are excluded тАФ accessible via /skills.
    ...
    # Drop the cmd_key тАФ Telegram only needs (name, desc) pairs.
...
def discord_skill_commands(
    ...
    - Hyphens are allowed in names (no ``-`` тЖТ ``_`` sanitization)
    ...
    return _collect_gateway_skill_entries(
        platform="discord",
        max_slots=max_slots,
        reserved_names=set(reserved_names),  # copy тАФ don't mutate caller's set
        desc_limit=100,
    )
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    ``skills.external_dirs`` тАФ matching the widened filter applied to the
    ...
    The legacy 25-group ├Ч 25-subcommand caps (from the old nested
    ``/skill <cat> <name>`` layout) are **not** applied тАФ the live caller
    ...
    # Map clamped-32-char-name тЖТ what it came from, so we can emit an
    ...
    # on the 32-char clamp" тАФ the latter is the rename-worthy case.
    ...
    try:
        ...
        # becomes its own scan root for category derivation тАФ a skill at
        ...
        for cmd_key in sorted(skill_cmds):
            ...
            if discord_name in _names_used:
                ...
                # meant skill authors had no way to discover the drop тАФ
                ...
                if prior == "<reserved>":
                    logger.warning(
                        ...
                        "clamp with a reserved gateway command name %r тАФ the "
                    ...
                    logger.warning(
                        ...
                        "Discord's 32-char command-name limit тАФ only %r "
            ...
            # scan root. e.g. creative/ascii-art/SKILL.md тЖТ ("creative", ...)
...
# gets clamped ("reset" was unpinned for exactly that тАФ /new keeps its
...
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional тАФ
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    for alias in _SLACK_PRIORITY_ALIASES:
        ...
        if cmd is not None:
            _add(alias, f"Alias for /{cmd.name} тАФ {cmd.description}", cmd.args_hint or "")
...
def slack_app_manifest(request_url: str = "https://hermes-agent.local/slack/commands") -> dict[str, Any]:
    ...
    The returned dict is the ``features.slash_commands`` portion only тАФ
...
class SlashCommandCompleter(Completer):
    ...
    # - Adding space makes "/model" тЖТ "/model " which blocks picker execution
    ...
    def _completion_text(cmd_name: str, word: str) -> str:
        ...
        NOT get a trailing space тАФ the TUI would apply the completion on Enter
        and block the picker from opening.
        """
        if cmd_name != word:
            return cmd_name
        # Don't add space for picker commands тАФ allows Enter to execute them
    ...
    def _context_completions(self, word: str, limit: int = 30):
        ...
        # Bare @ or @partial тАФ fuzzy project-wide file search
    ...
    def _fuzzy_file_completions(self, word: str, query: str, limit: int = 20):
        ...
        if not query:
            # No query тАФ show recently modified files (already sorted by mtime)
    ...
    def _tools_completions(sub_text: str, sub_lower: str):
        """Yield completions for /tools тАФ subcommand + toolset/MCP-server name.
    ...
    def _handoff_completions(sub_text: str, sub_lower: str):
        ...
        home channel is NOT required to list a platform тАФ it's often learned at
        runtime тАФ so the meta hints whether one is set yet. Completes only the
        ...
        for platform in platforms:
            ...
            try:
                ...
                home = None
            meta = f"тЖТ {home.name}" if home and getattr(home, "name", None) else "send this session here"
            yield Completion(
                name,
                start_position=-len(partial),
                display=name,
    ...
    def _personality_completions(sub_text: str, sub_lower: str):
        ...
        try:
            # Resolve from the same source the runtime applies personalities тАФ
        ...
        for cmd, info in self._iter_skill_bundles().items():
            ...
            if cmd_name.startswith(word):
                ...
                yield Completion(
                    ...
                    start_position=-len(word),
                    display=cmd,
                    display_meta=f"тЦг {short_desc} ({skill_count} skills)",
                )
            ...
            for cmd_name, cmd_info in get_plugin_commands().items():
                if cmd_name.startswith(word):
                    ...
                    yield Completion(
                        ...
                        start_position=-len(word),
                        display=f"/{cmd_name}",
                        display_meta=f"ЁЯФМ {short_desc}",
                    )
        except Exception:
            pass
...
class SlashCommandAutoSuggest(AutoSuggest):
    ...
    def get_suggestion(self, buffer, document):
        ...
        if len(parts) == 1 and not text.endswith(" "):
            # Still typing the command name: /upd тЖТ suggest "ate"
            word = text[1:].lower()
            for cmd in COMMANDS:
        ...
        # Command is complete тАФ suggest subcommands
        sub_text = parts[1] if len(parts) > 1 else ""
        sub_lower = sub_text.lower()
