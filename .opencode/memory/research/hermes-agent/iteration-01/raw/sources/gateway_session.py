...
def _hash_chat_id(value: str) -> str:
    ...
    ``telegram:12345`` тЖТ ``telegram:<hash>``
    ``12345``          тЖТ ``<hash>``
    """
    colon = value.find(":")
    if colon > 0:
...
from .config import (
    Platform,
    GatewayConfig,
    SessionResetPolicy,  # noqa: F401 тАФ re-exported via gateway/__init__.py
    HomeChannel,
)
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      ...
      2. `DISCORD_BOT_TOKEN` is set тАФ the tool's `check_fn` gates on it
         at registry time, so the toolset being enabled in config is not
         enough if the token isn't configured.

    Returns False (safe default тАФ keeps the stale-API disclaimer) on any
...
def build_session_context_prompt(
    ...
    *,
    ...
    # conversation.  Don't pin a single user name in the system prompt тАФ it
    ...
    if context.shared_multi_user_session:
        ...
        lines.append(
            f"**Session type:** {session_label} тАФ messages are prefixed "
    ...
    if context.source.platform == Platform.SLACK:
        ...
        lines.append(
            ...
            "You do NOT have access to Slack-specific APIs тАФ you cannot search "
        ...
        # Discord tools loaded this session тАФ i.e. the user opted into
        ...
        if _discord_tools_loaded():
            ...
            lines.append(
                ...
                "You do NOT have access to Discord-specific APIs тАФ you cannot search "
        ...
        lines.append(
            ...
            "Keep responses short and conversational тАФ think texts, not essays. "
            ...
            "one idea per bubble, 1тАУ3 sentences each. "
    ...
    for p in context.connected_platforms:
        if p != Platform.LOCAL:
            platforms_list.append(f"{p.value}: Connected тЬУ")
    ...
    # Origin delivery
    if context.source.platform == Platform.LOCAL:
        lines.append("- `\"origin\"` тЖТ Local output (saved to files)")
    else:
        _origin_label = context.source.chat_name or (
            _hash_chat_id(context.source.chat_id) if redact_pii else context.source.chat_id
        )
        lines.append(f"- `\"origin\"` тЖТ Back to this chat ({_origin_label})")

    # Local always available
    lines.append(
        f"- `\"local\"` тЖТ Save to local files only ({display_hermes_home()}/cron/output/)"
    )

    # Platform home channels
    for platform, home in context.home_channels.items():
        lines.append(f"- `\"{platform.value}\"` тЖТ Home channel ({home.name})")
...
class SessionEntry:
    ...
    # context-note prepend тАФ both wrong for an explicit manual reset.
    # See issue #6508.
    ...
    # restarts тАФ prevents redundant finalization runs.
    ...
    # ``resume_pending`` preserves the existing session_id on next access тАФ
...
def build_session_key(
    ...
    Group/channel rules:
      ...
      - thread_id differentiates threads within that parent chat.  When
        ...
        participants тАФ user_id is NOT appended, so every user in the thread
    ...
    if source.chat_type == "dm":
        ...
        # No chat_id тАФ fall back to the sender's own identifier before the
        ...
        # single cached agent ends up serving multiple people's conversations тАФ
...
class SessionStore:
    ...
    def _is_session_expired(self, entry: SessionEntry) -> bool:
        ...
        Works from the entry alone тАФ no SessionSource needed.
    ...
    def has_any_sessions(self) -> bool:
        ...
        stay at 1 for single-platform users тАФ which is the bug this fixes.
    ...
    def get_or_create_session(
        ...
        with self._lock:
            ...
            if session_key in self._entries and not force_new:
                ...
                # broke a stuck loop тАФ #7536).  ``suspended`` is the hard
                ...
                if entry.suspended:
                    ...
                    # means a re-interrupted retry keeps trying тАФ the
    ...
    def mark_resume_pending(
        ...
        with self._lock:
            ...
            if session_key in self._entries:
                entry = self._entries[session_key]
                # Never override an explicit ``suspended`` тАФ that is a hard
    ...
    def prune_old_entries(self, max_age_days: int) -> int:
        ...
        how old it is.  Entries marked ``suspended`` are kept тАФ the user
        ...
        the transcript in SQLite stays, but the session_key тЖТ session_id
        ...
        with self._lock:
            ...
            for key, entry in list(self._entries.items()):
                ...
                # attached тАФ the user may still be waiting on output.
        ...
        if self._db and not skip_db:
            try:
                self._db.append_message(
                    ...
                    # Platform-side message id (yuanbao msg_id, telegram update_id, тАж).
    ...
    def load_transcript(self, session_id: str) -> List[Dict[str, Any]]:
        ...
        in spec 002 тАФ pre-DB sessions on existing disks have already been
