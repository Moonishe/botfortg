...
def _collect_delegate_child_ids(conn, parent_ids: List[str]) -> List[str]:
    ...
    backfilled by the v16 migration) тАФ generic untagged children keep the
...
# silently тАФ /resume, /title, /history, /branch, kanban dispatcher, etc.
#
# Instead, fall back to ``journal_mode=DELETE`` (the pre-WAL default) which
# works on NFS.  Concurrency drops тАФ concurrent readers are blocked during
# a write тАФ but the feature works.
...
# do not update it (by design тАФ kanban failures are reported via their
...
# this, kanban_db.connect() (called on every kanban operation тАФ see
...
def _set_last_init_error(msg: Optional[str]) -> None:
    ...
    this to SET on failure тАФ it deliberately does NOT clear on success,
...
def format_session_db_unavailable(prefix: str = "Session database not available") -> str:
    ...
    responded with a bare ``"Session database not available."`` тАФ no
    ...
    Example output:
        ...
        on NFS/SMB тАФ see https://www.sqlite.org/wal.html).
    """
    ...
    if any(marker in cause.lower() for marker in _WAL_INCOMPAT_MARKERS):
        hint = " (state.db may be on NFS/SMB/FUSE тАФ see https://www.sqlite.org/wal.html)"
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    back to DELETE mode тАФ the pre-WAL default, which works on NFS тАФ and
    ...
    Never downgrades to DELETE if the on-disk DB header reports WAL тАФ see _on_disk_journal_mode.
    """
    # Read-only probe тАФ no flock, no checkpoint, no WAL/SHM unlink.
    ...
    try:
        ...
        if not any(marker in msg for marker in _WAL_INCOMPAT_MARKERS):
            # Unrelated OperationalError тАФ don't silently swallow.
            raise
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation тАФ see hermes_cli/kanban_db.py) would
    ...
    logger.warning(
        "%s: WAL journal_mode unsupported on this filesystem (%s) тАФ "
...
# the ``sqlite_master`` schema table itself becomes inconsistent тАФ most
...
# before it runs тАФ including ``PRAGMA journal_mode`` (which is why this trips
...
#
# The canonical ``sessions`` / ``messages`` data is intact in these cases тАФ
# only the derived schema is broken тАФ so recovery preserves all transcripts
...
def repair_state_db_schema(db_path: Path, *, backup: bool = True) -> Dict[str, Any]:
    ...
    # тФАтФА Strategy 1: de-duplicate sqlite_master (keeps FTS index) тФАтФА
    try:
        conn = sqlite3.connect(str(db_path), isolation_level=None)
        try:
    ...
    # тФАтФА Strategy 2: drop all FTS schema, VACUUM, rebuild on next open тФАтФА
    try:
        conn = sqlite3.connect(str(db_path), isolation_level=None)
        try:
            conn.execute("PRAGMA writable_schema=ON")
...
class SessionDB:
    ...
    # тФАтФА Write-contention tuning тФАтФА
        ...
        try:
            ...
            def _connect_and_init():
                self._conn = sqlite3.connect(
                    str(self.db_path),
                    check_same_thread=False,
                    # Short timeout тАФ application-level retry with random
            ...
            try:
                ...
                # row for messages_fts) fails on the very first statement тАФ
                # before _init_schema can run тАФ so it can't be caught at the
                ...
                if not is_malformed_db_error(exc) or not _claim_repair_attempt(self.db_path):
                    raise
                logger.error(
                    "state.db schema is malformed (%s) тАФ attempting automatic "
    ...
    # тФАтФА Core write helper тФАтФА
    ...
    def _execute_write(self, fn: Callable[[sqlite3.Connection], T]) -> T:
        ...
        *fn* receives the connection and should perform INSERT/UPDATE/DELETE
        statements.  The caller must NOT call ``commit()`` тАФ that's handled
        here after *fn* returns.

        BEGIN IMMEDIATE acquires the WAL write lock at transaction start
        (not at commit time), so lock contention surfaces immediately.
        On ``database is locked``, we release the Python lock, sleep a
        random 20-150ms, and retry тАФ breaking the convoy pattern that
            ...
            try:
                with self._lock:
                    ...
                    try:
                        ...
                        try:
                            ...
                            pass
                        raise
                # Success тАФ periodic best-effort checkpoint.
                self._write_count += 1
                ...
                if "locked" in err_msg or "busy" in err_msg:
                    ...
                    if attempt < self._WRITE_MAX_RETRIES - 1:
                        ...
                        time.sleep(jitter)
                        continue
                # Non-lock error or retries exhausted тАФ propagate.
                raise
    ...
    def _try_wal_checkpoint(self) -> None:
        ...
        truncates the WAL file тАФ the file stays at its high-water
        ...
        try:
            ...
            pass  # Best effort тАФ never fatal.
    ...
    def _parse_schema_columns(schema_sql: str) -> Dict[str, Dict[str, str]]:
        ...
        Uses an in-memory SQLite database to parse the SQL тАФ SQLite itself
    ...
    def _reconcile_columns(self, cursor: sqlite3.Cursor) -> None:
        ...
        This makes column additions a declarative operation тАФ just add
    ...
    def _init_schema(self):
        ...
        # тФАтФА Declarative column reconciliation тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        ...
        # AFTER _reconcile_columns runs тАФ declaring them in SCHEMA_SQL
        ...
        # column (idx_messages_session_active) тАФ same ordering constraint.
        ...
        # тФАтФА Schema version bookkeeping тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
        ...
        if row is None:
            ...
            if current_version < 16:
                ...
                # parent deletes that used to orphan them (parent_session_id тЖТ NULL).
                try:
                    cursor.execute(
        ...
        # Unique title index тАФ always ensure it exists
        try:
            cursor.execute(
    ...
    # new). That mutation is local to the AIAgent instance тАФ but ``state.db``
    ...
    # writes тАФ Damien's "parent тЖТ two orphan children" repro shape.
    #
    ...
    def try_acquire_compression_lock(
        ...
        if another holder already owns a non-expired lock тАФ the caller
        ...
        def _do(conn):
            ...
            # difference тАФ verify ownership via SELECT.
            conn.execute(
    ...
    def get_compression_lock_holder(self, session_id: str) -> Optional[str]:
        ...
        Diagnostic helper тАФ not used by the locking protocol itself.
        """
        if not session_id:
            return None
        now = time.time()
    ...
    def update_token_counts(
        ...
        When *absolute* is False (default), values are **incremented** тАФ use
        this for per-API-call deltas (CLI path).

        When *absolute* is True, values are **set directly** тАФ use this when
    ...
    def set_session_archived(self, session_id: str, archived: bool) -> bool:
        ...
        their messages тАФ this is a soft hide, not a delete. For compression
    ...
    def get_next_title_in_lineage(self, base_title: str) -> str:
        """Generate the next title in a lineage (e.g., "my session" тЖТ "my session #2").
    ...
    def get_compression_tip(self, session_id: str) -> Optional[str]:
        ...
        """
        current = session_id
        # Bound the walk defensively тАФ compression chains this deep are
    ...
    def list_sessions_rich(
        ...
        continuation тАФ one logical conversation = one list entry, showing the
        ...
        if not include_children:
            ...
            # live тАФ i.e., started_at < parent.ended_at).
            #
            ...
            #      the branch тАФ see issue #20856.
        ...
        # compression chain тАФ so searching a compression *root* id or a *tip*
        ...
        if order_by_last_active:
            ...
            if id_needle:
                ...
                # the small result set keep this bounded тАФ far cheaper than
    ...
    def list_cron_job_runs(
        ...
        ``id_query`` path тАФ that path seeds from *every* ``source='cron'`` row in
    ...
    def get_messages(
        ...
        timestamp тАФ see c03acca50 for the WSL2 clock-regression rationale.
        """
    ...
    def get_messages_around(
        ...
        ``session_id`` тАФ callers decide how to surface that.
        """
        if window < 0:
            window = 0
        with self._lock:
    ...
    def get_anchored_view(
        ...
        Built on top of ``get_messages_around``. Three slices:
          ...
          - ``bookend_start``: first ``bookend`` user/assistant messages of the
            session тАФ but only those whose id is strictly before the window's
        ...
        (opening) and the resolution (closing) on a single call тАФ without
        ...
        # Reuse the primitive тАФ handles anchor-existence, content decoding,
        ...
        # by id range, role, and non-empty content тАФ tool-call-only assistant
    ...
    def resolve_resume_session_id(self, session_id: str) -> str:
        ...
        child is where new messages actually land тАФ the parent ends up with
    ...
    # =========================================================================
    # Rewind (soft-delete) тАФ see /rewind slash command + issue #21910
    # =========================================================================
    ...
    def rewind_to_message(
        ...
        disk with ``active=0`` for audit / forensic inspection тАФ use
        ...
        *session_id* or if its role is not ``"user"``.
        ...
        Always increments ``sessions.rewind_count`` тАФ even when the
        target is already inactive тАФ so the counter accurately reflects
    ...
    def list_recent_user_messages(
        ...
        for row in rows:
            ...
            if isinstance(decoded, list):
                # Multimodal тАФ flatten text parts.
                text_parts = [
    ...
    def _sanitize_fts5_query(query: str) -> str:
        ...
        # parses as ``column:term`` and raises "no such column" тАФ swallowed at
    ...
    def search_messages(
        ...
        # splits CJK characters into individual tokens, so "хдзхИлх▒▒щб╣чЫо" becomes
        # "хдз AND хИл AND х▒▒ AND щб╣ AND чЫо" тАФ producing false positives and
        ...
        # CJK queries (1-2 chars), trigram can't match (it needs тЙе9 UTF-8
        ...
        if is_cjk:
            ...
            # per token. A query like "х╣┐ше┐ OR цбВцЮЧ OR ц╝Уц▒Я" has cjk_count=6
            # (>=3) but each individual token is only 2 chars тАФ trigram returns 0.
            ...
            if cjk_count >= 3 and not _any_short_cjk:
                # Trigram FTS5 path тАФ quote each non-operator token to handle
                ...
                # For multi-token OR queries (e.g. "х╣┐ше┐ OR цбВцЮЧ OR ц╝Уц▒Я"),
            ...
            with self._lock:
                try:
                    ...
                    # FTS5 query syntax error despite sanitization тАФ return empty
                    return []
                else:
    ...
    def session_count(
        ...
        totals) so the total matches the number of listable rows тАФ otherwise the
    ...
    def delete_session(
        ...
        (``parent_session_id тЖТ NULL``) so they remain accessible independently.
    ...
    def delete_session_if_empty(
        ...
        children (delegate subagent runs) are preserved тАФ a parent that
    ...
    def delete_sessions(
        ...
        * Unknown IDs are silently skipped (no 404) тАФ selection state
        ...
        * Delegate subagent children (``model_config._delegate_from``) are
          ...
          (``parent_session_id тЖТ NULL``) so they stay accessible.
        ...
        * On-disk transcript / ``request_dump_*`` files are cleaned up
        ...
        def _do(conn):
            ...
            # First, filter to IDs that actually exist тАФ we want to
            ...
            # of survivors тАФ the IN list on ``parent_session_id`` does
            # exactly this.
            conn.execute(
                f"UPDATE sessions SET parent_session_id = NULL "
    ...
    def count_empty_sessions(self) -> int:
        ...
        spawned session whose first message hasn't landed yet тАФ or one
        held open by the live agent тАФ is never sniped out from under
        the runtime.
    ...
    def delete_empty_sessions(
        ...
        * Orphans any child whose parent is in the kill list тАФ children
        ...
        * Deletes the rows in a single ``_execute_write`` callback so
          the operation is atomic тАФ a partial failure (e.g. SIGKILL
        ...
        * Cleans up on-disk transcript files (``.json`` / ``.jsonl`` /
          ...
          ``request_dump_*`` if it crashed before the first reply тАФ
        ...
        def _do(conn):
            ...
            for sid in session_ids:
                # DELETE FROM messages is paranoia тАФ by construction
                # these rows have ``message_count = 0`` тАФ but if a
    ...
    # тФАтФА Meta key/value (for scheduler bookkeeping) тФАтФА
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        Schema versions:
          v1 тАФ initial shape (no ON DELETE CASCADE on session_id FK)
          v2 тАФ session_id FK gets ON DELETE CASCADE so session pruning
        ...
        def _do(conn):
            ...
            # v1 тЖТ v2: rebuild telegram_dm_topic_bindings if its session_id FK
        ...
        *,
        ...
        def _do(conn):
            try:
                ...
                # Tables don't exist yet тАФ nothing to disable.
                return
        self._execute_write(_do)
    ...
    def list_unlinked_telegram_sessions_for_user(
        ...
        *,
        ...
        just returns this user's Telegram sessions тАФ there can't be any
        bindings yet.
        """
        with self._lock:
            try:
                ...
                # telegram_dm_topic_bindings doesn't exist yet тАФ no bindings
    ...
    # тФАтФА Space reclamation тФАтФА
    ...
    def optimize_fts(self) -> int:
        ...
        FTS5 indexes grow as a series of incremental segments тАФ one per
        ...
        This is purely a maintenance operation тАФ it changes neither search
    ...
    def vacuum(self) -> int:
        ...
        SQLite does not shrink the database file when rows are deleted тАФ
        ...
        layout-only optimization тАФ search results are unchanged.
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        Returns a dict with keys:
          - ``"skipped"`` (bool) тАФ true if within min_interval_hours of last run
          - ``"pruned"`` (int)   тАФ number of sessions deleted
          - ``"vacuumed"`` (bool) тАФ true if VACUUM ran
          - ``"error"`` (str, optional) тАФ present only on failure
        """
        ...
        try:
            ...
            # Only VACUUM if we actually freed rows тАФ VACUUM on a tight DB
    ...
    # тФАтФА Handoff (cross-platform session transfer) тФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФАтФА
    #
    # State machine:
    #   None       тАФ no handoff in flight
    #   "pending"  тАФ CLI requested handoff, gateway hasn't picked it up yet
    #   "running"  тАФ gateway is processing (session switch + synthetic turn)
    #   "completed"тАФ gateway successfully delivered the synthetic turn
    #   "failed"   тАФ gateway hit an error; reason in handoff_error
    #
    # The CLI writes "pending" then poll-waits for terminal state. The gateway
    # watcher transitions pendingтЖТrunningтЖТ{completed,failed}.
    ...
    def claim_handoff(self, session_id: str) -> bool:
        """Atomically transition pending тЖТ running. Returns True if claimed."""
        def _do(conn):
            cur = conn.execute(
