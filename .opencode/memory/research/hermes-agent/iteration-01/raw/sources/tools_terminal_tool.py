...
# ---------------------------------------------------------------------------
from tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 тАФ re-exported
...
# routines. Stored in thread-local state so overlapping ACP sessions тАФ
# each running in its own ThreadPoolExecutor thread тАФ don't stomp on
...
def set_sudo_password_callback(cb):
    ...
    Per-thread scope тАФ ACP sessions that run concurrently in a
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    for failure in sudo_failures:
        if failure in output:
            ...
            return output + f"\n\nЁЯТб Tip: To enable sudo over messaging, add SUDO_PASSWORD to {_dhh()}/.env on the agent machine."
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    try:
        ...
        print()
        print("тФМ" + "тФА" * 58 + "тФР")
        print("тФВ  ЁЯФР SUDO PASSWORD REQUIRED" + " " * 30 + "тФВ")
        print("тФЬ" + "тФА" * 58 + "тФд")
        print("тФВ  Enter password below (input is hidden), or:            тФВ")
        print("тФВ    тАв Press Enter to skip (command fails gracefully)     тФВ")
        print(f"тФВ    тАв Wait {timeout_seconds}s to auto-skip" + " " * 27 + "тФВ")
        print("тФФ" + "тФА" * 58 + "тФШ")
        print()
        ...
        if result["done"]:
            ...
            if password:
                print("  тЬУ Password received (cached for this session)")
            else:
                print("  тПн Skipped - continuing without sudo")
            print()
            sys.stdout.flush()
            return password
        else:
            print("\n  тП▒ Timeout - continuing without sudo")
        ...
        print()
        print("  тПн Cancelled - continuing without sudo")
        print()
        sys.stdout.flush()
        return ""
    except Exception as e:
...
def _rewrite_compound_background(command: str) -> str:
    ...
    never exits. It leaks as a process stuck in ``wait4`` forever тАФ and
    ...
    strings and parenthesised subshells. Leaves simple ``cmd &`` alone тАФ
    ...
    while i < n:
        ...
        # Newline terminates a statement at depth 0 тАФ reset chain state.
        ...
        # Comments (only at statement start тАФ conservative: any `#` not inside
        ...
        # Quoted tokens тАФ consume whole string via the shared tokenizer.
        if ch in {"'", '"'}:
        ...
        # output (`A && { B & }`) is idempotent тАФ the inner `&` is part of
        ...
        # Inside parens or brace groups, skip operators тАФ they parse in their
        ...
        if ch == "&":
            ...
            if i + 1 < n and command[i + 1] == ">":
                # `&>` redirect тАФ consume
                i += 2
                continue
            # `>&` / `<&` fd target тАФ look back past whitespace
            j = i - 1
        ...
        # Regular unquoted token тАФ advance past it via the shared tokenizer
        _, next_i = _read_shell_token(command, i)
        i = max(next_i, i + 1)
    ...
    for chain_end, amp_pos in reversed(rewrites):
        ...
        # preceded by `;` or `&` тАФ we're providing `&` from the backgrounding.
...
Do NOT use cat/head/tail to read files тАФ use read_file instead.
Do NOT use grep/rg/find to search тАФ use search_files instead.
Do NOT use ls to list directories тАФ use search_files(target='files') instead.
Do NOT use sed/awk to edit files тАФ use patch instead.
Do NOT use echo/cat heredoc to create files тАФ use write_file instead.
...
Foreground (default): Commands return INSTANTLY when done, even if the timeout is high. Set timeout=300 for long builds/scripts тАФ you'll still get the result in seconds if it's fast. Prefer foreground for short commands.
Background: Set background=true to get a session_id. Almost always pair with notify_on_complete=true тАФ bg without notify runs SILENTLY and you have no way to learn it finished short of calling process(action='poll') yourself. Two legitimate uses:
  (1) Long-lived processes that never exit (servers, watchers, daemons) тАФ silent is correct, there's no exit to notify on.
  (2) Long-running bounded tasks (tests, builds, deploys, CI pollers, batch jobs) тАФ MUST set notify_on_complete=true. Without it you'll either forget to poll or sit blocked waiting for the user to surface the result.
...
Do NOT use vim/nano/interactive tools without pty=true тАФ they hang without a pseudo-terminal. Pipe git output to cat if it might page.
"""
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    profile that match the issue #20561 leak class тАФ containers left behind
    ...
    only Exited containers older than ``2 ├Ч lifetime_seconds`` and scoped to
    the current profile.

    Gates:

    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      operator opted out тАФ usually because they're running multiple
      Hermes processes in the same profile and don't trust the
      conservative defaults).
    * ``_docker_orphan_reaper_ran`` flag тАФ sweep runs once per Python
    ...
    # 2 ├Ч lifetime_seconds gives sibling Hermes processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    tracking) are *not* isolation signals тАФ they should not cause each
...
def _get_env_config() -> Dict[str, Any]:
    ...
    return {
        ...
        # "ONE long-lived container shared across sessions" тАФ this toggle
        ...
        # Conservative: only sweeps Exited containers older than 2├Ч the
...
def cleanup_vm(task_id: str, *, force_remove: bool = False):
    ...
    *force_remove* (default False) is forwarded to backends that accept it
    тАФ currently only ``DockerEnvironment``. The default of False matches
    ...
    contract тАФ exactly the bug Ben reported when the container was killed
    ...
    via this function), so persist-mode idle envs are similarly no-op'd тАФ
...
def _atexit_cleanup():
    ...
    if _active_environments:
        ...
        # interpreter exits. Issue #20561 тАФ without this join, the daemon
...
def _interpret_exit_code(command: str, exit_code: int) -> str | None:
    ...
    # Extract the last command in a pipeline/chain тАФ that determines the
    ...
    semantics: dict[str, dict[int, str]] = {
        ...
        "git":   {1: "Non-zero exit (often normal тАФ e.g. 'git diff' returns 1 when files differ)"},
    }
...
def _resolve_notification_flag_conflict(
    *,
    ...
    These flags produce duplicate, delayed notifications when combined тАФ one
...
def terminal_tool(
    ...
    Args:
        ...
        watch_patterns: List of strings to watch for in background output. HARD rate limit: 1 notification per 15s per process. After 3 strike windows in a row, watch_patterns is disabled and the session is auto-promoted to notify_on_complete. Use ONLY for rare, one-shot mid-process signals on long-lived processes (server readiness, migration-done markers). NEVER use in loops/batch jobs тАФ error patterns there will hit the strike limit and get disabled. MUTUALLY EXCLUSIVE with notify_on_complete тАФ set one, not both.
    ...
    try:
        ...
        # a timeout above FOREGROUND_MAX_TIMEOUT тАФ nudge it toward background.
        ...
        if background:
            ...
            try:
                ...
                # ran fine, exited green, agent never noticed тАФ user had to
                ...
                if background and not notify_on_complete and not watch_patterns:
                    result_data["hint"] = (
                        ...
                        "this process runs SILENTLY тАФ you will not be told when "
                ...
                #     empty conclusion тЖТ poller declares all-green while 18/23
                ...
                # every one of these тАФ drive the loop off exit codes or on
                ...
                if background and command:
                    ...
                    _bad_shape = (
                        ...
                        # gh pr checks piped to jq is also wrong тАФ `gh pr
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "#31448, #31695, #31709, #31745, #32264, #33131) тАФ "
                            ...
                            "on `process(action='log')` to read THAT file тАФ "
            ...
            # formatting тАФ prevents it from copying escapes into file writes.
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from daytona import Daytona  # noqa: F401 тАФ SDK presence check
...
if __name__ == "__main__":
    ...
    if not check_terminal_requirements():
        print("\nтЭМ Requirements not met. Please check the messages above.")
        sys.exit(1)
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "background": {
                "type": "boolean",
                "description": "Run the command in the background. Almost always pair with notify_on_complete=true тАФ without it, the process runs silently and you'll have no way to learn it finished short of calling process(action='poll') yourself (easy to forget, leading to silent blindness on long jobs). Two legitimate patterns: (1) Long-lived processes that never exit (servers, watchers, daemons) тАФ these stay silent because there's no exit to notify on. (2) Long-running bounded tasks (tests, builds, deploys, CI pollers, batch jobs) тАФ these MUST set notify_on_complete=true. For short commands, prefer foreground with a generous timeout instead.",
                "default": False
            },
            "timeout": {
                "type": "integer",
                "description": f"Max seconds to wait (default: 180, foreground max: {FOREGROUND_MAX_TIMEOUT}). Returns INSTANTLY when command finishes тАФ set high for long tasks, you won't wait unnecessarily. Foreground timeout above {FOREGROUND_MAX_TIMEOUT}s is rejected; use background=true for longer commands.",
                "minimum": 1
            },
            "workdir": {
                "type": "string",
            ...
            "notify_on_complete": {
                "type": "boolean",
                "description": "When true (and background=true), you'll be automatically notified exactly once when the process finishes. **This is the right choice for almost every long-running task** тАФ tests, builds, deployments, multi-item batch jobs, anything that takes over a minute and has a defined end. Use this and keep working on other things; the system notifies you on exit. MUTUALLY EXCLUSIVE with watch_patterns тАФ when both are set, watch_patterns is dropped.",
                "default": False
            },
            "watch_patterns": {
                "type": "array",
                "items": {"type": "string"},
                "description": "Strings to watch for in background process output. HARD RATE LIMIT: at most 1 notification per 15 seconds per process тАФ matches arriving inside the cooldown are dropped. After 3 consecutive 15-second windows with dropped matches, watch_patterns is automatically disabled for that process and promoted to notify_on_complete behavior (one notification on exit, no more mid-process spam). USE ONLY for truly rare, one-shot mid-process signals on LONG-LIVED processes that will never exit on their own тАФ e.g. ['Application startup complete'] on a server so you know when to hit its endpoint, or ['migration done'] on a daemon. DO NOT use for: (1) end-of-run markers like 'DONE'/'PASS' тАФ use notify_on_complete instead; (2) error patterns like 'ERROR'/'Traceback' in loops or multi-item batch jobs тАФ they fire on every iteration and you'll hit the strike limit fast; (3) anything you'd ever combine with notify_on_complete. When in doubt, choose notify_on_complete. MUTUALLY EXCLUSIVE with notify_on_complete тАФ set one, not both."
            }
        },
        "required": ["command"]
    }
